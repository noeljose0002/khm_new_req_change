					...
					<div class="row">
						...
						<div class="col-xl-3">
							...
							<select class="form-control nice-select" name="copy_season_id" id="copy_season_id">
								...
								<?php if (!empty($all_veh_seasons)) {
									foreach ($all_veh_seasons as $vals) {
										echo '<option value="' . $vals['season_id'] . '">' . $vals['season_name'] . '</option>';
									}
								} else {
									echo '<option value="">Seasons are not defined</option>';
								} ?>
							</select>
						</div>
						...
						<div class="page-rightheader">
							<div class="ml-3 ml-auto d-flex">
								...
								<span class="mt-3 mt-md-0 pg-header">
									<a href="<?= site_url('Dashboard/add_object_vehicle/' . $object_class_id); ?>" class="btn btn-success ml-0 ml-md-4 mt-1 "><i class="typcn typcn-plus mr-1"></i>Create New</a>
								</span>
							</div>
						</div>
					</div>
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<div class="table-responsive">
										<table id="vehicle_table" class="table table-bordered table-striped" cellspacing="0" width="100%">
											<thead style="background-color:#c6ecd9;">
												<tr>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#copy_season_id').on('change', function() {
			...
			if (!seasonId) {
				// Clear fields
				modal.find('#rate_per_day').val('').trigger('input');
				modal.find('#km_rate').val('').trigger('input');
				modal.find('#extra_km_rate').val('').trigger('input');
				return;
			}

			$.ajax({
				url: '<?= site_url('Dashboard/get_veh_season_rates') ?>',
				type: 'POST',
				data: {
					season_id: seasonId
				},
				...
				success(response) {
					...
					if (!response.success || !response.data.length) {
						return alert('Season rate details not found.');
					}

					// assume data is an array with one element
					const rateData = response.data[0];

					const modal = $('#seasonsmodal');
					modal
						.find('#rate_per_day')
						.val(rateData.rate_per_day)
						.trigger('input');
					modal
						.find('#km_rate')
						.val(rateData.km_rate)
						.trigger('input');
					modal
						.find('#extra_km_rate')
						.val(rateData.extra_km_rate)
						.trigger('input');

					console.log('After update:');
					console.log('rate_per_day =', modal.find('#rate_per_day').val());
					console.log('km_rate     =', modal.find('#km_rate').val());
					console.log('extra_km_rate =', modal.find('#extra_km_rate').val());

				},
				error() {
					alert('Error fetching season rates.');
				}
			});
		});
	});
	...
	function loadCopySeasons(object_id) {
		const select = $('#copy_season_id');
		$.ajax({
			url: '<?= site_url('Dashboard/veh_seasons_modal_veh'); ?>',
			type: 'POST',
			dataType: 'json',
			data: {
				object_id
			...
			success: function(res) {
				...
				(res.all_veh_seasons || []).forEach(item => {
					select.append(
			...
			error: function() {
				select.html('<option value="">Error loading seasons</option>');
			}
		});
	}
	$('#seasonsmodal').on('shown.bs.modal', function() {
		const object_id = $('#hd_object_id_ssn').val();
		loadCopySeasons(object_id);
	});
	...
	$(document).ready(function() {
		...
		$('#vehicle_table').DataTable({
			...
			'ajax': {
				'url': '<?= site_url('Dashboard/vehicle_list_view'); ?>',
				'type': 'POST',
				'data': function(d) {
			...
			'columns': [{
				...
				{
					data: 'tour_travel_daily_rate'
				},
				{
					data: 'tour_travel_max_km'
				},
				{
					data: 'extra_km_rate'
				},
...

				{
					...
					render: function(data, type, row, meta) {
						...
						return '<a class="season_form" data-id-object=' + data + ' data-id-name=' + obj_name + ' data-toggle="tooltip" data-original-title="Season" href=""><i class="fa fa-car" style="color:#339966"></i></a>';
					}
				},
				{
					data: 'object_id',
					render: function(data, type, row, meta) {
						return '<a data-toggle="tooltip" data-original-title="Edit" href="<?= site_url('Dashboard/add_object_vehicle/'); ?>' + object_class_id + '/' + data + '"><i class="fa fa-edit" style="color:#339966"></i></a>';
					}
				}
				/*{
                data: 'object_ph_no',
                render: function (data, type, row, meta) {
					var mob_numbers_array = JSON.parse(data);
					return '<label>'+mob_numbers_array[0]+'</label>';
                }
            },
			{
                data: 'object_address',
                render: function (data, type, row, meta) {
					var address_array = JSON.parse(data);
					return '<label>'+address_array[0]+'</label>';
                }
            }*/
			]
		});
		$('#location_filter').on('change', function() {
			$('#vehicle_table').DataTable().ajax.reload(null, true);
		});
	});
</script>
