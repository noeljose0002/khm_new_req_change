...
<style>
    ...
    color: #f3640d !important;
    background: rgba(234, 235, 243, 0.1) !important;
}
</style>
	</head>
	...
	<body class="app sidebar-mini">	
		...
		<div id="perviewmodal" class="modal fade">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content ">
					<div class="modal-header pd-x-20">
						<h6 class="modal-title">Transaction Permissions</h6>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body pd-20 perviewmodal_body">
					
					</div><!-- modal-body -->
					<div class="modal-footer">
						<?php if($add_per==1 || $edit_per==1){ ?>
							<button type="button" class="btn btn-primary" id="btn_update_per">Update</button>
						<?php } ?>
			...
			<div class="page-main">
				...
				<div class="app-content icon-content">
					<div class="section">
						...
						<div class="page-header">
							<div class="page-leftheader">
								<h4 class="page-title mb-0">Transactions of the Role <?php echo $role_det[0]['role_name']; ?></h4>
								<small class="text-muted mt-0 fs-14">Assign Transactions and Permissions to the <?php echo $role_det[0]['role_name']; ?></small>
							</div>
									...
									<div class="card-body">
										...
										<!--<div class="table-responsive product-datatable">
											<table id="example" class="table table-striped table-bordered " >
												<thead>
													<tr>
														<th class="w-15p">Transactions</th>
														<th class="wd-10p">Action</th>
														<th class="wd-10p">Permissions</th>
													</tr>
												</thead>
												<tbody>
													<?php foreach($all_trans as $key => $val) { 
														...
														<tr>
															...
															<?php if(!empty($trans_exist)){ 
																foreach($trans_exist as $key1 => $val1){
																	if($val['entity_trans_id'] == $val1['entity_trans_id']){
																		$checked = "checked";
																	}
																}
																?>
																<td><input type="checkbox" name="trans_check[]" id="trans_check<?php echo $val['entity_trans_id']; ?>" value="<?php echo $val['entity_trans_id']; ?>" <?php echo $checked; ?>></td>
															<?php } else { ?>
																<td><input type="checkbox" name="trans_check[]" id="trans_check<?php echo $val['entity_trans_id']; ?>" value="<?php echo $val['entity_trans_id']; ?>"></td>
															<?php } ?>
															<td>
																<a class="nav-link entity_trans_per_id" data-id-name="<?php echo $val['entity_trans_name']; ?>" data-id="<?php echo $val['entity_trans_id']; ?>" href=""><i class="fa fa-eye"></i></a>
															</td>
														</tr>
													<?php } ?>
												</tbody>
					...
					<ul class="toggle-menu">
						<?php foreach($parent_menu as $key1 => $val1){ ?>
							<li class="slide">
								<a class="side-menu__item"  data-toggle="slide" href=""><span class="side-menu__label"><?php echo $val1['entity_trans_name']; ?></span><i class="angle fa fa-angle-right"></i></a>
								<ul class="slide-menu">
									<?php foreach($sub_menu as $key2 => $val2){ 
										if($val1['entity_trans_id'] == $val2['prs_parent_id']){
										?>
											<li><a class="slide-item nav-link menu_select_id" data-id-name="<?php echo $val2['entity_trans_name']; ?>" data-id="<?php echo $val2['entity_trans_id']; ?>" href=""><span><?php echo $val2['entity_trans_name']; ?></span></a></li>
									<?php } } ?>
								</ul>
							</li>
						<?php } ?>
...
<script type="text/javascript">
    $(document).on('click', '.menu_select_id', function(e) {
        e.preventDefault();
        var trans_id = $(this).attr('data-id');
		var trans_name = $(this).attr('data-id-name');
		var role_id = "<?php echo $role_id; ?>";
		var role_name = "<?php echo $role_name; ?>";
		$('#sel_trans_id').val(trans_id);
	
        $.ajax({
            type: "POST",
            url: "<?=site_url('Dashboard/assign_permissions');?>",
            data: {
				role_id:role_id,
                trans_id:trans_id,
            ...
            success: function(response) {
                $('.perviewmodal_body').html(response);
...
<script>
$(document).on('click', '#btn_update_per', function (e) {
        e.preventDefault();
		var role_id = $('#p_role_id').val();
		var trans_id = $('#p_trans_id').val();

		var selectedPers = [];
        $('#entity-role-datatable input.entity-per-input:checked').each(function () {
            selectedPers.push($(this).val());
        });
				$.ajax({
					url: '<?=site_url('Dashboard/updatePermissions');?>',
					method: 'POST',
					data: {
						trans_id:trans_id,
                		role_id: role_id,
...
$(document).ready(function () {
    $('body').on('click', 'input[name="trans_check[]"]', function () {
        var trans_ids = $('input[name="trans_check[]"]:checked').map(function () {
        ...
        $.ajax({
            url: '<?=site_url('Dashboard/updateTransaction');?>',
            type: 'POST',
            data: {
				trans_ids: trans_ids,
				entity_id: entity_id,
