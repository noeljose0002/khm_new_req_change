...
foreach($response as $index => $item){
        ...
        <div class="container">
            ...
            <table style="width:auto; min-width:600px;border-collapse: collapse;border: 1px solid black;">
                    ...
                    <tbody>
                        ...
                        // Collect all lines for this itinerary
                        $all_lines = array();
                        foreach ($item['iti_data'] as $key => $val) {
                            // Skip the last date if it's the end date (no accommodation row for departure)
                            if (isset($val['tour_date']) && date("d-m-Y", strtotime($val['tour_date'])) == date("d-m-Y", strtotime($item['object_det'][0]['end_date']))) {
                                continue;
                            }
                ...
                (isset($room_category_name) ? $room_category_name : 
                 (!empty($item['tour_expansion_details'][$tour_details_id][0]['room_category_name']) ? $item['tour_expansion_details'][$tour_details_id][0]['room_category_name'] : 'N/A'));
                                ...
                                if (empty($grouped_rows)) {
                                    ...
                                    $grouped_rows[$default_key] = [
                                        ...
                                        'meal_plan' => $meal_name,
                                        'double_count' => isset($val['double_room']) ? $val['double_room'] : $item['object_det'][0]['no_of_double_room'],
                                        'single_count' => isset($val['single_room']) ? $val['single_room'] : $item['object_det'][0]['no_of_single_room']
                                    ];
                                }
    ...
    if(isset($item['tour_plan_det'][0]['vehicle_details'])) {
        $vehicle_details_json = $item['tour_plan_det'][0]['vehicle_details'];
        if (!empty($vehicle_details_json)) { // FIXED: Added empty check
            $vehicle_details = json_decode($vehicle_details_json, true); // FIXED: Use true for assoc array
            if(!empty($vehicle_details) && is_array($vehicle_details)){
                foreach ($vehicle_details as $keyv => $valv) {
                    if(isset($valv['vehicle_model'])) { // FIXED: Use [] for assoc
    ...
    ?>
        ...
        <p><b>Note:</b></p>
            ...
            if($item['object_det'][0]['no_of_extra_bed'] > 0){ // FIXED: Changed from no_of_single_room to no_of_extra_bed
                $noextras = ", ".$item['object_det'][0]['no_of_extra_bed']." Extra Bed"; 
            }
            else{
                $noextras = '';
            }     
