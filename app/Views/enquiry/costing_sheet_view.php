                        ...
                        <table style="width:100%;border-collapse: collapse;border: 1px solid black;">  
							...
							foreach ($iti_data as $key => $val) {
								if ($key < $itinerary_details_save_count) {
									...
									// PRIORITY 1: Check itinerary_expansion_details (saved data)
									...
									// Extract cost data (existing logic)
							...
							?>
									            <tr>
									                <?php if ($row_counter === 0) { ?>
									                    <td rowspan="<?php echo $total_rows; ?>" style="text-align:center;border:1px solid black;"><?php echo $k; ?></td>
									                ...
									                <td style="border:1px solid black;text-align:center;"><?php echo $d_meal_plan_name; ?></td>
...
<?php } ?>
											...
											?>
									            <tr>
									                ...
									                <td style="border:1px solid black;">
    <?php echo !empty($val['room_category_name']) ? $val['room_category_name'] : 'Own Arrangement'; ?>
</td>
    ...
    <td rowspan="<?php echo $double_row_count; ?>" style="border:1px solid black;text-align:center;"><?php echo $adults_for_double; ?></td>
<?php } ?>
									                <td style="border:1px solid black;text-align:center"><?php echo $val['double_room']; ?></td>
											...
											?>
									            <tr>
									                <?php if ($row_counter === 0 && $double_row_count == 0) { ?>
									                    <td rowspan="<?php echo $total_rows; ?>" style="border:1px solid black;text-align:center;"><?php echo $k; ?></td>
									                    <td rowspan="<?php echo $total_rows; ?>" style="text-align:center;border:1px solid black;"><?php echo date("d-m-Y", strtotime($val['tour_date'])); ?></td>
									                    <td rowspan="<?php echo $total_rows; ?>" style="border:1px solid black;"><?php echo $val['geog_name']; ?></td>
									                    <td rowspan="<?php echo $total_rows; ?>" style="border:1px solid black;"><?php echo $val['remarks']; ?></td>
									                    <td rowspan="<?php echo $total_rows; ?>" style="border:1px solid black;"><?php echo $val['object_name'] ? $val['object_name'] : "Own Arrangements"; ?></td>
									                ...
									                <td style="border:1px solid black;text-align:center;"><?php echo $s_meal_plan_name; ?></td>
...
<?php
						...
						foreach ($iti_data as $dkey => $day) {
							...
							// NEW: Collect ALL sightseeing names from ss_data_json (replaces old $ss_name_temp cost loop)
							...
							foreach ($vehicles as $v) {
								...
								if ($first) {
									echo '<td rowspan="' . $rowspan . '" style="border:1px solid black;text-align:center;">' . $dayNo . '</td>';
									...
									// UPDATED: Use $ss_name_display for Destination (shows all sightseeings)
									if (date("d-m-Y", strtotime($tourDate)) == date("d-m-Y", strtotime($object_det[0]['date_of_tour_completion']))) {
										...
										echo '<td rowspan="' . $rowspan . '" style="border:1px solid black;">' . $final_destination . '</td>';
									} else {
										$destination = !empty($ss_name_display) ? $ss_name_display : $day['geog_name'];
										echo '<td rowspan="' . $rowspan . '" style="border:1px solid black;">' . $destination . '</td>';
									}
									$dayNo++;
									$first = false;
								}

								echo '<td style="border:1px solid black;text-align:center;">' . $v->travel_distance . '</td>';
...
?>
    <tr>
        <td colspan="4" style="border:1px solid black;text-align:center;">Extra Kilometer (Rate : <?php echo ($extra_rate); ?>)</td>
        <td style="border:1px solid black;text-align:center;"><?php echo ($total_extra_km); ?></td>
...
<table style="width:60%;border-collapse: collapse;">  
    ...
    <?php } ?>
    <tr> 
        <td style="background-color:#D3D3D3 ;"><b>Total Net Rate</b></td>
        <td style="text-align:right;background-color:#D3D3D3;"><b><?php echo $tnr_hidden; ?></b></td>
		...
		<td  style="text-align:left;padding-left:10%;" >
			...
			<td style="text-align:right;background-color:#D3D3D3;"><?php echo ($iti_cost_datas[0]['tpc_with_tcs']); ?></td>
				</tr>
        <?php } ?>
...
<?php if ($iti_cost_datas[0]['cs_confirmed_id'] > 0) { ?>
	<table style="float:right;">
		<tr>
			<td style="padding:5px;">
				<button type="button" id="save_cs_btn" class="btn btn-success btn-sm" style="float:right;margin-right:20px;" disabled>Save</button>
			</td>
		</tr>
	</table>
<?php } else { ?>
