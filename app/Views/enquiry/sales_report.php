	...
	function generatereport() {
		...
		if ((fromDate == "") || (toDate == "")) {
			...
			$('#report_table').DataTable({
				...
				'ajax': {
					'url': '<?= site_url('Enquiry/get_sales_report_data'); ?>',
					'type': 'POST',
					'data': function(d) {
				...
				'initComplete': function() {
					...
					$("div.report-title").html(
						'<h5 style="margin-top: 6px;font-weight: bold;font-style: italic;color:#006600">Sales Report: ' + formattedFromDate + ' to ' + formattedToDate + '</h5>'
					);
				}
			});
