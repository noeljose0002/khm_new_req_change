																						...
																						<div class="card">
																							...
																							<div class="card-body">
																								<div class="ibox teams mb-30 bg-boxshadow">
																									...
																									<div class="row mt-2 single_row">
																										...
																										<div class="col-xl-2 col-sm-12 col-md-2">
																											...
																											<select id="sight_selector<?php echo $iti_id; ?>"
																												...
																												<?php
																												// FIXED: Use $ttval['ss'] (now per location from controller); fallback to dep_ss for last day
																												$ss_default_temp = ($startDate1->format('d-m-Y') == $object_det[0]['end_date']) ? $dep_ss : $ttval['ss'];
																												foreach ($ss_default_temp as $skey => $sval) {
																												?>
																													<option value="<?php echo $sval['sightseeing_id']; ?>"
																														...
																														data-distance="<?php echo $sval['sightseeing_distance']; ?>"> <!-- FIXED: Ensure field match -->
																									...
																									<!-- Dynamic Rows Container (unchanged, but now populates with defaults) -->
																									<div id="ss_dynamic_rows<?php echo $iti_id; ?>" class="ss-rows-container">
																										<!-- Dynamic rows will be added here by JS on load -->
																									</div>
																									<div class="row">
...
<script>
	$(document).ready(function() {
		...
		function getTotalPax() {
			return <?php echo $object_det[0]['no_of_adult'] + $object_det[0]['no_of_child_with_bed'] + $object_det[0]['no_of_child_without_bed'] ?>;
		}
		...
		$(document).on('click', '.add_sightseeing_btn', function() {
			...
			var totalPax = <?php echo $object_det[0]['no_of_adult'] + $object_det[0]['no_of_child_with_bed'] + $object_det[0]['no_of_child_without_bed']; ?>;
...
$('[id^="travel_distance"]').each(function() {
    ...
    if (itinerariesWithSavedSS[itiId]) {
        // savedSSDistance = what's actually stored on server for this itinerary/date
        var savedSSDistance = parseFloat(itinerariesWithSavedSS[itiId].ssDistance) || 0;

        // Derive base distance from the current vehicle value:
        // - If PHP already provided base (currentDist == base), baseFromCurrent ~= currentDist
        // - If PHP provided base+defaults (currentDist == base + defaults), this subtracts defaults to get base
        var baseFromCurrent = currentDist - savedSSDistance;

        // Safety: clamp to zero (avoid negatives if something mismatched)
        if (baseFromCurrent < 0) baseFromCurrent = 0;

        // Store pure base in the hidden copy field
        $('#c_travel_distance_copy' + vid).val(baseFromCurrent.toFixed(2));

        // Ensure visible distance is base + saved SS (this will equal currentDist if currentDist already included savedSSDistance)
        var totalDistance = baseFromCurrent + savedSSDistance;
        $distInput.val(totalDistance.toFixed(2));

        console.log('  Vehicle', vid, '- Derived Base:', baseFromCurrent, 'Saved SS:', savedSSDistance, 'Display Total:', totalDistance);
        ...
        if (hasDefaultSS) {
            // Extract base from defaults (PHP has given total = base + defaultSS)
		...
		setTimeout(function() {
			...
			if (typeof updateTotalAccommodationCost === 'function') {
				updateTotalAccommodationCost();
			}
		}, 2000);
	});
