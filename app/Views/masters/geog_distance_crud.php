    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Geography Distance Form</h4>
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Geography Distance CRUD</title>
                    <!-- Bootstrap 4.1.3 CSS -->
                ...
                <body>
                    <div class="container mt-4">
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <h4>Geography Distance Form</h4>
                            </div>
                            <div class="card-body">
                                <form id="distanceForm">
                                    <!-- Hidden field for update -->
                                    <input type="hidden" name="geog_dist_id" id="geog_dist_id">
                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="geog_from_id">From Location</label>
                                            <select class="form-control" id="geog_from_id" name="geog_from_id" required>
                                                <option value="">Select Geography</option>
                                                <?php if (!empty($geographies)): ?>
                                                    <?php foreach ($geographies as $geo): ?>
                                                        <option value="<?= $geo->geog_id ?>"><?= $geo->geog_name ?></option>
                                                    <?php endforeach; ?>
                                                <?php endif; ?>
                                            </select>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="geog_to_id">To Location</label>
                                            <select class="form-control" id="geog_to_id" name="geog_to_id" required>
                                                <option value="">Select Geography</option>
                                                <?php if (!empty($geographies)): ?>
                                                    <?php foreach ($geographies as $geo): ?>
                                                        <option value="<?= $geo->geog_id ?>"><?= $geo->geog_name ?></option>
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="geog_km_distance">Distance (KM)</label>
                                            <input type="number" step="0.01" min="0" max="384400" class="form-control" id="geog_km_distance" name="geog_km_distance" required>
                                        </div>
                                    </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>Geography Distances List</h4>
                            </div>
                            <div class="card-body">
                                <table id="distanceTable" class="display table table-bordered" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>ID</th>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#distanceTable').DataTable({
            ...
            serverSide: true,
            "ajax": {
                "url": "<?= site_url('Geogdistancecrudcontroller/getAll') ?>",
                "type": "GET",
                "dataSrc": ""
            },
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data) {
                        return $('#geog_from_id option[value="' + data + '"]').text();
                    }
                },
                {
                    "data": "geog_to_id",
                    "render": function(data) {
                        return $('#geog_to_id option[value="' + data + '"]').text();
                    }
                },
                {
                    "data": "geog_km_distance"
                },
                {
                    "data": null,
                    "orderable": false,
                    "render": function(data, type, row) {
                        return '<i class="fa fa-edit action-icon edit" data-id="' + row.geog_dist_id + '"></i>&nbsp;&nbsp;<i class="fa fa-trash action-icon delete" data-id="' + row.geog_dist_id + '"></i>';
                    }
                }
            ]
        });
        ...
        // Form submission for create/update record using AJAX.
        $('#distanceForm').on('submit', function(e) {
            e.preventDefault();

            var geog_from_id = $('#geog_from_id').val();
            var geog_to_id = $('#geog_to_id').val();
            var currentRecord = $('#geog_dist_id').val();

            // 1️⃣ Only run duplicate‐check on CREATE (no currentRecord)
            if (currentRecord === "") {
                var isDuplicate = false;
                table.rows().every(function() {
                    var data = this.data();

                    // Direct duplicate: A → B
                    if (data.geog_from_id == geog_from_id && data.geog_to_id == geog_to_id) {
                        isDuplicate = true;
                        return false; // break
                    }
                    // Reverse duplicate: B → A
                    if (data.geog_from_id == geog_to_id && data.geog_to_id == geog_from_id) {
                        isDuplicate = true;
                        return false; // break
                    }
                });
                if (isDuplicate) {
                    return Swal.fire({
                        ...
                        text: 'A record already exists in one of the directions.'
                    });
                }
            }

            // 2️⃣ Serialize + pick URL
            var formData = $(this).serialize();
            var ajaxUrl = currentRecord === "" ?
                "<?= site_url('Geogdistancecrudcontroller/create') ?>" :
                "<?= site_url('Geogdistancecrudcontroller/update') ?>";
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'duplicate') {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: currentRecord === "" ? 'Saved!' : 'Updated!',
                            text: 'The record has been successfully ' + (currentRecord === "" ? 'saved.' : 'updated.')
                        });
        ...
        $('#distanceTable').on('click', '.edit', function() {
            var id = $(this).data('id');
            $.ajax({
                type: "POST",
                url: "<?= site_url('Geogdistancecrudcontroller/getRecord') ?>",
                data: {
                    geog_dist_id: id
                },
                dataType: "json",
                success: function(data) {
                    $('#geog_dist_id').val(data.geog_dist_id);
                    $('#geog_from_id').val(data.geog_from_id);
                    $('#geog_to_id').val(data.geog_to_id);
                    $('#geog_km_distance').val(data.geog_km_distance);
                    $('#saveBtn').text('Update');
                },
                error: function() {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: 'Could not fetch the record details.'
                    });
                }
            });
        });

        // Delete record (soft delete) with SweetAlert confirmation.
        $('#distanceTable').on('click', '.delete', function() {
            var id = $(this).data('id');
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "<?= site_url('Geogdistancecrudcontroller/delete') ?>",
                        data: {
                            geog_dist_id: id
                        },
                        dataType: "json",
                        ...
                        error: function() {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error!',
                                text: 'Deletion failed due to an error.'
                            });
                        }
                    });
                }
