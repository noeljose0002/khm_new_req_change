                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">TARGETS</h4>
...
<head>
  <meta charset="UTF-8">
  <title>Target CRUD with AJAX - Enhanced UI</title>
  <!-- Bootstrap 4 CSS -->
  ...
  <style>
    ...
    .action-btn { border: none; background: none; padding: 0; }
    .action-btn i { font-size: 1.1rem; }
    .card {
      margin-top: 20px;
    }
    /* Card header styling: ash background with rectangular look */
    .card-header {
    ...
    /* DataTable header: green background, white text */
    #targetTable thead th {
    ...
    /* Row hover effect: light green background */
...
<div class="container mt-5">
  <!-- <h2 class="mb-4">Target CRUD with AJAX</h2> -->
  ...
  <div id="addSection" class="card">
    ...
    <div class="card-body">
      <form id="addForm">
        <div id="targetEntries">
          <div class="target-entry border p-3 mb-3">
            <div class="form-group">
              <label for="target_name_1">Target Name</label>
              <input type="text" class="form-control target_name" id="target_name_1" name="target_name[]" required>
            </div>
            <div class="form-group">
              <label for="from_date_1">From Date</label>
              <input type="date" class="form-control" id="from_date_1" name="from_date[]" required>
            </div>
            <div class="form-group">
              <label for="to_date_1">To Date</label>
              <input type="date" class="form-control" id="to_date_1" name="to_date[]" required>
            </div>
            <div class="form-group">
              <label for="amount_1">Amount</label>
              <input type="number" step="0.01" max="9999999999999999" class="form-control" id="amount_1" name="amount[]" required>
            </div>
            <div class="form-check mb-2">
              <input type="checkbox" class="form-check-input" id="target_status_1" name="target_status[]" value="1" checked>
              <label class="form-check-label" for="target_status_1">Active</label>
            </div>
            <div>
              <!-- <button type="button" class="btn btn-success "><i class="fas fa-eraser"></i></button>
              <button type="button" class="btn btn-success"><i class="fas fa-trash-alt"></i></button> -->
            </div>
          </div>
        </div>
        <div class="mb-3">
          <!-- <button type="button" class="btn btn-success" id="addMore"></i> Add</button> -->
          <button type="submit" class="btn btn-success" id="saveAllBtn"></i> Save </button>
          <button type="button" class="btn btn-success" id="clearAll"></i> Clear </button>
    ...
    <div class="card-body">
      <form id="editForm">
        ...
        <div class="form-check mb-3">
          <input type="checkbox" class="form-check-input" id="edit_target_status" name="target_status" value="1">
          <label class="form-check-label" for="edit_target_status">Active</label>
        </div>
        <div>
          <button type="submit" class="btn btn-success" id="updateBtn"> Update</button>
          <button type="button" class="btn btn-success" id="cancelEdit">Cancel </button>
        </div>
      </form>
    </div>
  </div>
...
<!-- jQuery, Bootstrap 4 JS, DataTables JS, SweetAlert2 JS -->
...
$(document).ready(function(){
  var targetTable; // DataTable instance

  // Custom validator for dynamic add form "To Date"
  $.validator.addMethod("dateOrder", function(value, element) {
    var fromDate = $(element).closest('.target-entry').find("input[name='from_date[]']").val();
    if(fromDate && value) {
      return Date.parse(value) >= Date.parse(fromDate);
    }
    return true;
  }, "To Date must be later than or equal to From Date");

  // Custom validator for edit form "To Date"
  $.validator.addMethod("dateOrderEdit", function(value, element) {
  ...
  // Custom validator to check for unique target name entries.
  $.validator.addMethod("uniqueTargetName", function(value, element) {
    var count = 0;
    $("input[name='target_name[]']").each(function(){
      if($.trim($(this).val()).toLowerCase() === $.trim(value).toLowerCase()){
        count++;
      }
    });
    return count < 2;
  ...
  // Trigger re-validation on keyup/change for target name fields.
  $(document).on('keyup change', "input[name='target_name[]']", function(){
    $(this).valid();
  });

  // Function to load target records into table.
  function loadTargets(){
    $.ajax({
      url: '<?= site_url("TargetAjaxController/listTargets") ?>',
      type: 'GET',
      dataType: 'json',
      success: function(data){
        ...
        $.each(data, function(i, item){
          html += '<tr>';
          html += '<td></td>';
          html += '<td>' + item.target_name + '</td>';
          html += '<td>' + item.target_from_date + '</td>';
          html += '<td>' + item.target_to_date + '</td>';
          html += '<td>' + item.amount + '</td>';
          html += '<td>' + (item.target_status == 1 ? 'Active' : 'Inactive') + '</td>';
          html += '<td>';
          html += '<i class="fas fa-edit action-icon editAssign" data-id="' + item.target_id + '" title="Edit" style="margin-right: 10px; cursor: pointer; border: none; outline: none;"></i>';
          html += '<i class="fas fa-trash action-icon deleteAssign" data-id="' + item.target_id + '" title="Delete" style="cursor: pointer; border: none; outline: none;"></i>';
          html += '</td>';
          html += '</tr>';
        });
        ...
        if ( $.fn.DataTable.isDataTable('#targetTable') ) {
          ...
          targetTable.rows.add($(html)).draw();
        } else {
      ...
      error: function(xhr){
        console.error("Error loading targets:", xhr.responseText);
      }
    });
  }
  ...
  // Add more target entry.
  $('#addMore').click(function(){
    var entry = `<div class="target-entry border p-3 mb-3">
      <div class="form-group">
        <label for="target_name_${counter}">Target Name</label>
        <input type="text" class="form-control target_name" id="target_name_${counter}" name="target_name[]" required>
      </div>
      <div class="form-group">
        <label for="from_date_${counter}">From Date</label>
        <input type="date" class="form-control" id="from_date_${counter}" name="from_date[]" required>
      </div>
      <div class="form-group">
        <label for="to_date_${counter}">To Date</label>
        <input type="date" class="form-control" id="to_date_${counter}" name="to_date[]" required>
      </div>
      <div class="form-group">
        <label for="amount_${counter}">Amount</label>
        <input type="number" step="0.01" class="form-control" id="amount_${counter}" name="amount[]" required>
      </div>
      <div class="form-check mb-2">
        <input type="checkbox" class="form-check-input" id="target_status_${counter}" name="target_status[]" value="1" checked>
        <label class="form-check-label" for="target_status_${counter}">Active</label>
      </div>
      <div>
        // <button type="button" class="btn btn-success btn-sm clearEntry"><i class="fas fa-eraser"></i></button>
        // <button type="button" class="btn btn-success btn-sm removeEntry"><i class="fas fa-trash-alt"></i></button>
      </div>
    </div>`;
  ...
  $(document).on('click', '.clearEntry', function(){
    $(this).closest('.target-entry').find('input').each(function(){
  ...
  $.validator.addMethod("validName", function(value, element) {
    return this.optional(element) ||
           (/^[a-zA-Z0-9 ]+$/.test(value) && /[a-zA-Z]/.test(value));
  }, "Please enter a valid name that contains at least one alphabet.");
  
  // Validate and submit Add Form (Save All) with duplicate check.
  $('#addForm').validate({
    errorClass: 'error',
    rules: {
      'target_name[]': {
    ...
    messages: {
      'target_name[]': {
        ...
        minlength: "Target name must be at least 3 characters.",
        maxlength: "Target name cannot exceed 100 characters."
      },
      'from_date[]': {
      ...
      'amount[]': {
        ...
        max: "Amount exceeds the maximum allowed value."
      }
    },
    submitHandler: function(form) {
      // Additional duplicate check before processing the submission.
      var names = [];
      var duplicateFound = false;
      $('.target-entry').each(function(){
        var name = $.trim($(this).find("input[name='target_name[]']").val()).toLowerCase();
        if(names.indexOf(name) !== -1){
          duplicateFound = true;
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Duplicate target name found: ' + name
          });
          return false;
        }
        names.push(name);
      ...
      $('.target-entry').each(function(){
        var $entry = $(this);
        var targetName = $.trim($entry.find("input[name='target_name[]']").val());
        var fromDate = $entry.find("input[name='from_date[]']").val();
        var toDate = $entry.find("input[name='to_date[]']").val();
        var amount = $entry.find("input[name='amount[]']").val();
        var status = $entry.find("input[name='target_status[]']").is(':checked') ? 1 : 0;
        if(targetName && fromDate && toDate && amount) {
          entries.push({
      ...
      if(entries.length === 0) {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'No valid entries to save.'
        });
        return false;
      }
      
      $.ajax({
        url: '<?= site_url("TargetAjaxController/store") ?>',
        type: 'POST',
        ...
        success: function(response){
          if(response.status){
            ...
            // Reset the form to a single entry.
            $('#targetEntries').html('');
            counter = 1;
            var initialEntry = `<div class="target-entry border p-3 mb-3">
              <div class="form-group">
                <label for="target_name_1">Target Name</label>
                <input type="text" class="form-control target_name" id="target_name_1" name="target_name[]" required>
              </div>
              ...
              <div class="form-group">
                <label for="amount_1">Amount</label>
                <input type="number" step="0.01" class="form-control" id="amount_1" name="amount[]" required>
              </div>
              <div class="form-check mb-2">
                <input type="checkbox" class="form-check-input" id="target_status_1" name="target_status[]" value="1" checked>
                <label class="form-check-label" for="target_status_1">Active</label>
            ...
            $('#targetEntries').append(initialEntry);
            counter = 2;
          } else {
            Swal.fire({
              icon: 'error',
        ...
        error: function(xhr){
          console.error("Error in save:", xhr.responseText);
          Swal.fire({
            icon: 'error',
            title: 'Error',
  ...
  // Validate and submit Edit Form (Update)
  $('#editForm').validate({
    errorClass: 'error',
    rules: {
      target_name: {
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        url: '<?= site_url("TargetAjaxController/update") ?>',
        type: 'POST',
        data: formData,
        dataType: 'json',
        success: function(response){
        ...
        error: function(xhr){
          console.error("Error in update:", xhr.responseText);
          Swal.fire({
            icon: 'error',
            title: 'Error',
  ...
  $(document).on('click', '.editAssign', function(){
    var id = $(this).data('id');
    $.ajax({
      url: '<?= site_url("TargetAjaxController/edit") ?>/' + id,
      type: 'GET',
      dataType: 'json',
      success: function(response){
        if(response.status){
          ...
          $('#edit_target_name').val(data.target_name);
          $('#edit_from_date').val(data.target_from_date);
          $('#edit_to_date').val(data.target_to_date);
          $('#edit_amount').val(data.amount);
          $('#edit_target_status').prop('checked', data.target_status == 1);
          $('#addSection').hide();
          $('#editSection').show();
          $('html, body').animate({
                scrollTop: $('#editForm').offset().top - 200
              }, 'fast');
        } else {
          Swal.fire({
            icon: 'error',
      ...
      error: function(xhr){
        console.error("Error in edit:", xhr.responseText);
      }
    });
  });
  ...
  $(document).on('click', '.deleteAssign', function(){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: '<?= site_url("TargetAjaxController/delete") ?>/' + id,
          type: 'POST',
          dataType: 'json',
          success: function(response){
            if(response.status){
          ...
          error: function(xhr){
            console.error("Error in delete:", xhr.responseText);
            Swal.fire(
              'Error!',
