                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">TARGETS ASSIGNMENT</h4>
...
<head>
  ...
  <style>
    ...
    /* table.dataTable tbody tr:hover { background-color: #d8f3dc !important; } */
    /* Action icons: display green color */
    .fa-edit, .fa-trash {
      cursor: pointer;
      margin-right: 10px;
      font-size: 18px;
    ...
    .fa-edit:hover, .fa-trash:hover { color: #28a745; }
  </style>
</head>
<body>
  <div class="container mt-4">
    ...
    <div class="card">
      <div class="card-header">
        <h4>Assign Target to Employee</h4>
      </div>
      <div class="card-body">
        <form id="assignForm">
          <!-- Hidden field for assignment_id (used when editing) -->
          ...
          <div class="form-row">
                ...
                <?php if(!empty($targets)): ?>
                  <?php foreach($targets as $t): ?>
                    <option value="<?= $t['target_id'] ?>" 
                            data-from="<?= $t['target_from_date'] ?>" 
                            data-to="<?= $t['target_to_date'] ?>" 
            ...
            <div class="form-group col-md-4">
              ...
              <input type="number" class="form-control" name="amount" id="amount" required min="0" step="any" max="99999999">
            </div>
          </div>
          <div class="form-row">
            <!-- From Date (read-only) -->
            <div class="form-group col-md-6">
              ...
              <input type="date" class="form-control" name="from_date" id="from_date" required readonly>
            </div>
            <!-- To Date (read-only) -->
            <div class="form-group col-md-6">
              <label for="to_date">To Date</label>
              <input type="date" class="form-control" name="to_date" id="to_date" required readonly>
            </div>
          </div>
          <button type="submit" class="btn btn-success" id="saveBtn">Save</button>
          <!-- Cancel button, initially hidden; shown in update mode -->
          <button type="button" class="btn btn-success" id="cancelBtn" style="display:none;">Cancel</button>
        </form>
      </div>
    </div>
...
<script>
  ...
  <!-- jQuery 3.2.1, Bootstrap 4.1.3 JS, DataTables, SweetAlert2 -->
  ...
  <script>
    $(document).ready(function(){
      // Initialize DataTable for assignments with S.No column orderable.
      let table = $('#assignmentTable').DataTable({
        ajax: {
          url: '<?= site_url("AssignmentController/fetchAssignments") ?>',
          type: 'POST',
          dataSrc: 'data'
        },
        columns: [
          {
            // S.No column: compute the serial number for display and sorting.
            data: null,
            orderable: true,
            render: function(data, type, row, meta) {
              // For display and sort, calculate based on current page info.
              return meta.row + meta.settings._iDisplayStart + 1;
            }
          },
          { data: 'employee_name' },
          { data: 'target_name' },
          { data: 'target_from_date' },
          { data: 'target_to_date' },
          { data: 'amount' },
          { 
            data: null, 
            ...
            render: function(data, type, row){
              return '<i class="fas fa-edit editBtn" data-id="'+row.target_assign_id+'" title="Edit"></i>' +
                     '<i class="fas fa-trash deleteBtn" data-id="'+row.target_assign_id+'" title="Delete"></i>';
            }
          }
        ],
        order: [[0, 'asc']]
      ...
      // Auto-fill target details when the target dropdown changes.
      $('#target_id').change(function(){
        let option = $(this).find(':selected');
        let fromDate = option.data('from');
        let toDate   = option.data('to');
        let amount   = parseFloat(option.data('amount'));
        if(fromDate) {
      ...
      // Form submission without checking a fixed target limit.
      $('#assignForm').on('submit', function(e){
        e.preventDefault();
        if(!$('#employee_id').val() || !$('#target_id').val()){
          Swal.fire('Error', 'Please select both employee and target', 'error');
          return;
        }
        let amountVal = $('#amount').val();
        if(amountVal === '' || isNaN(parseFloat(amountVal)) || parseFloat(amountVal) < 0){
          Swal.fire('Error', 'Please enter a valid, non-negative target amount', 'error');
          return;
        }
        $.ajax({
          url: '<?= site_url("AssignmentController/saveAssignment") ?>',
          type: 'POST',
          data: $(this).serialize(),
          dataType: 'json',
          ...
          error: function(){
            Swal.fire('Error', 'Server error: Could not save assignment.', 'error');
          }
        });
      });

      // Edit assignment: populate form for editing.
      $('#assignmentTable').on('click', '.editBtn', function(){
        let assignmentId = $(this).data('id');
        $.ajax({
          url: '<?= site_url("AssignmentController/editAssignment") ?>',
          type: 'POST',
          data: { assignment_id: assignmentId },
          dataType: 'json',
          success: function(data){
            if(data) {
              $('#assignment_id').val(data.target_assign_id);
              $('#employee_id').val(data.entity_id);
              $('#target_id').val(data.target_id).change();
              $('#from_date').val(data.target_from_date);
              $('#to_date').val(data.target_to_date);
              $('#amount').val(parseFloat(data.amount).toFixed(2));
              $('#saveBtn').text('Update');
              $('#cancelBtn').show();
              $('html, body').animate({ scrollTop: $('#assignForm').offset().top - 200 }, 'fast');
            }
          },
          error: function(){
            Swal.fire('Error', 'Could not fetch assignment details.', 'error');
          }
        });
      });

      // Cancel button: reset form to default create mode.
      ...
      // Delete assignment.
      $('#assignmentTable').on('click', '.deleteBtn', function(){
        let assignmentId = $(this).data('id');
        if(!assignmentId){
          Swal.fire('Error', 'Assignment ID is required.', 'error');
          return;
        }
        Swal.fire({
          title: 'Are you sure?',
          ...
          if(result.isConfirmed){
            $.ajax({
              url: '<?= site_url("AssignmentController/deleteAssignment") ?>',
              type: 'POST',
              data: { assignment_id: assignmentId },
              dataType: 'json',
              success: function(res){
                if(res.status === 'success'){
                  ...
                  Swal.fire('Error', res.message || 'Could not delete assignment', 'error');
                }
              },
              error: function(){
                Swal.fire('Error', 'Server error: Could not delete assignment.', 'error');
              }
            });
          }
        });
