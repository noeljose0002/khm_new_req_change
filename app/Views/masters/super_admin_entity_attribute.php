				...
				<div class="section">
					...
					<div class="page-header">
						<div class="page-leftheader">
							<h4 class="page-title mb-0">Add New Entity Attribute</h4>
				...
				<body>
					<div class="container mt-5">
								...
								<form id="addForm">
									<div class="form-row">
										...
										<div class="form-group col-md-4">
											<label for="entity_attribute_name">Entity Attribute Name</label>
											<input
												type="text"
												class="form-control"
												id="entity_attribute_name"
										...
										<div class="form-group col-md-4">
											<label for="entity_attribute_data_type">Entity Attribute Data Type</label>
											<select
												class="form-control"
												id="entity_attribute_data_type"
												name="entity_attribute_data_type">
						...
						<div id="editSection" class="card" style="display:none;">
							...
							<div class="card-body">
								<form id="editForm">
									...
									<input type="hidden" id="edit_entity_attribute_id" name="entity_attribute_id">

									<div class="form-row">

										<div class="form-group col-md-4">
											<label for="edit_entity_attribute_name">Entity Attribute Name</label>
											<input
												type="text"
												class="form-control"
												id="edit_entity_attribute_name"
												name="entity_attribute_name"
												maxlength="50"
												required>
										</div>
										...
										<div class="form-group col-md-4">
											<label for="edit_entity_attribute_data_type">Entity Attribute Data Type</label>
											<select
												class="form-control"
												id="edit_entity_attribute_data_type"
												name="entity_attribute_data_type">
												<option value="">-- Select Data Type --</option>
											</select>
										</div>
						...
						<div class="card mt-4">
							...
							<div class="card-body">
								<table id="entityAttributeTable" class="table table-bordered">
									<thead></thead>
									<tbody></tbody>
								</table>
							</div>
...
<script>
	$(document).ready(function() {
		...
		$.getJSON('<?= site_url("SuperAdminEntityAttribute/entClass") ?>', function(res) {
			allClasses = res.data;
			...
			$('#entity_attribute_data_type').html(buildOptions(allDataTypes, '-- Select Data Type --'));
			// inject data‑types into EDIT form
			$('#edit_entity_attribute_data_type').html(buildOptions(allDataTypes, '-- Select Data Type --'));
		});

		// 3) Initialize DataTable
		const table = $('#entityAttributeTable').DataTable({
			ajax: {
				url: '<?= site_url("SuperAdminEntityAttribute/list") ?>',
				dataSrc: 'data'
			},
			columns: [{
				...
				{
					data: 'entity_attribute_data_type',
					title: 'Data Type'
				},
				{
					title: 'Action',
		...
		$('#addForm').submit(function(e) {
			e.preventDefault();
			$.post('<?= site_url("SuperAdminEntityAttribute/create") ?>', $(this).serialize())
				.done(() => {
					$('#addForm')[0].reset();
					table.ajax.reload();
		...
		// 6) SHOW EDIT
		$('#entityAttributeTable').on('click', '.editBtn', function() {
			const id = $(this).data('id');
			$.getJSON(`<?= site_url("SuperAdminEntityAttribute/edit") ?>/${id}`, function(data) {
				// fill hidden ID
				$('#edit_entity_attribute_id').val(data.entity_attribute_id);
				// fill other fields
				$('#edit_entity_attribute_name').val(data.entity_attribute_name);
				...
				$('#edit_entity_attribute_data_type')
					.html(buildOptions(allDataTypes, '-- Select Data Type --', data.entity_attribute_data_type));
		...
		$('#editForm').submit(function(e) {
			e.preventDefault();
			const id = $('#edit_entity_attribute_id').val();
			$.ajax({
					url: `<?= site_url("SuperAdminEntityAttribute/update") ?>/${id}`,
					type: 'POST',
					data: $(this).serialize()
				})
				.done(() => {
		...
		// 9) DELETE
		$('#entityAttributeTable').on('click', '.deleteBtn', function() {
			const id = $(this).data('id');
			Swal.fire({
				title: 'Delete?',
				...
				if (res.isConfirmed) {
					$.ajax({
							url: `<?= site_url("SuperAdminEntityAttribute/delete") ?>/${id}`,
							type: 'DELETE'
						})
						.done(() => {
							table.ajax.reload();
