    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Add New Table Of Table Field</h4>
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Table Of Table Field </title>
                    <!-- Bootstrap 4 CSS -->
                ...
                <body>
                    <div class="container mt-5">
                            ...
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-5">
                                            <label for="table_field_name"> Table Field Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="table_field_name"
                                        ...
                                        <div class="form-group col-md-5">
                                            <label for="table_field_type"> Table Field Type</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="table_field_type"
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            ...
                            <div class="card-body">
                                <form id="editForm">
                                    ...
                                    <input type="hidden" id="edit_table_field_id" name="table_field_id ">

                                    <div class="form-row">

                                     <div class="form-group col-md-5">
                                            <label for="edit_table_id">Table Name</label>
                                            <select class="form-control" name="table_id" id="edit_table_id" data-live-search="true">
                                                <option value="">-- Select Class --</option>
                                            </select>
                                        </div>

                                        <div class="form-group col-md-5">
                                            <label for="edit_table_field_name">Table Field Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="edit_table_field_name"
                                                name="table_field_name"
                                                maxlength="50"
                                                required>
                                        </div>
                                          ...
                                          <div class="form-group col-md-5">
                                            <label for="edit_table_field_type"> Table Field Type</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="edit_table_field_type"
                                                name="table_field_type"
                                                maxlength="50"
                                                required>
                                        </div>
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <table id="tableOfTableField" class="table table-bordered">
                                    <thead></thead>
                                    <tbody></tbody>
                                </table>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $.getJSON('<?= site_url("SuperAdminTableOfTableFields/list2") ?>',function(res){
            allTables = res.data;

            const classopts = (sel) => {
                let h =  `<option value="">-- Select Class --</option>`;
                allTables.forEach(o=>{
                    const s = (''+o.table_id === ''+sel) ? ' selected' : '';
                    h += `<option value="${o.table_id}"${s}>${o.table_name}</option>`;
                });
                return h;


            };
            $('#table_id').html(classopts(null));
			// inject into EDIT form
			$('#edit_table_id').html(classopts(null));
        ...
        const table = $('#tableOfTableField').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminTableOfTableFields/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'table_field_id',
                ...
                },
                {
                    ...
                    title:'Table Name',
                    // render:cid =>{
                    //     const cls = allTables.find(c=>'+c.table_id'=== '' + cid)
                    //     return cls ? cls.table_id : '';
                    // }
                },
                {
                 data: 'table_field_name',
                 title: 'Field Name'
             },
                {
                    data: 'table_field_type',
                    title: 'type'
                },
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post('<?= site_url("SuperAdminTableOfTableFields/create") ?>', $(this).serialize())
                .done(() => {
                    $('#addForm')[0].reset();
                    table.ajax.reload();
        ...
        // 6) SHOW EDIT
        $('#tableOfTableField').on('click', '.editBtn', function() {
            const id = $(this).data('id');
            $.getJSON(`<?= site_url("SuperAdminTableOfTableFields/edit") ?>/${id}`, function(data) {
                // fill hidden ID
                $('#edit_table_field_id ').val(data.table_field_id);
               
                $('#edit_table_field_type ').val(data.table_field_type);
                // fill other fields
                $('#edit_table_field_name').val(data.table_field_name);
                $('#edit_enterprise_id').val(data.enterprise_id);

                let ch = `<option value="">-- Select Class --</option>`;
				allTables.forEach(o => {
					const s = ('' + o.table_id === '' + data.table_id) ? ' selected' : '';
					ch += `<option value="${o.table_id}"${s}>${o.table_name}</option>`;
				});

                $('#edit_table_id ').html(ch);
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            const id = $('#edit_table_field_id').val();
            $.ajax({
                    url: `<?= site_url("SuperAdminTableOfTableFields/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .done(() => {
        ...
        // 9) DELETE
        $('#tableOfTableField').on('click', '.deleteBtn', function() {
            const id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminTableOfTableFields/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(response => {
