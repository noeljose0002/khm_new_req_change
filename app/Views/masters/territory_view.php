	...
	<div class="page">
		<div class="page-main">
			...
			<div class="app-content icon-content">
				<div class="section">
					...
					<div class="page-header">
						<div class="page-leftheader">
							<h4 class="page-title mb-0">TERRITORY </h4>
				...
				<head>
					<meta charset="UTF-8">
					<title>Territory & Employee Assignment</title>
					<!-- Bootstrap 4.1.3 CSS -->
					<!-- <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css"> -->
					<!-- DataTables Bootstrap 4 CSS -->
					<!-- <link rel="stylesheet" href="https://cdn.datatables.net/1.13.1/css/dataTables.bootstrap4.min.css"> -->
					<link rel="stylesheet" href="https://cdn.datatables.net/1.10.25/css/dataTables.bootstrap4.min.css">
					<!-- SweetAlert2 CSS (Bootstrap 4 theme) -->
					...
					<style>
						...
						/* DataTable background color and header styling */
						...
						/* Row overlay (hover) is light green */
						...
						/* Ensure clear button appears inside the container */
						.select2-container {
							position: relative;
						}

						/* Card header style change to ash rectangular style */
						.card-header {
						...
						/* Icon styling for action columns; action buttons are now green */
						.action-icon {
							cursor: pointer;
							margin-right: 10px;
							font-size: 18px;
						...
						/* Hover effect on icons can optionally be adjusted if required */
						.action-icon:hover {
							opacity: 0.8;
						}
					</style>
				...
				<body class="bg-light">
					<div class="container">
						...
						<div class="card shadow-sm mb-4">
							...
							<div class="card-body">
								<!-- Territory Form -->
								<form id="territoryForm" class="mb-3">
									<input type="hidden" name="territory_id" id="territory_id">
									<div class="form-group">
										<label for="territory_name" class="font-weight-bold">Territory Name:</label>
										<input type="text" class="form-control" name="territory_name" id="territory_name" placeholder="Enter territory name" required autocomplete="off" spellcheck="false">
									</div>
									<div class="form-group">
										<label for="locations" class="font-weight-bold">Select Locations:</label>
										<select name="locations[]" id="locations" multiple="multiple" class="form-control" style="width:100%;">
											<?php if (!empty($locations)): ?>
												<?php foreach ($locations as $loc): ?>
													<option value="<?= $loc['geog_id'] ?>"><?= $loc['geog_name'] ?></option>
												<?php endforeach; ?>
											<?php else: ?>
									...
									<button type="submit" id="territorySubmitBtn" class="btn btn-success">Save</button>
									<!-- <button type="button" id="territoryClearBtn" class="btn btn-warning">Clear</button> -->
									<button type="button" id="territoryCancelBtn" class="btn btn-success" style="display: none;">Cancel</button>
								</form>
								<hr>
								<!-- Territory DataTable -->
								<div class="table-responsive">
									<table id="territoryTable" class="table table-striped table-bordered" style="width:100%;">
						...
						<!-- Employee Territory Assignment Section -->
						<div class="card shadow-sm">
							<div class="card-header">
								<h4>Employee Territory Assignment</h4>
							</div>
							<div class="card-body">
								<!-- Employee Assignment Form -->
								<form id="employeeAssignForm" class="mb-3">
									<input type="hidden" name="emp_teritory_assign_id" id="emp_teritory_assign_id" value="">
									<div class="form-group">
										<label for="assign_territory" class="font-weight-bold">Select Territory:</label>
										<select name="territory_id" id="assign_territory" class="form-control">
											<option value="">-- Select Territory --</option>
											<!-- Options loaded dynamically -->
										</select>
									</div>
									<div class="form-group">
										<label for="entity_ids" class="font-weight-bold">Select Employee(s):</label>
										<select name="entity_ids[]" id="entity_ids" multiple="multiple" class="form-control" style="width:100%;">
											<?php if (!empty($employees)): ?>
												<?php foreach ($employees as $emp): ?>
													<option value="<?= $emp['entity_id'] ?>"><?= $emp['entity_name'] ?></option>
												<?php endforeach; ?>
											<?php else: ?>
									...
									<button type="button" id="assignCancelBtn" class="btn btn-success" style="display: none;">Cancel </button>
									<!-- <button type="button" id="assignClearBtn" class="btn btn-success">Clear</button> -->
								</form>
								<hr>
								<!-- Employee Assignment DataTable -->
								<div class="table-responsive">
									<table id="employeeAssignTable" class="table table-striped table-bordered" style="width:100%;">
										<thead class="thead-dark">
											<tr>
												<th>S.No</th>
...
<script>
	// Function to add a clear (close) button to a Select2 field
	function addClearButton($select) {
		var $container = $select.next('.select2-container');
		// Remove any existing button to avoid duplicates
		$container.find('.clear-select2').remove();
		var $clearBtn = $('<button type="button" class="clear-select2">&times;</button>');
	...
	$(document).ready(function() {
		...
		// Custom matcher that uses default matching but always returns disabled options
		function hideSelectedMatcher(params, data) {
			...
			var defaultMatch = data.text.toLowerCase().indexOf(params.term.toLowerCase()) > -1;
			return defaultMatch ? data : null;
		}

		// Initialize Select2 for locations (Territory form)
		$('#locations').select2({
		...
		addClearButton($('#locations'));

		// Initialize Select2 for employee selection (Employee Assignment form)
		$('#entity_ids').select2({
		...
		addClearButton($('#entity_ids'));

		// Keep a copy of original #locations <option> tags for resetting
		var originalLocations = $('#locations').html();
		var locationMapping = {};
		$('#locations option').each(function() {
			var id = $(this).val();
			var name = $(this).text();
			if (id) {
				locationMapping[id] = name;
			}
		});

		// Initialize the Territory DataTable with icon actions
		var territoryTable = $('#territoryTable').DataTable({
			processing: true,
			ajax: {
				url: "<?= site_url('TerritoryController/getTerritoryList') ?>",
				type: "POST",
				dataSrc: 'data'
			},
			columns: [{
				...
				},
				{
					...
					render: function(data) {
						...
						if (Array.isArray(data)) {
							...
							try {
								locArr = JSON.parse(data);
							} catch (e) {
								locArr = data.split(',');
							}
						}
						var nameArr = Array.isArray(locArr) ? locArr.map(function(id) {
							return locationMapping[id] || id;
						}) : locArr;
						return Array.isArray(nameArr) ? nameArr.join(', ') : nameArr;
					}
				},
				{
					data: null,
					...
					render: function(data, type, row) {
						return '<i class="fas fa-edit action-icon editTerritory" data-id="' + row.territory_id + '" title="Edit"></i>' +
							'<i class="fas fa-trash action-icon deleteTerritory" data-id="' + row.territory_id + '" title="Delete"></i>';
					}
				}
			],
		...
		// Initialize the Employee Assignment DataTable with icon actions
		var employeeTable = $('#employeeAssignTable').DataTable({
			processing: true,
			ajax: {
				url: "<?= site_url('TerritoryController/getEmployeeAssignments') ?>",
				type: "POST",
				dataSrc: 'data'
			},
			columns: [{
				...
				},
				{
					...
					title: 'Actions',
					render: function(data, type, row) {
						return '<i class="fas fa-edit action-icon editAssign" data-id="' + row.emp_teritory_assign_id + '" data-entity=\'' + row.entity_id + '\' data-territory="' + row.territory_id + '" title="Edit"></i>' +
							'<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.emp_teritory_assign_id + '" title="Delete"></i>';
					}
				}
			]
		});

		// Rebuild #locations dropdown to exclude already used ones (unless editing)
		// function updateLocationsDropdown(selectedValues) {
		// 	var usedLocations = [];
		// 	territoryTable.rows().data().each(function(row) {
		// 		var locArr;
		// 		if (Array.isArray(row.locations)) {
		// 			locArr = row.locations;
		// 		} else {
		// 			try {
		// 				locArr = JSON.parse(row.locations);
		// 			} catch (e) {
		// 				locArr = row.locations ? row.locations.split(',') : [];
		// 			}
		// 		}
		// 		locArr.forEach(function(loc) {
		// 			if (loc) {
		// 				usedLocations.push(loc.toString().trim());
		// 			}
		// 		});
		// 	});
		// 	usedLocations = [...new Set(usedLocations)];
		// 	var editing = $('#territory_id').val() ? true : false;
		// 	var currentSelected = editing && selectedValues ? selectedValues : [];
		// 	var newOptionsHtml = '';
		// 	for (var key in locationMapping) {
		// 		if (locationMapping.hasOwnProperty(key)) {
		// 			if (usedLocations.indexOf(key) === -1 || currentSelected.indexOf(key) !== -1) {
		// 				newOptionsHtml += '<option value="' + key + '">' + locationMapping[key] + '</option>';
		// 			}
		// 		}
		// 	}
		// 	$('#locations').empty().append(newOptionsHtml);
		// 	$('#locations').val(currentSelected).trigger('change');
		// 	addClearButton($('#locations'));
		// }
		// ALTERNATIVE CODE FOR  LOCATION  SELECTION DUPLICATES
		function updateLocationsDropdown(selectedValues) {
			var usedLocations = [];
			territoryTable.rows().data().each(function(row) {
				var locArr;
			...
			// NOTE: Duplicate values are intentionally preserved:
			// usedLocations = [...new Set(usedLocations)];  <-- This line has been removed.
			...
			for (var key in locationMapping) {
				if (locationMapping.hasOwnProperty(key)) {
					// Even if duplicates are present in usedLocations, this condition is checked:
					if (usedLocations.indexOf(key) === -1 || currentSelected.indexOf(key) !== -1) {
						newOptionsHtml += '<option value="' + key + '">' + locationMapping[key] + '</option>';
					}
				}
		...
		// Disable/Enable forms so only one is active at a time.
		function disableEmployeeForm(disable) {
			$('#employeeAssignForm :input').prop('disabled', disable);
		}

		function disableTerritoryForm(disable) {
			$('#territoryForm :input').prop('disabled', disable);
		}

		// Territory form submission
		$('#territoryForm').submit(function(e) {
			e.preventDefault();
			$('#locations option').prop('disabled', false);
			var territoryName = $('#territory_name').val();
			var trimmedName = territoryName.trim().replace(/\s+/g, ' ');
			if (trimmedName.length === 0) {
				Swal.fire('Error', "Territory name cannot be blank.", 'error');
				return false;
			}
			if (trimmedName.length < 2 || trimmedName.length > 50) {
				Swal.fire('Error', "Territory name must be between 2 and 50 characters.", 'error');
				return false;
			}
			var regex = /^[\p{L}\s'-]+$/u;
			if (!regex.test(trimmedName)) {
				Swal.fire('Error', "Territory name can only contain letters, spaces, hyphens, and apostrophes.", 'error');
				return false;
			}
			...
			if (!$('#territory_id').val()) {
				var duplicate = false;
				territoryTable.rows().data().each(function(value) {
					if (value.territory_name.trim().toLowerCase() === territoryName.trim().toLowerCase()) {
						duplicate = true;
					}
				});
				if (duplicate) {
					Swal.fire('Error', 'Duplicate territory name. Please choose a unique name.', 'error');
					return false;
				}
			}
			var formData = $(this).serialize();
			$.ajax({
				url: "<?= site_url('TerritoryController/saveTerritory') ?>",
				type: "POST",
				data: formData,
				dataType: "json",
				success: function(resp) {
					if (resp.status) {
						...
						$('#territory_id').val('');
						$('#territorySubmitBtn').text('Save');
						$('#territoryCancelBtn').hide();
						territoryTable.ajax.reload(function() {
							updateLocationsDropdown();
						});
						reloadTerritoryDropdown();
		...
		// Clear territory form
		// $('#territoryClearBtn').click(function() {
		// 	$('#territoryForm')[0].reset();
		// 	$('#territory_id').val('');
		// 	$('#territorySubmitBtn').text('Save');
		// 	$('#territoryCancelBtn').hide();
		// 	$('#locations').html(originalLocations);
		// 	$('#locations').select2({
		// 		placeholder: "Select one or more locations",
		...
		$('#territoryCancelBtn').click(function() {
			$('#territoryForm')[0].reset();
			$('#territory_id').val('');
			$('#territorySubmitBtn').text('Save');
			$(this).hide();
		...
		// Territory deletion with assignment check
		$('#territoryTable').on('click', '.deleteTerritory', function() {
			var territoryId = $(this).data('id');
			var isAssigned = false;
			employeeTable.rows().data().each(function(row) {
				if (row.territory_id == territoryId) {
					isAssigned = true;
				}
			});
			if (isAssigned) {
				Swal.fire('Error', 'Cannot delete territory because it is assigned to one or more employees. Please remove the assignment(s) first.', 'error');
				return;
			}
			Swal.fire({
				title: 'Are you sure?',
				text: "This will delete the territory and any related employee assignments.",
				icon: 'warning',
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "<?= site_url('TerritoryController/deleteTerritory') ?>/" + territoryId,
						type: "POST",
						dataType: "json",
						success: function(resp) {
							if (resp.status) {
		...
		// Territory edit event
		$('#territoryTable').on('click', '.editTerritory', function() {
			var rowData = territoryTable.row($(this).parents('tr')).data();
			$('#territory_id').val(rowData.territory_id);
			$('#territory_name').val(rowData.territory_name);
			var locArr;
			if (Array.isArray(rowData.locations)) {
				locArr = rowData.locations;
			} else {
				try {
					locArr = JSON.parse(rowData.locations);
				} catch (e) {
					locArr = rowData.locations ? rowData.locations.split(',') : [];
				}
			}
			updateLocationsDropdown(locArr);
			$('#territorySubmitBtn').text('Update');
			$('#territoryCancelBtn').show();
			disableEmployeeForm(true);
			disableTerritoryForm(false);
			$('html, body').animate({
                scrollTop: $('#territoryForm').offset().top - 200
              }, 'fast');
		});

		// Employee assignment form submission
		$('#employeeAssignForm').submit(function(e) {
			e.preventDefault();
			$('#entity_ids option').prop('disabled', false);
			if (!$('#assign_territory').val()) {
				Swal.fire('Error', 'Please select a territory for assignment.', 'error');
				return false;
			}
			if (!$('#entity_ids').val() || $('#entity_ids').val().length === 0) {
				Swal.fire('Error', 'Please select at least one employee.', 'error');
				return false;
			}
			var formData = $(this).serialize();
			var assignmentId = $('#emp_teritory_assign_id').val();
			var url = assignmentId ? "<?= site_url('TerritoryController/updateAssignment') ?>" : "<?= site_url('TerritoryController/assignEmployee') ?>";
			$.ajax({
				url: url,
				type: "POST",
				data: formData,
				...
				success: function(resp) {
					if (resp.status) {
						Swal.fire('Success', resp.message, 'success');
						$('#employeeAssignForm')[0].reset();
						$('#entity_ids').val(null).trigger('change');
						addClearButton($('#entity_ids'));
						$('#assign_territory').val('');
						$('#emp_teritory_assign_id').val('');
		...
		// Cancel employee assignment edit
		...
		// Clear employee assignment form
		// $('#assignClearBtn').click(function() {
		// 	$('#employeeAssignForm')[0].reset();
		// 	$('#entity_ids').val(null).trigger('change');
		// 	addClearButton($('#entity_ids'));
		// 	$('#assign_territory').val('');
		...
		// Employee assignment edit event
		$('#employeeAssignTable').on('click', '.editAssign', function() {
			var rowData = employeeTable.row($(this).parents('tr')).data();
			$('#emp_teritory_assign_id').val(rowData.emp_teritory_assign_id);
			var empIds;
			try {
				empIds = JSON.parse(rowData.entity_id);
			} catch (e) {
				empIds = rowData.entity_id ? rowData.entity_id.toString().split(',') : [];
			}
			$('#entity_ids').val(empIds).trigger('change');
			addClearButton($('#entity_ids'));
			$('#assign_territory').val(rowData.territory_id);
			$('#assignBtn').text('Update Assignment');
			$('#assignCancelBtn').show();
			disableTerritoryForm(true);
			disableEmployeeForm(false);
			$('html, body').animate({
                scrollTop: $('#employeeAssignForm').offset().top - 200
              }, 'fast');
		});

		// Employee assignment delete event
		$('#employeeAssignTable').on('click', '.deleteAssign', function() {
			var id = $(this).data('id');
			Swal.fire({
				title: 'Are you sure?',
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "<?= site_url('TerritoryController/deleteEmployeeAssignment') ?>/" + id,
						type: "POST",
						dataType: "json",
						success: function(resp) {
							if (resp.status) {
								...
								Swal.fire('Error', "Assignment deletion failed: " + resp.message, 'error');
							}
						},
						error: function(xhr, status, error) {
							Swal.fire('Error', 'An error occurred while deleting the assignment: ' + error, 'error');
						}
					});
				}
			});
		...
		// Reload territory dropdown in the Employee Assignment form
		function reloadTerritoryDropdown() {
			$.ajax({
				url: "<?= site_url('TerritoryController/getTerritoryList') ?>",
				type: "POST",
				dataType: "json",
				success: function(resp) {
					if (resp.data) {
						...
						$.each(resp.data, function(i, ter) {
							options += '<option value="' + ter.territory_id + '">' + ter.territory_name + '</option>';
						});
						$('#assign_territory').html(options);
					}
				},
				error: function() {
					console.log("Failed to load territories for dropdown.");
				}
			});
		}
		reloadTerritoryDropdown();
