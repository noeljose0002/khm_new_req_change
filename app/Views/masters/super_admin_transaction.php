            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Add New process-Transaction</h4>
                ...
                <body>
                    <div class="container mt-5">
                        ...
                        <!-- Add Transaction-Process Section -->
                        <div id="addSection" class="card">
                            <div class="card-header form-header">
                                <h4>Add New Transaction-Process</h4>
                            </div>
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="entity_trans_name">Name</label>
                                            <input type="text" class="form-control" id="entity_trans_name" name="entity_trans_name" required>
                                        </div>

                                        <!-- Corrected ID here -->
                                        <div class="form-group col-md-4">
                                            <label for="prs_type_id">Process Type</label>
                                            <select class="form-control" id="prs_type_id" name="prs_type_id" required>
                                                <option value="">-- Select Type --</option>
                                            </select>
                                        </div>

                                        <div class="form-group col-md-4">
                                            <label for="prs_parent_id">Parent Process</label>
                                            <select class="form-control" id="prs_parent_id" name="prs_parent_id">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            ...
                                            <input type="number" class="form-control" id="enterprise_id" name="enterprise_id" required>
                                        </div>
                                        <div class="form-group col-md-4">
                        ...
                        <!-- Edit Transaction-Process Section -->
                        <div id="editSection" class="card" style="display:none;">
                            <div class="card-header form-header">
                                <h4>Edit Transaction-Process</h4>
                            </div>
                            <div class="card-body">
                                <form id="editForm">
                                      <input type="hidden" name="_method" value="PUT">
                                    <input type="hidden" id="edit_entity_trans_id" name="entity_trans_id">

                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="edit_entity_trans_name">Name</label>
                                            <input type="text" class="form-control" id="edit_entity_trans_name" name="entity_trans_name" required>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="edit_prs_type_id">Process Type</label>
                                            <select class="form-control" id="edit_prs_type_id" name="prs_type_id" required>
                                                <option value="">-- Select Type --</option>
                                            </select>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="edit_prs_parent_id">Parent Process</label>
                                            <select class="form-control" id="edit_prs_parent_id" name="prs_parent_id">
                                                <option value="">-- None --</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            ...
                                            <input type="number" class="form-control" id="edit_enterprise_id" name="enterprise_id" required>
                                        </div>
                                        <div class="form-group col-md-4">
                        ...
                        <!-- Transaction-Process List Table -->
                        <div class="card">
                            <div class="card-header">
                                <h4>Transaction-Process List</h4>
                            </div>
                            <div class="card-body">
...
<script>
    $(document).ready(function() {
        ...
        // 1) Fetch items for both dropdowns
        $.getJSON('<?= site_url("SuperAdminTransactionProcess/list") ?>', function(res) {
            allItems = res.data;

            // helper to build options from any [{id,text}] array
            function buildOptions(arr, noneLabel, selected) {
            ...
            // prepare arrays
            const parentArr = allItems.map(i => ({
                id: i.entity_trans_id,
                text: i.entity_trans_name
            }));
          

            // inject into ADD form
           
            $('#prs_parent_id').html(buildOptions(parentArr, '-- None --'));

            // inject into EDIT form (we’ll re-select when editing)
        
            $('#edit_prs_parent_id').html(buildOptions(parentArr, '-- None --'));
        });

        $.getJSON('<?= site_url("SuperAdminTransactionProcess/listType") ?>', function(res) {
            allTypes = res.data;
            ...
            // inject into ADD form
            $('#prs_type_id').html(buildOptions(typeArr, '-- Select Type --'));
          

            // inject into EDIT form (we’ll re-select when editing)
            $('#edit_prs_type_id').html(buildOptions(typeArr, '-- Select Type --'));
        ...
        const table = $('#transTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminTransactionProcess/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'entity_trans_id' ,
                   
                },
                {
                    data: 'entity_trans_name',
                    title: 'Name'
                },
                {
                    data: 'prs_type_id',
                    title: 'Process Type',
                    render: function(pid) {
                        const found = allTypes.find(x => '' + x.prs_type_id === '' + pid);
                        return found ? found.prs_name : '';
                    }
                },
                {
                    data: 'prs_parent_id',
                    ...
                    render: function(pid) {
                        const found = allItems.find(x => '' + x.entity_trans_id === '' + pid);
                        return found ? found.entity_trans_name : '';
                    }
                },
                {
                    data: 'enterprise_id',
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post(
                '<?= site_url("SuperAdminTransactionProcess/create") ?>',
                $(this).serialize()
            ).done(() => {
                $('#addForm')[0].reset();
                table.ajax.reload();
                Swal.fire('Saved!', 'New entry added.', 'success');
            }).fail(xhr => {
                Swal.fire('Error', xhr.responseJSON?.errors?.entity_trans_name || 'Failed', 'error');
            });
        });

        // 5) SHOW EDIT
        $('#transTable').on('click', '.editBtn', function() {
            let id = $(this).data('id');
            $.getJSON(`<?= site_url("SuperAdminTransactionProcess/edit") ?>/${id}`, function(data) {
                $('#edit_entity_trans_id').val(data.entity_trans_id);
                $('#edit_entity_trans_name').val(data.entity_trans_name);
                $('#edit_prs_type_id').val(data.prs_type_id);
                $('#edit_prs_parent_id').val(data.prs_parent_id);
                $('#edit_enterprise_id').val(data.enterprise_id);
                $('#edit_menu_link').val(data.menu_link);

                // repopulate dropdown in case list changed
                $('#edit_prs_parent_id').html(
                    allItems.reduce((h, item) => {
                        let sel = item.entity_trans_id == data.prs_parent_id ? ' selected' : '';
                        return h + `<option value="${item.entity_trans_id}"${sel}>${item.entity_trans_name}</option>`;
                    }, '<option value="">-- None --</option>')
                );

                $('#edit_prs_type_id').html(
                    allTypes.reduce((h, item) => {
                        let sel = item.prs_type_id == data.prs_type_id ? ' selected' : '';
                        return h + `<option value="${item.prs_type_id}"${sel}>${item.prs_name}</option>`;
                    }, '<option value="">-- None --</option>')
                );
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            let id = $('#edit_entity_trans_id').val();
            $.ajax({
                url: `<?= site_url("SuperAdminTransactionProcess/update") ?>/${id}`,
                type: 'POST',
                data: $(this).serialize()
            }).done(() => {
                $('#editForm')[0].reset();
        ...
        // 8) DELETE
        $('#transTable').on('click', '.deleteBtn', function() {
            let id = $(this).data('id');
            Swal.fire({
                title: 'Delete?',
                ...
                if (res.isConfirmed) {
                    $.ajax({
                        url: `<?= site_url("SuperAdminTransactionProcess/delete") ?>/${id}`,
                        type: 'DELETE'
                    }).done(() => {
                        table.ajax.reload();
