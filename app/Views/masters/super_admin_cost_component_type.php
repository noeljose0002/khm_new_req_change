	...
	<div class="page">
		<div class="page-main">
			...
			<div class="app-content icon-content">
				<div class="section">
					...
					<div class="page-header">
						<div class="page-leftheader">
							<h4 class="page-title mb-0">Add New Cost Component Type</h4>
				...
				<head>
					<meta charset="UTF-8">
					<title>Cost Component Type </title>
					<!-- Bootstrap 4 CSS -->
					...
					<style>
						...
						#costComponentTypeTable thead th {
					...
					<div class="container mt-5">
						...
						<div id="addSection" class="card">
							<div class="card-header form-header">
								<h4>Add New Cost Component Type</h4>
							</div>
							<div class="card-body">
								<form id="addForm">
									<input type="hidden" name="_method" value="POST">
									<input type="hidden" id="cost_component_type_id" name="cost_component_type_id">
									<div class="form-row">

										<div class="form-group col-md-5">
											<label for="cost_component_type_name">Cost Component Type Name</label>
											<input
												type="text"
												class="form-control"
												id="cost_component_type_name"
												name="cost_component_type_name"
												maxlength="50"
												required>
										</div>
						...
						<div id="editSection" class="card" style="display:none;">
							<div class="card-header form-header">
								<h4>Edit Cost Component Type</h4>
							</div>
							<div class="card-body">
								<form id="editForm">
									<input type="hidden" name="_method" value="PUT">
									<input type="hidden" id="edit_cost_component_type_id" name="cost_component_type_id">

									<div class="form-row">

										<div class="form-group col-md-5">
											<label for="edit_cost_component_type_name">Cost Component Type Name</label>
											<input
												type="text"
												class="form-control"
												id="edit_cost_component_type_name"
												name="cost_component_type_name"
												maxlength="50"
												required>
										</div>
						...
						<div class="card mt-4">
							...
							<div class="card-body">
								<table id="costComponentTypeTable" class="table table-bordered">
									<thead></thead>
									<tbody></tbody>
								</table>
							</div>
...
<script>
	$(document).ready(function() {
		...
		const table = $('#costComponentTypeTable').DataTable({
			ajax: {
				url: '<?= site_url("SuperAdminCostComponentType/list") ?>',
				dataSrc: 'data'
			},
			columns: [{
					data: 'cost_component_type_id',
					title: 'ID'
				},
				{
					data: 'cost_component_type_name',
					title: 'Name'
				},
		...
		$('#addForm').submit(function(e) {
			e.preventDefault();
			$.post('<?= site_url("SuperAdminCostComponentType/create") ?>', $(this).serialize())
				.done(() => {
					$('#addForm')[0].reset();
					  $('#cost_component_type_id').val('');
					table.ajax.reload();
		...
		// 6) SHOW EDIT
		$('#costComponentTypeTable').on('click', '.editBtn', function() {
			const id = $(this).data('id');
			$.getJSON(`<?= site_url("SuperAdminCostComponentType/edit") ?>/${id}`, function(data) {
				// fill hidden ID
				$('#edit_cost_component_type_id').val(data.cost_component_type_id);
				// fill other fields
				$('#edit_cost_component_type_name').val(data.cost_component_type_name);
		...
		$('#editForm').submit(function(e) {
			e.preventDefault();
			const id = $('#edit_cost_component_type_id').val();
			$.ajax({
					url: `<?= site_url("SuperAdminCostComponentType/update") ?>/${id}`,
					type: 'POST',
					data: $(this).serialize()
				})
				.done(() => {
		...
		// 9) DELETE
		$('#costComponentTypeTable').on('click', '.deleteBtn', function() {
			const id = $(this).data('id');
			Swal.fire({
				title: 'Delete?',
				...
				if (res.isConfirmed) {
					$.ajax({
							url: `<?= site_url("SuperAdminCostComponentType/delete") ?>/${id}`,
							type: 'DELETE'
						})
						.done(() => {
							table.ajax.reload();
