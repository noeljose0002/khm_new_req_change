			...
			<div class="app-content icon-content">
				...
				<script>
					window.addEventListener("load", function() {
						...
						if (existingDistance) {
							$('#distanceValue').text("Distance: " + parseFloat(existingDistance) + " km");
							$('#distanceResult').show();
						}
								...
								$.ajax({
									...
									success: function(response) {
										if (response && !response.not_found && response.records && response.records.length > 0) {
											...
											if (exactRecord) {
												var currentDistance = parseFloat(exactRecord.geog_km_distance);
											...
											if (response.records.length > 1) {
												var firstDistance = parseFloat(response.records[0].geog_km_distance),
													secondDistance = parseFloat(response.records[1].geog_km_distance);
											...
											response.records.forEach(function(rec) {
												...
												tableHtml += '<td><input type="number" step="any" class="form-control edit-distance" value="' + parseFloat(rec.geog_km_distance) + '" readonly></td>';
    ...
    ${record.geog_name}
						...
						function calculateDistance() {
							...
							$.getJSON(apiUrl, function(response) {
								if (response.features && response.features.length > 0) {
									...
									$('#distanceValue').text("Distance: " + distance + " km");
						...
						$('#saveDistanceBtn').on('click', function() {
							...
							if ($('#distanceInput').length > 0) {
								...
								$('#distanceValue').text("Distance: " + distanceVal + " km");
							} else {
						...
						$('#editDistanceBtn').on('click', function() {
							...
							if ($('#distanceInput').length === 0) {
								...
								$('#distanceValue').html("Distance: <input type='number' step='any' id='distanceInput' value='" + parseFloat(currentText) + "'> km");
								...
								$.ajax({
									...
									success: function(response) {
										var storedDistance = "";
										if (response && !response.not_found && response.records && response.records.length > 0) {
											storedDistance = parseFloat(response.records[0].geog_km_distance);
											// If a second record exists, compare distances.
											if (response.records.length > 1) {
												var secondDistance = parseFloat(response.records[1].geog_km_distance);
										...
										$.ajax({
											...
											success: function(response) {
												...
												if (response.success) {
													...
													$('#distanceValue').text("Distance: " + newDistance + " km");
													// Update the datatable's value (if it exists).
													$('.edit-distance').val(newDistance);
									...
									if (response && !response.not_found && response.records && response.records.length > 0) {
										...
										response.records.forEach(function(rec) {
											...
											tableHtml += '<td><input type="number" step="any" class="form-control edit-distance" value="' +
												parseFloat(rec.geog_km_distance) + '" readonly></td>';
