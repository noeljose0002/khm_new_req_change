...
class Dashboard_m extends Model
{
    ...
    //nj//
    public function get_season_veh($object_id)
    {
        $db = \Config\Database::connect();
        $selected_table = $db->table('khm_season_calendar a');
        $result = $selected_table->select('a.*')
    ...
    public function get_veh_season_rates_model($season_id)
    {
        $db = \Config\Database::connect();
        $selected_table = $db->table('khm_obj_khm_vehicle_special_tariff');
        $result = $selected_table->select('rate_per_day, km_rate, extra_km_rate')
            ->where('special_tariff_id', $season_id)
            ->get()->getResultArray();
        return $result;
    }
    ...
    //nj//
    public function check_rc_exist_except(int $hotel_id, int $room_category_id, int $exclude_id): bool
    {
        return (bool) $this->db
            ->table($this->khm_obj_hotel_room_category)
            ->where('hotel_id',         $hotel_id)
            ->where('room_category_id', $room_category_id)
            ->where('hotel_room_category_id !=', $exclude_id)
            ->countAllResults();
    }
    /////
    ...
    public function get_veh_seasons_table($params)
    {
        ...
        $query->orderBy('f.season_start_date','DESC');
    ...
    public function get_room_basic_tariff_for_system_filter($object_id, $system_id)
    {
        ...
        $builder->distinct()
            ...
            ->join(
                'khm_obj_mst_hotel_room_category rc',
                'rc.room_category_id = a.room_category_id',
                'left'
            )
            ->join(
                'khm_obj_mst_room_type rt',
                'rt.room_type_id = a.room_type_id',
                'left'
            )
            ->join(
                'khm_obj_mst_room_occupancy ro',
    ...
    public function vehicle_list_view($params)
    {
        ...
        $query = $db->table('khm_obj_mst a')
            ...
            a.*,
            m.geog_name,
            hub.hub_location_name,
            mod.vehicle_model_name,
            seat.vehicle_seat_capacity,
            t.tour_travel_daily_rate,
            t.tour_travel_max_km,
            t.extra_km_rate
        ', false)
            ->join('khm_obj_vehicle h', 'h.object_id = a.object_id', 'left')
            ->join('khm_obj_mst_vehicle_type t', 't.vehicle_type_id = h.vehicle_type_id', 'left')
            ->join('khm_obj_mst_vehicle_hub_location hub', 'hub.hub_location_id = t.hub_location_id', 'left')
            ->join('khm_obj_mst_vehicle_model mod', 'mod.vehicle_model_id = t.vehicle_model_id', 'left')
            ->join('khm_obj_mst_vehicle_seat seat', 'seat.vehicle_seat_id = t.vehicle_seat_id', 'left')
            ->join('khm_loc_mst_geography m', 'm.geog_id = a.object_location_id', 'left')
            ->where('a.object_class_id', $object_class_id);
    ...
    public function ss_list_view($params)
    {
        ...
        (
            SELECT DATE_FORMAT(MAX(s.season_end_date), '%d/%m/%y')
            FROM khm_season_calendar s
            WHERE s.object_id = a.object_id
              AND s.enterprise_id = a.enterprise_id
        ) AS latest_end_date
