...
// Handles tour plan: $object_id is required, $edit_id is optional for editing a record,default null.
    public function tour_plan($object_id,$edit_id=null)
    {  
        if (!empty(session()->get('user_id'))) { //Checks whether the session contains user_id (i.e., user is logged in). If not logged in → redirect to login page. 
            $Enquiry_model = new Enquiry_m();//Enquiry_m handles tour enquiry-related database queries.
            $Dashboard_model = new Dashboard_m();//Dashboard_m handles user roles, permissions, menus, etc.
            $pre_enq_details = [];//will hold previous enquiry header/details array (default empty).
            $pre_tour_plan = []; //will hold previous tour plan rows (default empty).
            $pre_start_date = '1978-01-01';//default fallback previous start date (odd default — probably to ensure a valid date string).
            $pre_no_night = 0;//default previous number of nights.
