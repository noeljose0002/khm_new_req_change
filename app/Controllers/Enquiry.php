        ...
        if (!empty(session()->get('user_id'))) {
            ...
            foreach ($tour_plan_det as $keys => $vals) {
                ...
                for ($date = clone $start1; $date < $end1; $date->modify('+1 day')) {
                    ...
                    // NEW: Only load defaults if there was NO saved/draft record at all.
                    // This ensures that if the user removed defaults and saved an empty array, we respect that.
                    ...
                    // - Days 3 onwards (including last day): Load "Leisure" sightseeing
                    if (!$had_saved_record) {
                        $saved_sightseeing = [];
                        $saved_ss_ids = [];
                        $ss_total_distance = 0;
                        ...
                        if ($day_number === 2) {
                            ...
                            // Days 3 onwards (including last day): Auto-select "Leisure" if it exists for this location
                            $leisure_ss = $Enquiry_model->get_leisure_sight_seeing($vals['tour_location']);

                            if (!empty($leisure_ss)) {
                                // Take the first Leisure entry (there should typically be only one per location)
                                $ss = $leisure_ss[0];
                                ...
                                // Calculate values (Leisure typically has 0 tariff and 0 distance)
                                if ($ssItem['is_pax'] == 1) {
                                    $ssItem['calculated_value'] = $ssItem['tariff'] * $totalPax;
                                    $ssItem['cost'] = $ssItem['calculated_value'];
                                    $ss_pax_cost += $ssItem['calculated_value'];
                                ...
                                log_message('info', 'Auto-selected Leisure SS for ' . $day_type . ' (date ' . $tour_date . ', location ' . $vals['tour_location'] . ')');
                            } else {
                                // No Leisure available for this location - leave empty
                                $day_type = $is_last_day ? 'Last day' : 'Day ' . $day_number;
                                log_message('info', 'No Leisure sightseeing available for location ' . $vals['tour_location'] . ' on ' . $day_type . ' - leaving empty');
                            }
                        } else {
                            // Day 1: No defaults
                    ...
                    foreach ($tour_expansion_details[$tid] as &$expansion) {
                        ...
                        if (is_array($vehicle_details) && !empty($vehicle_details)) {
                            foreach ($vehicle_details as &$veh) {
                                ...
                                // Derive base: saved_total - saved_ss (or db_total - 0 for fresh/default)
                                ...
                                // Backward compat: No total_travel_distance set (or set to base if needed)
                        ...
                        // Log for debugging
                        log_message('info', 'Expansion date ' . $expansion_date . ': DB Total=' . $db_total . 'km, Saved SS=' . $ss_dist . 'km, Derived Base=' . $base_dist . 'km (display base; JS adds current SS)');
                    }
