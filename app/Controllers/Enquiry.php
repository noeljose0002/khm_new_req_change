...
public function makeItineraryCurrent()
{
    ...
    try {
        ...
        if (!empty($tour_details_ids)) {
            ...
            $update_current_itinerary_details = true; // No itinerary details to update
        }
        
        if ($update_all_itineraries && $update_current_itinerary && 
            $update_all_tour_details && $update_current_tour_details &&
            $update_all_itinerary_details && $update_current_itinerary_details) {
            echo json_encode([
                'success' => true, 
                'message' => 'Itinerary, tour details, and itinerary details updated successfully'
            ]);
        } else {
    ...
    public function viewItinerarySheet()
    {
        ...
        // Fetch expansion details for itinerary (NEW)
        $itinerary_expansion_details = [];
        $tour_expansion_details = [];
        ...
        // Fallback: Fetch tour expansion details if no itinerary expansion exists
        if (empty($itinerary_expansion_details)) {
            foreach ($tour_plan_det as $vals) {
                $tour_details_id = $vals['tour_details_id'];
                $tour_expansion_details[$tour_details_id] = $Enquiry_model->get_tour_expansion_by_tour_id($tour_details_id);
            }
        }
        ...
        // NEW: Pass expansion data to view
        $response['itinerary_expansion_details'] = $itinerary_expansion_details;
        $response['tour_expansion_details'] = $tour_expansion_details;
    ...
    public function viewMultipleItinerarySheet()
    {
        ...
        foreach ($selectedIDs as $key => $id) {
            ...
            $response[$key]['tour_expansion_details'] = $tour_expansion_details;
