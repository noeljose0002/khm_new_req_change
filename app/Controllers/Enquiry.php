...
class Enquiry extends BaseController
{
    ...
    //                             if ($no_of_single_room > 0 && isset($night_data['s_adult_rate'][1]) && !$has_first_single) {
    //                                 $first_s_adult_rate = (float)$night_data['s_adult_rate'][1];
    //                                 $first_s_child_rate = (float)($night_data['s_child_rate'][1] ?? 0);
    //                                 $first_s_child_wb_rate = (float)($night_data['s_child_wb_rate'][1] ?? 0);
    //                                 $first_s_extra_bed_rate = (float)($night_data['s_extra_bed_rate'][1] ?? 0);
    //                                 $has_first_single = true;
    //                             }
    //                         }
    //                     }
    ...
    //                                     'tour_details_id' => $tour_details_id,
    //                                     'tour_expansion_date' => $exp_date_str,
    //                                     'room_category_id' => isset($night_data['roomcat'][$s_room]) ? (int)$night_data['roomcat'][$s_room] : 0,
    //                                     'meal_plan_id' => isset($night_data['mealplan'][$s_room]) ? (int)$night_data['mealplan'][$s_room] : 0,
    //                                     'room_rate_double' => 0,
    //                                     'child_with_bed_double' => 0,
    //                                     'child_without_bed_double' => 0,
    //                                     'extra_bed_double' => 0,
    //                                     'double_total_rate' => 0,
    //                                     'room_rate_single' => isset($night_data['s_adult_rate'][$s_room]) ? (float)$night_data['s_adult_rate'][$s_room] : 0,
    //                                     'child_with_bed_single' => isset($night_data['s_child_rate'][$s_room]) ? (float)$night_data['s_child_rate'][$s_room] : 0,
    //                                     'child_without_bed_single' => isset($night_data['s_child_wb_rate'][$s_room]) ? (float)$night_data['s_child_wb_rate'][$s_room] : 0,
    //                                     'extra_bed_single' => isset($night_data['s_extra_bed_rate'][$s_room]) ? (float)$night_data['s_extra_bed_rate'][$s_room] : 0,
    //                                     'single_total_rate' => isset($night_data['s_total_rate'][$s_room]) ? (float)$night_data['s_total_rate'][$s_room] : 0,
    //                                     'vehicle_details_json' => $veh_json_str
    //                                 ];
    //                                 $Enquiry_model->insert_tour_expansion($expansion_data);
    //                             }
    ...
    public function itinerary($object_id, $final_save_flag, $edit_id = null, $iti_edit_id = null, $extension_ref_id = null)
    {
        if (!empty(session()->get('user_id'))) {
            $Enquiry_model = new Enquiry_m();
            $Dashboard_model = new Dashboard_m();
            $db = \Config\Database::connect();  // Add DB connection
            $system_name = session('system_name');
            $markups = $Enquiry_model->get_markup_details($system_name);
            if (!empty($markups)) {
                $mark_up = $markups[0]['mark_up'];
            ...
            foreach ($tour_plan_det as $keys => $vals) {
                ...
                // Implement expansion data fetch and processing
                ...
                foreach ($expansion_data as $exp) {
                    $date_key = date('Y-m-d', strtotime($exp['tour_expansion_date']));
                    if (!isset($tour_expansions[$date_key])) {
                        $tour_expansions[$date_key] = [];
                    }
                    // Map to expected structure
                    $hot_detail = [
                        'tour_date' => $date_key,
                        'remarks' => $exp['remarks'] ?? '',
                        'hotel_name' => $exp['hotel_name'] ?? 'Own Arrangements',
                        'room_category_name' => $exp['exp_room_category_name'],
                        'meal_plan_name' => $exp['exp_meal_plan_name'] ?? $vals['meal_plan_name'], // fallback
                        'double' => $exp['double'] ?? 0,
                        'single' => $exp['single'] ?? 0,
                        'no_of_ch' => $exp['no_of_ch'] ?? 0,
                        'no_of_cw' => $exp['no_of_cw'] ?? 0,
                        'no_of_extra' => $exp['no_of_extra'] ?? 0,
                        'd_adult_rate' => $exp['room_rate_double'],
                        'd_child_rate' => $exp['child_with_bed_double'],
                        'd_child_wb_rate' => $exp['child_without_bed_double'],
                        'd_extra_bed_rate' => $exp['extra_bed_double'],
                        's_adult_rate' => $exp['room_rate_single'],
                        's_child_rate' => $exp['child_with_bed_single'],
                        's_child_wb_rate' => $exp['child_without_bed_single'],
                        's_extra_bed_rate' => $exp['extra_bed_single'],
                        'fac_rate' => 0, // if needed
                    ];
                    $tour_expansions[$date_key][] = $hot_detail;
                }
                ...
                if (!empty($result2)) {
                    foreach ($result2 as &$day_detail) {
                        $date_key = date('Y-m-d', strtotime($day_detail['tour_date']));
                        if (isset($tour_expansions[$date_key])) {
                            $day_detail['hotel_details'] = json_encode($tour_expansions[$date_key]);
                        } else {
                            // Fallback to empty or main
                            $day_detail['hotel_details'] = '[]';
                        }
                    }
                    $itinerary_details_save = [...$itinerary_details_save, ...$result2];
                }
