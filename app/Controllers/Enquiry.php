...
class Enquiry extends BaseController
{
    ...
    private function extractDayBlocks(string $html): array
    {
        ...
        $isStop = function (\DOMNode $node) use ($stopMarkers): bool {
            ...
            foreach ($stopMarkers as $m) {
                if (strpos($txt, $m) !== false) return true;
            }
            return false;
        };
 ...
 public function getProformaOfficeData()
{
    ...
    public function saveTourPlan()
    {
        if (!empty(session()->get('user_id'))) {
            ...
            $enquiry_header_id = $this->request->getPost('enquiry_header_id');
            $enquiry_details_id = $this->request->getPost('enquiry_details_id');
            $previous_active_tour_id = null;

            //  $prev_tour = $Enquiry_model->get_tour_plan_details2($enquiry_header_id);
            //     if (!empty($prev_tour) && isset($prev_tour[0]['tour_details_id'])) {
            //         $previous_active_tour_id = $prev_tour[0]['tour_details_id'];
            //     }

              log_message('debug', 'edit_id: ' . print_r($edit_id, true));
        log_message('debug', 'enquiry_header_id: ' . print_r($enquiry_header_id, true));

           if ($edit_id > 0) { 
    log_message('debug', 'Edit mode ON, fetching previous tour'); 
    // Pass the current enquiry_details_id for comparison
    $prev_tour = $Enquiry_model->get_tour_plan_details2($enquiry_header_id, $edit_id, $enquiry_details_id); 
    log_message('debug', 'prev_tour result: ' . print_r($prev_tour, true)); 
    if (!empty($prev_tour)) { 
        // First try to get extension_ref_id, fallback to tour_details_id 
        if (isset($prev_tour[0]['extension_ref_id']) && $prev_tour[0]['extension_ref_id'] !== null) { 
            $previous_active_tour_id = $prev_tour[0]['extension_ref_id']; 
        } elseif (isset($prev_tour[0]['tour_details_id'])) { 
            $previous_active_tour_id = $prev_tour[0]['tour_details_id']; 
        } 
    }
                $up_data = array(
            ...
            if (!empty($addloc)) {
                ...
                foreach ($addloc as $seq => $item) {
                    ...
                    // GST tracking for eighteen_details (if tax_status = 1)
                    $gst_data_double = [];
                    $gst_data_single = [];

                    if (isset($item['nights']) && is_array($item['nights'])) {
                        foreach ($item['nights'] as $night => $night_data) {
                            // Calculate double totals
                            if (isset($night_data['dd_total_rate'])) {
                                $double_total += (float)$night_data['dd_total_rate'];
                            }

                            // Calculate single totals
                            if (isset($night_data['ss_total_rate'])) {
                                $single_total += (float)$night_data['ss_total_rate'];
                            }

                            // Collect GST data for eighteen_details
                            if ($item['tax_status'] == 1) {
                                // Process double rooms
                                if (isset($night_data['d_adult_rate']) && is_array($night_data['d_adult_rate'])) {
                                    foreach ($night_data['d_adult_rate'] as $room_idx => $adult_rate) {
                                        $base_total = (float)($adult_rate ?? 0) +
                                            (float)($night_data['d_child_rate'][$room_idx] ?? 0) +
                                            (float)($night_data['d_child_wb_rate'][$room_idx] ?? 0) +
                                            (float)($night_data['d_extra_bed_rate'][$room_idx] ?? 0);

                                        $gst_percent = (float)($night_data['d_gst_per'][$room_idx] ?? 0);
                                        $gst_amount = (float)($night_data['d_gst_amt'][$room_idx] ?? 0);
                                        $total_with_gst = (float)($night_data['d_total_rate'][$room_idx] ?? 0);

                                        // Create unique identifier for room
                                        $room_key = "{$seq}{$night}{$room_idx}";

                                        $gst_data_double[$room_key] = [
                                            'sequence_id' => $room_key,
                                            'room_rate' => (float)($night_data['d_adult_rate'][$room_idx] ?? 0),
                                            'no_of_child' => 0,
                                            'child_rate' => (float)($night_data['d_child_rate'][$room_idx] ?? 0),
                                            'no_of_child_wb' => 0,
                                            'child_wb_rate' => (float)($night_data['d_child_wb_rate'][$room_idx] ?? 0),
                                            'no_of_extra' => 0,
                                            'extra_rate' => (float)($night_data['d_extra_bed_rate'][$room_idx] ?? 0),
                                            'total' => $base_total,
                                            'gst' => $gst_percent,
                                ...
                                if (isset($night_data['s_adult_rate']) && is_array($night_data['s_adult_rate'])) {
                                    foreach ($night_data['s_adult_rate'] as $room_idx => $adult_rate) {
                                        $base_total = (float)($adult_rate ?? 0) +
                                            (float)($night_data['s_child_rate'][$room_idx] ?? 0) +
                                            (float)($night_data['s_child_wb_rate'][$room_idx] ?? 0) +
                                            (float)($night_data['s_extra_bed_rate'][$room_idx] ?? 0);

                                        $gst_percent = (float)($night_data['s_gst_per'][$room_idx] ?? 0);
                                        $gst_amount = (float)($night_data['s_gst_amt'][$room_idx] ?? 0);
                                        $total_with_gst = (float)($night_data['s_total_rate'][$room_idx] ?? 0);

                                        $room_key = "{$seq}{$night}{$room_idx}";

                                        $gst_data_single[$room_key] = [
                                            'sequence_id' => $room_key,
                                            'room_rate' => (float)($night_data['s_adult_rate'][$room_idx] ?? 0),
                                            'no_of_child' => 0,
                                            'child_rate' => (float)($night_data['s_child_rate'][$room_idx] ?? 0),
                                            'no_of_child_wb' => 0,
                                            'child_wb_rate' => (float)($night_data['s_child_wb_rate'][$room_idx] ?? 0),
                                            'no_of_extra' => 0,
                                            'extra_rate' => (float)($night_data['s_extra_bed_rate'][$room_idx] ?? 0),
                                            'total' => $base_total,
                                            'gst' => $gst_percent,
                                            'gst_amount' => $gst_amount,
                                            'grand_total' => $total_with_gst
                            ...
                            if (isset($night_data['veh_model']) && !empty(array_filter($night_data['veh_model'] ?? []))) {
                                ...
                                $max_kms_day = $night_data['max_km_day'] ?? [];
                                ...
                                for ($j = 0; $j < count($veh_models); $j++) {
                                    ...
                                    if (!isset($vehicle_types[$type_id])) {
                                        $vehicle_types[$type_id] = [
                                            ...
                                            'extra_kilometer' => 0.0,
                                            'veh_total' => 0.0,
                                            'max_km_day' => (float)($max_kms_day[$j] ?? 0),
                                            'veh_header' => '',
                                            'pre_to_cur' => '',
                                            'cur_to_dep' => '',
                                            'dep_to_arr' => '',
                                    ...
                                    $header_fields = ['veh_header', 'pre_to_cur', 'cur_to_dep', 'dep_to_arr', 'hub_to_arr', 'arr_to_loc'];
                                    foreach ($header_fields as $field) {
                                        $new_val = trim($night_data[$field] ?? '');
                                        if ($new_val && $vt[$field]) {
                                            $vt[$field] .= ' + ' . $new_val;
                                        } elseif ($new_val) {
                                            $vt[$field] = $new_val;
                    ...
                    if ($tour_exist > 0) {
                        ...
                        // Delete old expansion and eighteen records
                        ...
                        $tour_data_update = array(
                            ...
                            'room_category_id' => $item['roomcat_common'] ?? 0,
                        ...
                        // Insert GST data to eighteen_details
                        if ($item['tax_status'] == 1) {
                            // Insert double room GST data
                            foreach ($gst_data_double as $gst_item) {
                                $ster_ddata = array(
                                    'tour_details_id' => $tour_details_id,
                                    'sequence_id' => $gst_item['sequence_id'],
                                    'room_rate' => $gst_item['room_rate'],
                                    'no_of_child' => $gst_item['no_of_child'],
                                    'child_rate' => $gst_item['child_rate'],
                                    'no_of_child_wb' => $gst_item['no_of_child_wb'],
                                    'child_wb_rate' => $gst_item['child_wb_rate'],
                                    'no_of_extra' => $gst_item['no_of_extra'],
                                    'extra_rate' => $gst_item['extra_rate'],
                                    'total' => $gst_item['total'],
                                    'gst' => $gst_item['gst'],
                                    'grand_total' => $gst_item['grand_total']
                                );
                                $Enquiry_model->insert_eighteen_details_double($ster_ddata);
                            }

                            // Insert single room GST data
                            if ($no_of_single_room > 0) {
                                foreach ($gst_data_single as $gst_item) {
                                    $ster_sdata = array(
                                        'tour_details_id' => $tour_details_id,
                                        'sequence_id' => $gst_item['sequence_id'],
                                        'room_rate' => $gst_item['room_rate'],
                        ...
                        // Insert expansion per night per room
                        if (isset($item['nights']) && is_array($item['nights'])) {
                            ...
                            foreach ($item['nights'] as $night => $night_data) {
                                ...
                                // Loop over double rooms
                                for ($d_room = 1; $d_room <= $no_of_double_room; $d_room++) {
                                    $expansion_data = [
                                        'tour_details_id' => $tour_details_id,
                                        'tour_expansion_date' => $exp_date_str,
                                        'room_category_id' => isset($night_data['roomcat'][$d_room]) ? (int)$night_data['roomcat'][$d_room] : 0,
                                        'meal_plan_id' => isset($night_data['mealplan'][$d_room]) ? (int)$night_data['mealplan'][$d_room] : 0,
                                        'room_rate_double' => isset($night_data['d_adult_rate'][$d_room]) ? (float)$night_data['d_adult_rate'][$d_room] : 0,
                                        'child_with_bed_double' => isset($night_data['d_child_rate'][$d_room]) ? (float)$night_data['d_child_rate'][$d_room] : 0,
                                        'child_without_bed_double' => isset($night_data['d_child_wb_rate'][$d_room]) ? (float)$night_data['d_child_wb_rate'][$d_room] : 0,
                                        'extra_bed_double' => isset($night_data['d_extra_bed_rate'][$d_room]) ? (float)$night_data['d_extra_bed_rate'][$d_room] : 0,
                                        'double_total_rate' => isset($night_data['d_total_rate'][$d_room]) ? (float)$night_data['d_total_rate'][$d_room] : 0,
                                        'room_rate_single' => 0,
                                        'child_with_bed_single' => 0,
                                        'child_without_bed_single' => 0,
                                        'extra_bed_single' => 0,
                                        'single_total_rate' => 0,
                                ...
                                // Loop over single rooms
                                for ($s_room = 1; $s_room <= $no_of_single_room; $s_room++) {
                                    $s_index = $no_of_double_room + $s_room;
                                    $expansion_data = [
                                        'tour_details_id' => $tour_details_id,
                                        'tour_expansion_date' => $exp_date_str,
                                        'room_category_id' => isset($night_data['roomcat'][$s_index]) ? (int)$night_data['roomcat'][$s_index] : 0,
                                        'meal_plan_id' => isset($night_data['mealplan'][$s_index]) ? (int)$night_data['mealplan'][$s_index] : 0,
                                        'room_rate_double' => 0,
                                        'child_with_bed_double' => 0,
                                        'child_without_bed_double' => 0,
                                        'extra_bed_double' => 0,
                                        'double_total_rate' => 0,
                                        'room_rate_single' => isset($night_data['s_adult_rate'][$s_index]) ? (float)$night_data['s_adult_rate'][$s_index] : 0,
                                        'child_with_bed_single' => isset($night_data['s_child_rate'][$s_index]) ? (float)$night_data['s_child_rate'][$s_index] : 0,
                                        'child_without_bed_single' => isset($night_data['s_child_wb_rate'][$s_index]) ? (float)$night_data['s_child_wb_rate'][$s_index] : 0,
                                        'extra_bed_single' => isset($night_data['s_extra_bed_rate'][$s_index]) ? (float)$night_data['s_extra_bed_rate'][$s_index] : 0,
                                        'single_total_rate' => isset($night_data['s_total_rate'][$s_index]) ? (float)$night_data['s_total_rate'][$s_index] : 0,
                                        'vehicle_details_json' => $veh_json_str
                                    ];
                                    $Enquiry_model->insert_tour_expansion($expansion_data);
                                }
                        ...
                        // Update QQ if quick quote
                        if ($tour_updated && $is_quick_quote == 1) {
                            $qq_updates = [
                                ['tariff' => $first_d_adult_rate, 'room_type' => 2, 'component' => 6],
                                ['tariff' => $first_s_adult_rate, 'room_type' => 1, 'component' => 6],
                                ['tariff' => $first_d_child_rate, 'room_type' => 2, 'component' => 12],
                                ['tariff' => $first_s_child_rate, 'room_type' => 1, 'component' => 12],
                                ['tariff' => $first_d_child_wb_rate, 'room_type' => 2, 'component' => 15],
                                ['tariff' => $first_s_child_wb_rate, 'room_type' => 1, 'component' => 15],
                                ['tariff' => $first_d_extra_bed_rate, 'room_type' => 2, 'component' => 9],
                                ['tariff' => $first_s_extra_bed_rate, 'room_type' => 1, 'component' => 9]
                            ];

                            foreach ($qq_updates as $qq) {
                                $qq_data = array(
                                    'quick_quote_tariff' => $qq['tariff'],
                                    'is_draft' => $is_draft
                                );
                                $Enquiry_model->update_qq_details($qq_data, $tour_details_id, $qq['component'], $qq['room_type']);
                            }
                        }
                    } else {
                        // INSERT NEW LOCATION
                        ...
                        //     foreach ($qq_inserts as $qq) {
                        //         $qq_data = array(
                        //             'enquiry_header_id' => $enquiry_header_id,
                        //             'enquiry_details_id' => $enquiry_details_id,
                        //             'tour_details_id' => $tour_details_id,
                        //             'room_type_id' => $qq['room_type'],
                        //             'cost_component_id' => $qq['component'],
                        //             'quick_quote_tariff' => $qq['tariff'],
                        //             'is_active' => 1,
                        //             'is_draft' => $is_draft,
                        //             'enterprise_id' => 1
                        //         );
                        //         $Enquiry_model->insert_qq_details($qq_data);
                        //     }
                        // }
                        if ($tour_details_id) {
                            $qq_inserts = [
                ...
                // Final step: Link itinerary with tour plan if not draft
    ...
    public function saveTourLocation()
    {
        if (!empty(session()->get('user_id'))) {
            ...
            $no_of_double_room = (int)$this->request->getPost('no_of_double_room');
            $no_of_single_room = (int)$this->request->getPost('no_of_single_room');
            $addloc = $this->request->getPost('addloc');
            $loc_index = 1;
            ...
            // GST tracking for eighteen_details
            $gst_data_double = [];
            $gst_data_single = [];
            ...
            foreach ($nights as $night_index => $night) {
                ...
                if ($night_index === 1 && empty($d_adult_rate)) {
                    ...
                    $first_single_index = $no_of_double_room + 1;
                    $s_adult_rate = (float)($night['s_adult_rate'][$first_single_index] ?? 0);
                    $d_child_rate = (float)($night['d_child_rate'][1] ?? 0);
                    $s_child_rate = (float)($night['s_child_rate'][$first_single_index] ?? 0);
                    $d_child_wb_rate = (float)($night['d_child_wb_rate'][1] ?? 0);
                    $s_child_wb_rate = (float)($night['s_child_wb_rate'][$first_single_index] ?? 0);
                    $d_extra_bed_rate = (float)($night['d_extra_bed_rate'][1] ?? 0);
                    $s_extra_bed_rate = (float)($night['s_extra_bed_rate'][$first_single_index] ?? 0);
                }

                // Collect GST data for eighteen_details (if tax_status = 1)
                if (isset($loc['tax_status']) && $loc['tax_status'] == 1) {
                    // Process double rooms
                    if (isset($night['d_adult_rate']) && is_array($night['d_adult_rate'])) {
                        foreach ($night['d_adult_rate'] as $room_idx => $adult_rate) {
                            $base_total = (float)($adult_rate ?? 0) +
                                (float)($night['d_child_rate'][$room_idx] ?? 0) +
                                (float)($night['d_child_wb_rate'][$room_idx] ?? 0) +
                                (float)($night['d_extra_bed_rate'][$room_idx] ?? 0);

                            $gst_percent = (float)($night['d_gst_per'][$room_idx] ?? 0);
                            $gst_amount = (float)($night['d_gst_amt'][$room_idx] ?? 0);
                            $total_with_gst = (float)($night['d_total_rate'][$room_idx] ?? 0);

                            $room_key = "{$loc_index}{$night_index}{$room_idx}";

                            $gst_data_double[$room_key] = [
                                'sequence_id' => $room_key,
                                'room_rate' => (float)($night['d_adult_rate'][$room_idx] ?? 0),
                                'no_of_child' => 0,
                                'child_rate' => (float)($night['d_child_rate'][$room_idx] ?? 0),
                                'no_of_child_wb' => 0,
                                'child_wb_rate' => (float)($night['d_child_wb_rate'][$room_idx] ?? 0),
                                'no_of_extra' => 0,
                                'extra_rate' => (float)($night['d_extra_bed_rate'][$room_idx] ?? 0),
                                'total' => $base_total,
                                'gst' => $gst_percent,
                                'gst_amount' => $gst_amount,
                                'grand_total' => $total_with_gst
                    ...
                    // Process single rooms
                    if (isset($night['s_adult_rate']) && is_array($night['s_adult_rate'])) {
                        foreach ($night['s_adult_rate'] as $room_idx => $adult_rate) {
                            $base_total = (float)($adult_rate ?? 0) +
                                (float)($night['s_child_rate'][$room_idx] ?? 0) +
                                (float)($night['s_child_wb_rate'][$room_idx] ?? 0) +
                                (float)($night['s_extra_bed_rate'][$room_idx] ?? 0);

                            $gst_percent = (float)($night['s_gst_per'][$room_idx] ?? 0);
                            $gst_amount = (float)($night['s_gst_amt'][$room_idx] ?? 0);
                            $total_with_gst = (float)($night['s_total_rate'][$room_idx] ?? 0);

                            $room_key = "{$loc_index}{$night_index}{$room_idx}";

                            $gst_data_single[$room_key] = [
                                'sequence_id' => $room_key,
                                'room_rate' => (float)($night['s_adult_rate'][$room_idx] ?? 0),
                                'no_of_child' => 0,
                                'child_rate' => (float)($night['s_child_rate'][$room_idx] ?? 0),
                                'no_of_child_wb' => 0,
                                'child_wb_rate' => (float)($night['s_child_wb_rate'][$room_idx] ?? 0),
                                'no_of_extra' => 0,
                                'extra_rate' => (float)($night['s_extra_bed_rate'][$room_idx] ?? 0),
                                'total' => $base_total,
                                'gst' => $gst_percent,
                                'gst_amount' => $gst_amount,
                                'grand_total' => $total_with_gst
                ...
                $veh_totals = $night['veh_total'] ?? [];
                $max_kms_day = $night['max_km_day'] ?? [];
            ...
            $vehicle_details = json_encode($vehicle_summaries, JSON_PRETTY_PRINT);
            $room_type_json = json_encode([$room_type], JSON_PRETTY_PRINT);
            ...
            $common_data = [
                ...
                'accom_double_total' => $accom_double_total,
                'accom_single_total' => $accom_single_total,
            ...
            if ($tour_exist > 0) {
                ...
                if ($loc['tax_status'] == 1) {
                    foreach ($gst_data_double as $gst_item) {
                        $Enquiry_model->insert_eighteen_details_double(array(
                            'tour_details_id' => $tour_details_id,
                            'sequence_id' => $gst_item['sequence_id'],
                            'room_rate' => $gst_item['room_rate'],
                            'no_of_child' => $gst_item['no_of_child'],
                    ...
                    if ($no_of_single_room > 0) {
                        foreach ($gst_data_single as $gst_item) {
                            $Enquiry_model->insert_eighteen_details_single(array(
                                'tour_details_id' => $tour_details_id,
                                'sequence_id' => $gst_item['sequence_id'],
                                'room_rate' => $gst_item['room_rate'],
                                'no_of_child' => $gst_item['no_of_child'],
                ...
                // Insert expansion per night per room
                if (isset($nights) && is_array($nights)) {
                    $checkin_date = new DateTime($loc['checkin']);
                    foreach ($nights as $night_index => $night_data) {
                        $expansion_date = clone $checkin_date;
                        $expansion_date->add(new DateInterval('P' . ($night_index - 1) . 'D'));
                        $exp_date_str = $expansion_date->format('Y-m-d');
                        ...
                        // Loop over double rooms
                        for ($d_room = 1; $d_room <= $no_of_double_room; $d_room++) {
                            $Enquiry_model->insert_tour_expansion([
                                'tour_details_id' => $tour_details_id,
                                'tour_expansion_date' => $exp_date_str,
                                'room_category_id' => isset($night_data['roomcat'][$d_room]) ? (int)$night_data['roomcat'][$d_room] : 0,
                                'meal_plan_id' => isset($night_data['mealplan'][$d_room]) ? (int)$night_data['mealplan'][$d_room] : 0,
                ...
                if ($is_quick_quote == 1 && $tour_updated) {
                    $qq_updates = [
                        ['tariff' => $d_adult_rate, 'room_type' => 2, 'component' => 6],
                        ['tariff' => $s_adult_rate, 'room_type' => 1, 'component' => 6],
                        ['tariff' => $d_child_rate, 'room_type' => 2, 'component' => 12],
                        ['tariff' => $s_child_rate, 'room_type' => 1, 'component' => 12],
                        ['tariff' => $d_child_wb_rate, 'room_type' => 2, 'component' => 15],
                        ['tariff' => $s_child_wb_rate, 'room_type' => 1, 'component' => 15],
                        ['tariff' => $d_extra_bed_rate, 'room_type' => 2, 'component' => 9],
                        ['tariff' => $s_extra_bed_rate, 'room_type' => 1, 'component' => 9]
                    ];
                ...
                // Insert GST data to eighteen_details
                if ($loc['tax_status'] == 1 && $tour_details_id) {
                    foreach ($gst_data_double as $gst_item) {
                        $Enquiry_model->insert_eighteen_details_double(array(
                            'tour_details_id' => $tour_details_id,
                            'sequence_id' => $gst_item['sequence_id'],
                ...
                // Insert expansion per night per room
                if ($tour_details_id && isset($nights) && is_array($nights)) {
                    $checkin_date = new DateTime($loc['checkin']);
                    foreach ($nights as $night_index => $night_data) {
                        $expansion_date = clone $checkin_date;
                        $expansion_date->add(new DateInterval('P' . ($night_index - 1) . 'D'));
    ...
    //             // FOR ITINERARY FORM: Fetch tour expansion details (old structure - from khm_obj_enquiry_tour_expansion)
    ...
    //             if (!empty($result1)) {
    //                 $itinerary_details_draft = [...$itinerary_details_draft, ...$result1];
    //                 // FOR COSTING SHEET: Collect itinerary_details_ids
    ...
    //         // FOR COSTING SHEET: Fetch itinerary expansion details (new structure) after collecting all IDs
    ...
    //             }
    //         }
    //         // Override the original with the rekeyed version for the view
    //         $itinerary_expansion_details = $itinerary_expansion_details;
    ...
    //             $data['itinerary_expansion_details'] = $itinerary_expansion_details; // For Costing Sheet (now rekeyed by tour_details_id)
    ...
    //                 // FOR ITINERARY FORM: Fetch tour expansion details (old structure)
    //                 $tour_expansion_details[$tid] = $Enquiry_model->get_tour_expansion_by_tour_id($vals['tour_details_id']);
    //             }
    ...
    public function itinerary($object_id, $final_save_flag, $edit_id = null, $iti_edit_id = null, $extension_ref_id = null)
    {
        if (!empty(session()->get('user_id'))) {
            ...
            foreach ($tour_plan_det as $keys => $vals) {
                ...
                // PRELOAD PREVIOUS ITINERARY DATA (after tour_plan_det loop starts, before date loop)
                // CRITICAL FIX: Use the CURRENT tour plan's extension_ref_id to look up previous_active_tour_id
                $previous_itinerary_data = [];
                if (!empty($previous_itinerary_data)) {
                    log_message('info', 'Available previous itinerary keys: ' . implode(', ', array_keys($previous_itinerary_data)));
                    log_message('info', 'Current tour location ID: ' . $vals['tour_location']);

                    // Sample the first record to see its structure
                    $sample = reset($previous_itinerary_data);
                    log_message('info', 'Sample previous record: ' . json_encode([
                        'tour_date' => $sample['tour_date'] ?? 'N/A',
                        'tour_details_id' => $sample['tour_details_id'] ?? 'N/A',
                        'ss_data_json' => substr($sample['ss_data_json'] ?? '', 0, 100)
                    ]));
                }
                // Get the current extension_ref_id being worked on
                $current_extension_ref_id = null;

                if ($edit_id > 0 && $extension_ref_id !== null) {
                    // Editing a specific version
                    $current_extension_ref_id = $extension_ref_id;
                } else {
                    // Get the latest/current tour plan's extension_ref_id
                    if (!empty($tour_plan_det)) {
                        $current_extension_ref_id = $tour_plan_det[0]['extension_ref_id'] ?? null;
                    }
                }

                log_message('info', 'Current extension_ref_id for previous itinerary lookup: ' . ($current_extension_ref_id ?? 'NULL'));

                if (!empty($current_extension_ref_id)) {
                    // This will internally look up previous_active_tour_id from the current tour plan
                    $previous_itinerary_data = $Enquiry_model->get_previous_itinerary_grouped(
                        $object_det[0]['enquiry_header_id'],
                        $enquiry_details_id_new,
                        $current_extension_ref_id  // Pass current extension_ref_id, model will find previous_active_tour_id
                    );

                    log_message('info', 'Loaded previous itinerary data: ' . count($previous_itinerary_data) . ' records from current extension_ref_id=' . $current_extension_ref_id);
                } else {
                    log_message('warning', 'No current_extension_ref_id available - cannot load previous itinerary');
                }

                // Inside the foreach ($tour_plan_det as $keys => $vals) loop
                // Replace the date loop section with this updated version:
                ...
                for ($date = clone $start1; $date < $end1; $date->modify('+1 day')) {
                    ...
                    $had_draft_record = false;
                    $had_previous_record = false;
                    $saved_sightseeing = [];
                    $saved_ss_ids = [];
                    $ss_total_distance = 0;
                    $ss_pax_cost = 0;
                    ...
                    // PRIORITY 4: Load defaults if no saved/draft/previous data exists
                    if (!$had_saved_record && !$had_draft_record && !$had_previous_record) {
                        $saved_sightseeing = [];
                        $saved_ss_ids = [];
                        $ss_total_distance = 0;
                        $ss_pax_cost = 0;
                        ...
                        if ($day_number === 2) {
                            ...
                            log_message('info', 'PRIORITY 4 - Loaded DEFAULT SS for Day 2 (date ' . $tour_date . ', location=' . $vals['tour_location'] . ')');
                        } elseif ($day_number > 2) {
                            // Days 3 onwards: Create hardcoded "Leisure" entry
                            $leisure_id = -999;
                            ...
                            log_message('info', 'PRIORITY 4 - Hardcoded Leisure for ' . $day_type . ' (date ' . $tour_date . ', location=' . $vals['tour_location'] . ')');
                        } else {
                            log_message('info', 'PRIORITY 4 - Day 1 (date ' . $tour_date . ', location=' . $vals['tour_location'] . ') - No defaults');
                        }
                    }

                    // Store the data by date for this tour
                    if (!isset($saved_sightseeing_by_date[$vals['tour_details_id']])) {
                        $saved_sightseeing_by_date[$vals['tour_details_id']] = [];
                    }

                    // FIXED: Ensure json_addons and json_special_event are always arrays
                    if (!isset($json_addons) || !is_array($json_addons)) {
                        $json_addons = [];
                    }
                    if (!isset($json_special_event) || !is_array($json_special_event)) {
                        $json_special_event = [];
                    }
                ...
                if (isset($tour_expansion_details[$tid])) {
                    foreach ($tour_expansion_details[$tid] as &$expansion) {
                        ...
                        // Get SS distance ONLY for logging/display purposes
                        $ss_dist = 0;
                        ...
                        if (is_array($vehicle_details) && !empty($vehicle_details)) {
                            foreach ($vehicle_details as &$veh) {
                                // CRITICAL FIX: travel_distance in DB is ALREADY the base distance
                                // No subtraction needed - use it directly as base
                                $base_dist = (float)($veh['travel_distance'] ?? 0);

                                // Set base distance (this is what we'll display + SS will be added by JS)
                                $veh['base_travel_distance'] = $base_dist;
                                $veh['travel_distance'] = $base_dist;  // View displays this as base
                        ...
                        // Log for debugging - show what we're working with
                        log_message('info', 'Expansion date ' . $expansion_date . ': Base Distance=' . $base_dist . 'km, Current SS=' . $ss_dist . 'km, Expected Total=' . ($base_dist + $ss_dist) . 'km (JS will add SS dynamically)');
                    }
            ...
            if (!empty($itinerary_details_ids)) {
                ...
                foreach ($itinerary_expansion_details as $tid => &$exp_group) {
                    foreach ($exp_group as &$expansion) {
                        ...
                        // Get SS distance for subtraction ONLY if saved (not default/fresh)
                        $ss_dist = 0;
                        if (isset($saved_sightseeing_by_date[$tid][$expansion_date])) {
                            $data = $saved_sightseeing_by_date[$tid][$expansion_date];
                            $ss_dist = $data['is_saved'] ? $data['ss_total_distance'] : 0;
                        ...
                        log_message('info', 'Itinerary expansion date ' . $expansion_date . ': Derived Base=' . $base_dist . 'km (no SS added)');
                    }
            ...
            if (empty($tour_plan_det)) {
                ...
                $data['all_agents'] = $Enquiry_model->get_all_agents();
                $data['tour_plan_det'] = $tour_plan_det;
                $tour_plan_draft_det = $Enquiry_model->get_tour_plan_draft_details(enquiry_header_id: $object_det[0]['enquiry_header_id'], enquiry_details_id: $enquiry_details_id_new);
                $data['tour_plan_draft_det'] = $tour_plan_draft_det;
                if ($object_det[0]['is_quick_quote'] && !empty($tour_plan_det)) {
                    $quick_quote_det = $Enquiry_model->get_quick_quote_details($object_det[0]['enquiry_header_id'], $enquiry_details_id_new, $tour_plan_det[0]['tour_details_id']);
                }
                ...
                if ($submit_type == "draft") {
                    ...
                    foreach ($additi as $key => $item) {
                        ...
                        if ($tax_status == 1) {
                            ...
                            if (!empty($gst_totals['child_eighteen_double'])) {
                                $item['child_eighteen_double'] = $gst_totals['child_eighteen_double'];
                            }
                            if (!empty($gst_totals['child_wb_eighteen_double'])) {
                                $item['child_wb_eighteen_double'] = $gst_totals['child_wb_eighteen_double'];
                            }
                            if (!empty($gst_totals['extra_eighteen_double'])) {
                                $item['extra_eighteen_double'] = $gst_totals['extra_eighteen_double'];
                            }
                            if (!empty($gst_totals['adult_eighteen_single'])) {
                                $item['adult_eighteen_single'] = $gst_totals['adult_eighteen_single'];
                            }
                        }
                        ...
                        if ($tour_exist > 0) {
                            ...
                            if (!empty(array_filter($item['veh_model'] ?? []))) {
                                foreach ($item['veh_model'] as $index => $model_raw) {
                                    ...
                                    $travel_distance = (float) ($item['travel_distance'][$index] ?? 0);
                                    $extra_km_rate = (float) ($item['extra_km_rate'][$index] ?? 0);
                                    $adhoc_rate = (float) ($item['adhoc_rate'][$index] ?? 0);
                                    $extra_kilometer = $travel_distance > $max_km_day ? $travel_distance - $max_km_day : 0;
                                    $extra_cost = $extra_kilometer * $extra_km_rate;
                            ...
                            $iti_data_update = array(
                                ...
                                'child_without_bed' => $item['no_of_cw'],
                                'extra_bed' => $item['no_of_extra'],
                                'double_room' => $total_double_rooms,
                                'single_room' => $total_single_rooms,
                                'vehicle_details' => $json_output,
                                'hotel_details' => $json_hotels,
                                'special_event_name' => $item['spcl_event'],
                                'json_special_event' => $json_specials,
     ...
     * 
     * @param object $Enquiry_model Model instance
     * @param int $tour_details_id Tour details ID
     * @param string $tour_date Tour date in Y-m-d format
     * @return array Array with GST-inclusive totals
     */
    private function calculateGSTInclusiveTotals($Enquiry_model, $tour_details_id, $tour_date)
    {
        $double_total = 0;
        $single_total = 0;
        $adult_eighteen_double = 0;
        ...
        // Fetch double room data with GST - matching by tour_date
        $eighteen_datas_double = $Enquiry_model->get_eighteen_datas_double_by_date($tour_details_id, $tour_date);

        if (!empty($eighteen_datas_double)) {
            foreach ($eighteen_datas_double as $row) {
                $grand_total = (float)$row['grand_total']; // Grand total already includes GST
                $double_total += $grand_total;

                // Calculate base total for this row
                $base_total = (float)$row['room_rate'] +
                    ((float)$row['child_rate'] * (int)$row['no_of_child']) +
                    ((float)$row['child_wb_rate'] * (int)$row['no_of_child_wb']) +
                    ((float)$row['extra_rate'] * (int)$row['no_of_extra']);

                // GST is the amount, not rate
                $tax_amount = (float)$row['gst'];

                // Apportion tax for adult (room_rate assumed for adults)
                if ($base_total > 0) {
                    $adult_base = (float)$row['room_rate'];
                    $adult_tax = ($adult_base / $base_total) * $tax_amount;
                    $adult_eighteen_double += $adult_base + $adult_tax;

                    // Apportion for child with bed
                    $child_base = (float)$row['child_rate'] * (int)$row['no_of_child'];
                    $child_tax = ($child_base / $base_total) * $tax_amount;
                    $child_eighteen_double += $child_base + $child_tax;

                    // Apportion for child without bed
                    $child_wb_base = (float)$row['child_wb_rate'] * (int)$row['no_of_child_wb'];
                    $child_wb_tax = ($child_wb_base / $base_total) * $tax_amount;
                    $child_wb_eighteen_double += $child_wb_base + $child_wb_tax;

                    // Apportion for extra bed
                    $extra_base = (float)$row['extra_rate'] * (int)$row['no_of_extra'];
                    $extra_tax = ($extra_base / $base_total) * $tax_amount;
                    $extra_eighteen_double += $extra_base + $extra_tax;
                } else {
                    // If base_total is 0, just add the base values without tax
                    $adult_eighteen_double += (float)$row['room_rate'];
                    $child_eighteen_double += (float)$row['child_rate'] * (int)$row['no_of_child'];
                    $child_wb_eighteen_double += (float)$row['child_wb_rate'] * (int)$row['no_of_child_wb'];
                    $extra_eighteen_double += (float)$row['extra_rate'] * (int)$row['no_of_extra'];
                }
            }
        }

        // Fetch single room data with GST - matching by tour_date
        $eighteen_datas_single = $Enquiry_model->get_eighteen_datas_single_by_date($tour_details_id, $tour_date);

        if (!empty($eighteen_datas_single)) {
            foreach ($eighteen_datas_single as $row) {
                $grand_total = (float)$row['grand_total']; // Grand total already includes GST
                $single_total += $grand_total;

                // For single: full room rate + full tax
                $adult_eighteen_single += $grand_total;
            }
        }

        return [
            'double_with_gst' => $double_total,
            'single_with_gst' => $single_total,
            'total_with_gst' => $double_total + $single_total,
            'adult_eighteen_double' => $adult_eighteen_double,
            'child_eighteen_double' => $child_eighteen_double,
            'child_wb_eighteen_double' => $child_wb_eighteen_double,
            'extra_eighteen_double' => $extra_eighteen_double,
            'adult_eighteen_single' => $adult_eighteen_single
        ];
    }

    /**
     * Insert expansion data for double or single rooms
     */
    private function insertExpansionData($Enquiry_model, $itinerary_details_id, $item, $room_type = 'double')
    {
        // If tax_status is 1, no expansions
        if (isset($item['tax_status']) && $item['tax_status'] == 1) {
            return;
        }

        // Determine which arrays to use based on room type
        if ($room_type == 'double') {
            $room_cats = $item['d_room_cat'] ?? [];
            $meal_plans = $item['d_meal_plan'] ?? [];
            $room_counts = $item['double'] ?? [];
            $adult_rates = $item['d_adult_rate'] ?? [];
            $child_rates = $item['d_child_rate'] ?? [];
            $child_wb_rates = $item['d_child_wb_rate'] ?? [];
            $extra_bed_rates = $item['d_extra_bed_rate'] ?? [];
        } else {
            $room_cats = $item['s_room_cat'] ?? [];
            $meal_plans = $item['s_meal_plan'] ?? [];
            $room_counts = $item['single'] ?? [];
            $adult_rates = $item['s_adult_rate'] ?? [];
            $child_rates = $item['s_child_rate'] ?? [];
            $child_wb_rates = $item['s_child_wb_rate'] ?? [];
            $extra_bed_rates = $item['s_extra_bed_rate'] ?? [];
        }

        // If no room data, return
        if (!is_array($room_cats) || count($room_cats) == 0) {
            return;
        }

        // Prepare vehicle details JSON (same for all rooms on this date)
        $vehicle_json = !empty($item['veh_model']) ? $this->prepareVehicleJson($item) : null;

        // Get total quantities from the itinerary (for tracking purposes)
        $total_child_qty = isset($item['no_of_ch']) ? intval($item['no_of_ch']) : 0;
        $total_child_wb_qty = isset($item['no_of_cw']) ? intval($item['no_of_cw']) : 0;
        $total_extra_qty = isset($item['no_of_extra']) ? intval($item['no_of_extra']) : 0;

        // Track how many of each we've distributed
        $child_distributed = 0;
        $child_wb_distributed = 0;
        $extra_distributed = 0;

        // Insert one row for EACH room with its INDIVIDUAL rates
        foreach ($room_cats as $index => $room_cat) {
            // Get individual room data
            $room_category_id = intval($room_cat);
            $meal_plan_id = isset($meal_plans[$index]) ? intval($meal_plans[$index]) : 0;
            $room_count = isset($room_counts[$index]) ? intval($room_counts[$index]) : 1;

            // Get INDIVIDUAL rates for THIS specific room (NOT divided)
            $adult_rate = isset($adult_rates[$index]) ? floatval($adult_rates[$index]) : 0;
            $child_rate = isset($child_rates[$index]) ? floatval($child_rates[$index]) : 0;
            $child_wb_rate = isset($child_wb_rates[$index]) ? floatval($child_wb_rates[$index]) : 0;
            $extra_bed_rate = isset($extra_bed_rates[$index]) ? floatval($extra_bed_rates[$index]) : 0;

            // Determine quantities for THIS room based on whether it has rates
            // Only assign quantity if this room has a rate AND we haven't distributed all yet
            $room_child_qty = ($child_rate > 0 && $child_distributed < $total_child_qty) ? 1 : 0;
            $room_child_wb_qty = ($child_wb_rate > 0 && $child_wb_distributed < $total_child_wb_qty) ? 1 : 0;
            $room_extra_qty = ($extra_bed_rate > 0 && $extra_distributed < $total_extra_qty) ? 1 : 0;

            // Update distributed counts
            $child_distributed += $room_child_qty;
            $child_wb_distributed += $room_child_wb_qty;
            $extra_distributed += $room_extra_qty;

            // Calculate total for THIS room
            $room_total = $adult_rate +
                ($child_rate * $room_child_qty) +
                ($child_wb_rate * $room_child_wb_qty) +
                ($extra_bed_rate * $room_extra_qty);

            // Insert expansion record for this specific room
            $expansion_data = array(
                'itinerary_details_id' => $itinerary_details_id,
                'tour_details_id' => $item['tour_details_id'],
                'tour_expansion_date' => $item['tour_date'],
                'room_category_id' => $room_category_id,
                'meal_plan_id' => $meal_plan_id,
                'room_rate_' . $room_type => $adult_rate,
                'child_with_bed_' . $room_type => $child_rate,
                'child_without_bed_' . $room_type => $child_wb_rate,
                'extra_bed_' . $room_type => $extra_bed_rate,
                'no_of_child_with_bed' => $room_child_qty,
                'no_of_child_without_bed' => $room_child_wb_qty,
                'no_of_extra_bed' => $room_extra_qty,
                $room_type . '_total_rate' => $room_total,
                'vehicle_details_json' => $vehicle_json
            );

            $Enquiry_model->insert_expansion_data($expansion_data);
        }
    }
    ...
    //         // Calculate base total for THIS room
    //         $base_total = $adult_rate +
    //             ($child_rate * $room_child_qty) +
    //             ($child_wb_rate * $room_child_wb_qty) +
    //             ($extra_bed_rate * $room_extra_qty);

    //         // FIXED: For tax=1, add GST to total_rate (base rates remain base)
    //         $gst_rate = 0;
    //         $room_total = $base_total;  // Start with base
    //         if ($item['tax_status'] == 1) {
    //             // Derive sequence_id from index (e.g., index +1, adjust based on your logic)
    //             $sequence_id = $index + 1;  // Or fetch from form/item if available
    //             $gst_rate = $Enquiry_model->get_gst_rate_by_tour_and_sequence($item['tour_details_id'], $sequence_id);
    //             if ($gst_rate > 0) {
    //                 $tax_amount = $base_total * ($gst_rate / 100);
    //                 $room_total += $tax_amount;  // Now inclusive
    //             }
    //         }
    ...
    //             'room_category_id' => $room_category_id,
    //             'meal_plan_id' => $meal_plan_id,
    //             'room_rate_' . $room_type => $adult_rate,  // BASE (unchanged)
    //             'child_with_bed_' . $room_type => $child_rate,  // BASE
    //             'child_without_bed_' . $room_type => $child_wb_rate,  // BASE
    //             'extra_bed_' . $room_type => $extra_bed_rate,  // BASE
    //             'no_of_child_with_bed' => $room_child_qty,
    //             'no_of_child_without_bed' => $room_child_wb_qty,
    //             'no_of_extra_bed' => $room_extra_qty,
    //             $room_type . '_total_rate' => round($room_total, 2),  // INCLUSIVE if tax=1
    //             'vehicle_details_json' => $vehicle_json
    //         );
    //         $Enquiry_model->insert_expansion_data($expansion_data);
    //     }
    ...
    /**
     * Prepare vehicle details JSON for expansion table
     */
    private function prepareVehicleJson($item)
    {
        $vehicle_data = [];

        if (!empty($item['veh_model']) && is_array($item['veh_model'])) {
            foreach ($item['veh_model'] as $index => $model_raw) {
                $model_name = trim($model_raw);

                // Skip if vehicle model is empty
                if (empty($model_name)) {
                    continue;
                }
                ...
                $vehicle_data[] = [
                    "vehicle_model" => $model_name,
                    "veh_type_id" => isset($item['veh_type_id'][$index]) ? $item['veh_type_id'][$index] : '',
                    "tour_date" => isset($item['v_tour_date'][$index]) ? $item['v_tour_date'][$index] : '',
                    "day_rent" => isset($item['day_rent'][$index]) ? $item['day_rent'][$index] : 0,
                    "max_km_day" => isset($item['max_km_day'][$index]) ? $item['max_km_day'][$index] : 0,
                    "extra_km_rate" => isset($item['extra_km_rate_hidden'][$index]) ? $item['extra_km_rate_hidden'][$index] : 0,
                    "travel_distance" => isset($item['travel_distance'][$index]) ? $item['travel_distance'][$index] : 0,
                    "extra_kilometer" => isset($item['extra_kilometer'][$index]) ? $item['extra_kilometer'][$index] : 0,
                    "adhoc_rate" => isset($item['adhoc_rate'][$index]) ? floatval($item['adhoc_rate'][$index]) : 0, // ADD THIS LINE
                    "veh_total" => isset($item['veh_total'][$index]) ? $item['veh_total'][$index] : 0,
                    "veh_header" => isset($item['veh_header']) ? $item['veh_header'] : '',
                    "chk_vehicle" => isset($item['chk_vehicle_value'][$index]) ? $item['chk_vehicle_value'][$index] : '',
                    "chk_vehicle_status" => isset($item['chk_vehicle_hidden'][$index]) ? $item['chk_vehicle_hidden'][$index] : 0
                ];
            }
        }

        return !empty($vehicle_data) ? json_encode($vehicle_data, JSON_PRETTY_PRINT) : null;
    }
    /**
     * Update cost details (for draft mode)
     */
    private function updateCostDetails($Enquiry_model, $itinerary_details_id, $item)
    {
        // Calculate total rates from arrays
        $d_adult_rate_total = $this->sumRates($item['d_adult_rate'] ?? 0);
        $s_adult_rate_total = $this->sumRates($item['s_adult_rate'] ?? 0);
        $d_child_rate_total = $this->sumRates($item['d_child_rate'] ?? 0);
        $s_child_rate_total = $this->sumRates($item['s_child_rate'] ?? 0);
        $d_child_wb_rate_total = $this->sumRates($item['d_child_wb_rate'] ?? 0);
        $s_child_wb_rate_total = $this->sumRates($item['s_child_wb_rate'] ?? 0);
        $d_extra_bed_rate_total = $this->sumRates($item['d_extra_bed_rate'] ?? 0);
        $s_extra_bed_rate_total = $this->sumRates($item['s_extra_bed_rate'] ?? 0);

        $d_room_rate = array('tariff' => $d_adult_rate_total);
        $Enquiry_model->update_iti_cost_details($d_room_rate, $itinerary_details_id, 6, 2);

        $s_room_rate = array('tariff' => $s_adult_rate_total);
        $Enquiry_model->update_iti_cost_details($s_room_rate, $itinerary_details_id, 6, 1);

        $d_c_rate = array('tariff' => $d_child_rate_total);
        $Enquiry_model->update_iti_cost_details($d_c_rate, $itinerary_details_id, 12, 2);

        $s_c_rate = array('tariff' => $s_child_rate_total);
        $Enquiry_model->update_iti_cost_details($s_c_rate, $itinerary_details_id, 12, 1);

        $d_cw_rate = array('tariff' => $d_child_wb_rate_total);
        $Enquiry_model->update_iti_cost_details($d_cw_rate, $itinerary_details_id, 15, 2);

        $s_cw_rate = array('tariff' => $s_child_wb_rate_total);
        $Enquiry_model->update_iti_cost_details($s_cw_rate, $itinerary_details_id, 15, 1);

        $d_e_rate = array('tariff' => $d_extra_bed_rate_total);
        $Enquiry_model->update_iti_cost_details($d_e_rate, $itinerary_details_id, 9, 2);

        $s_e_rate = array('tariff' => $s_extra_bed_rate_total);
        $Enquiry_model->update_iti_cost_details($s_e_rate, $itinerary_details_id, 9, 1);

        $spcl_tariff = array('tariff' => $item['spcl_tariff'] ?? 0);
        $Enquiry_model->update_iti_cost_details($spcl_tariff, $itinerary_details_id, 17, 1);

        $hot_fac_id = array('tariff' => 0);
        $Enquiry_model->update_iti_cost_details($hot_fac_id, $itinerary_details_id, 18, 1);

        $hot_fac_tariff = array('tariff' => $item['fac_rate'] ?? 0);
        $Enquiry_model->update_iti_cost_details($hot_fac_tariff, $itinerary_details_id, 19, 1);

        $ss_id = array('tariff' => $item['sight'] ?? 0);
        $Enquiry_model->update_iti_cost_details($ss_id, $itinerary_details_id, 21, 1);

        $ss_distance = array('tariff' => $item['ss_distance'] ?? 0);
        $Enquiry_model->update_iti_cost_details($ss_distance, $itinerary_details_id, 22, 1);

        $daily_addon = array('tariff' => $item['daily_addon'] ?? 0);
        $Enquiry_model->update_iti_cost_details($daily_addon, $itinerary_details_id, 23, 1);
    }

    /**
     * Update cost details with is_draft = 1 (for final mode)
     * Fixed to handle arrays properly
     */
    private function updateCostDetailsFinal($Enquiry_model, $itinerary_details_id, $item)
    {
        // Calculate total rates from arrays
        $d_adult_rate_total = $this->sumRates($item['d_adult_rate'] ?? 0);
        $s_adult_rate_total = $this->sumRates($item['s_adult_rate'] ?? 0);
        ...
        $d_room_rate = array('tariff' => $d_adult_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($d_room_rate, $itinerary_details_id, 6, 2);

        $s_room_rate = array('tariff' => $s_adult_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($s_room_rate, $itinerary_details_id, 6, 1);

        $d_c_rate = array('tariff' => $d_child_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($d_c_rate, $itinerary_details_id, 12, 2);

        $s_c_rate = array('tariff' => $s_child_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($s_c_rate, $itinerary_details_id, 12, 1);

        $d_cw_rate = array('tariff' => $d_child_wb_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($d_cw_rate, $itinerary_details_id, 15, 2);

        $s_cw_rate = array('tariff' => $s_child_wb_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($s_cw_rate, $itinerary_details_id, 15, 1);

        $d_e_rate = array('tariff' => $d_extra_bed_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($d_e_rate, $itinerary_details_id, 9, 2);

        $s_e_rate = array('tariff' => $s_extra_bed_rate_total, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($s_e_rate, $itinerary_details_id, 9, 1);

        $spcl_tariff = array('tariff' => $item['spcl_tariff'] ?? 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($spcl_tariff, $itinerary_details_id, 17, 1);

        $hot_fac_id = array('tariff' => 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($hot_fac_id, $itinerary_details_id, 18, 1);

        $hot_fac_tariff = array('tariff' => $item['fac_rate'] ?? 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($hot_fac_tariff, $itinerary_details_id, 19, 1);

        $ss_id = array('tariff' => $item['sight'] ?? 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($ss_id, $itinerary_details_id, 21, 1);

        $ss_distance = array('tariff' => $item['ss_distance'] ?? 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($ss_distance, $itinerary_details_id, 22, 1);

        $daily_addon = array('tariff' => $item['daily_addon'] ?? 0, 'is_draft' => 1);
        $Enquiry_model->update_iti_cost_details($daily_addon, $itinerary_details_id, 23, 1);
    }

    /**
     * Helper function to sum rates whether they're arrays or single values
     */
    private function sumRates($value)
    {
        if (is_array($value)) {
    ...
    /**
     * Insert cost details (for draft mode)
     */
    private function insertCostDetails($Enquiry_model, $itinerary_details_id, $item)
    {
        // Calculate total rates from arrays
        $d_adult_rate_total = $this->sumRates($item['d_adult_rate'] ?? 0);
        $s_adult_rate_total = $this->sumRates($item['s_adult_rate'] ?? 0);
        ...
        $cost_details = [
            ['cost_component_id' => 6, 'room_type_id' => 2, 'tariff' => $d_adult_rate_total],
            ['cost_component_id' => 6, 'room_type_id' => 1, 'tariff' => $s_adult_rate_total],
            ['cost_component_id' => 12, 'room_type_id' => 2, 'tariff' => $d_child_rate_total],
            ['cost_component_id' => 12, 'room_type_id' => 1, 'tariff' => $s_child_rate_total],
            ['cost_component_id' => 15, 'room_type_id' => 2, 'tariff' => $d_child_wb_rate_total],
            ['cost_component_id' => 15, 'room_type_id' => 1, 'tariff' => $s_child_wb_rate_total],
            ['cost_component_id' => 9, 'room_type_id' => 2, 'tariff' => $d_extra_bed_rate_total],
            ['cost_component_id' => 9, 'room_type_id' => 1, 'tariff' => $s_extra_bed_rate_total],
            ['cost_component_id' => 17, 'room_type_id' => 1, 'tariff' => $item['spcl_tariff'] ?? 0],
            ['cost_component_id' => 18, 'room_type_id' => 1, 'tariff' => 0],
            ['cost_component_id' => 19, 'room_type_id' => 1, 'tariff' => $item['fac_rate'] ?? 0],
            ['cost_component_id' => 21, 'room_type_id' => 1, 'tariff' => $item['sight'] ?? 0],
            ['cost_component_id' => 22, 'room_type_id' => 1, 'tariff' => $item['ss_distance'] ?? 0],
            ['cost_component_id' => 23, 'room_type_id' => 1, 'tariff' => $item['daily_addon'] ?? 0],
        ];

        foreach ($cost_details as $detail) {
            $data = array(
                'itinerary_details_id' => $itinerary_details_id,
                'cost_component_id' => $detail['cost_component_id'],
            ...
            $Enquiry_model->insert_iti_cost_details($data);
        }
    }

    /**
     * Insert cost details with is_draft = 1 (for final mode)
     * Fixed to handle arrays properly
     */
    private function insertCostDetailsFinal($Enquiry_model, $itinerary_details_id, $item)
    {
        // Calculate total rates from arrays
        $d_adult_rate_total = $this->sumRates($item['d_adult_rate'] ?? 0);
        $s_adult_rate_total = $this->sumRates($item['s_adult_rate'] ?? 0);
