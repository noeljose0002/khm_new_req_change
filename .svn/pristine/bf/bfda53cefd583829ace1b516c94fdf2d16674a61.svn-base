      ...
      <div class="app-content icon-content">
        <div class="section">
          ...
          <div class="page-header">
            <div class="page-leftheader">
              <h4 class="page-title mb-0">Follow ups</h4>
        ...
        <head>
          ...
          <style>
            ...
            /* Fix DataTable spacing and alignment */
            #callFollowUpTable_wrapper {
              padding: 10px;
            }
            ...
            #callFollowUpTable th,
            #callFollowUpTable td {
              text-align: center;
              padding: 8px;
            ...
            #callFollowUpTable_wrapper .dataTables_length,
            #callFollowUpTable_wrapper .dataTables_filter {
        ...
        <body>
          ...
          <div class="card">
            ...
            <div class="card-body">
              <table id="markupTable" class="table table-bordered table-striped">
                <thead align="center">
                  <tr>
                    <th>SL NO</th>
                    <th>ENQUIRY NAME</th>
                ...
                <tbody>
                  ...
                  foreach ($enquiries as $enquiry): ?>
                    <tr align="center">
                      <td><?= $i++; ?></td>
                      <td><?= esc($enquiry['object_name']); ?></td>
                      <td align="center">
                        <i class="bi bi-eye text-success open-modal"
                          style="font-size: 22px; color: #14532d; cursor: pointer; margin: 5px; transition: transform 0.2s;"
                          onmouseover="this.style.transform='scale(1.2)'"
                          onmouseout="this.style.transform='scale(1)'"
                          style="cursor: pointer;"
                          data-enterprise="<?= $enquiry['enterprise_id']; ?>"
                          data-object_id="<?= $enquiry['object_id']; ?>"
                          data-enquiry_name="<?= esc($enquiry['object_name']); ?>"
                          title="View Followup"></i>
          ...
          <!-- Main Followup Modal that opens from the Enquiry List -->
            ...
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="followupModalTitle"></h5>
                  <button type="button" class="close text-black" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <div class="followup-btn-container" id="followupTypesContainer">
                    <div class="followup-group" id="enquiryGroup">
                      <h4>Enquiry</h4>
                      <div class="followup-btns">
                        <button class="btn btn-success" data-toggle="modal" data-target="#executiveModal">Executive</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#arrivalDetailsModal">Arrival Details</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#departureDetailsModal">Departure Details</button>
                        <button class="btn btn-success common-modal-btn" data-followup="Pre Arrival call" data-followup-id="9">Pre Arrival Call</button>
                        <button class="btn btn-success common-modal-btn" data-followup="Arrival call" data-followup-id="15">Arrival Call</button>
                        <button class="btn btn-success common-modal-btn" data-followup="Departure call" data-followup-id="10">Departure Call</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#placardModal">Placard</button>
                      </div>
                    </div>
                    <div class="followup-group" id="itineraryGroup">
                      <h4>Itinerary</h4>
                      <div class="followup-btns">
                        <button class="btn btn-success" data-toggle="modal" data-target="#hotelConfirmationModal">Hotel Confirmation</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#transportFollowUpModal">Transport Follow Up</button>
                        <button class="btn btn-success common-modal-btn" data-followup="Driver follow up" data-followup-id="16">Driver Followup</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#itineraryModal">Itinerary</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#voucherModal">Voucher</button>
                      </div>
                    </div>
                    <div class="followup-group" id="viewOnlyGroup">
                      <h4>View Only</h4>
                      <div class="followup-btns">
                        <button class="btn btn-success" data-toggle="modal" data-target="#transportItineraryModal">Transport Itinerary</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#finalItineraryModal">Final Itinerary</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#proformaOfficeCopyModal">Proforma (Office Copy)</button>
                        <button class="btn btn-success" data-toggle="modal" data-target="#proformaGuestCopyModal">Proforma (Guest Copy)</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <!-- Common Modal for Pre Arrival Call, Arrival Call, Departure Call, and Driver Followup -->
          <div class="modal fade" id="commonModal" tabindex="-1" role="dialog" aria-labelledby="commonModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <button class="btn btn-success custom-create-btn mb-3" id="btnAddFollowUp">
                    <i class="fas fa-plus"></i> Create New
                  </button>

                  <table id="callFollowUpTable" class="table table-bordered">
                    <thead>
                      <tr>
                        <th>Call Time</th>
                        <th>Call Status</th>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="addFollowupForm">
                    <input type="hidden" id="followup_type_id_form" name="followup_type_id" value="">
                    <input type="hidden" id="object_id_form" name="object_id" value="">
                    <div class="form-group">
                      <label for="call_status_form">Call Status</label>
                      <input type="text" class="form-control" id="call_status_form" name="call_status" required>
                    </div>
                    <div class="form-group">
                      <label for="call_time_form">Call Time</label>
                      <input type="datetime-local" class="form-control" id="call_time_form" name="call_time" required>
                    </div>
                    <div class="form-group">
                      <label for="comments_form">Comments</label>
                      <textarea class="form-control" id="comments_form" name="comments"></textarea>
                    </div>
          ...
          <!-- Additional Modals (unchanged) -->
          <div class="modal fade" id="executiveModal" tabindex="-1" role="dialog" aria-labelledby="executiveModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="executiveModalLabel">Executive Follow Up</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Executive followup goes here -->
                </div>
              </div>
            </div>
          </div>
          <!-- arrivaldetails model form and datatble -->
          <div class="modal fade" id="arrivalDetailsModal" tabindex="-1" role="dialog" aria-labelledby="arrivalDetailsModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content ">
                <div class="modal-header">
                  <h5 class="modal-title" id="arrivalDetailsModalLabel">Arrival Details</h5>
                  <button type="button" class="close text-black" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <button class="btn btn-success custom-create-btn mb-3" id="btnArrivalFollowUp">
                    <i class="fas fa-plus"></i> Create New
                  </button>
                  <table id="arrivalFollowUpTable" class="table table-bordered">
                    <thead>
                      <tr>
                        <th>Call Date</th>
                        <th>Call Time</th>
                  ...
                  <!-- Content for Arrival Details followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <!-- Add Arrival Up Modal -->
          <!-- Add Arrival Followup Modal -->
          <div class="modal fade" id="arrivalFollowUpModal" tabindex="-1" role="dialog" aria-labelledby="arrivalFollowUpModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document"> <!-- Using modal-lg for a larger modal -->
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="addarrivalFollowUpModalLabel">Add New Arrival Followup</h5>
                  <button type="button" class="close text-black" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <form id="arrivalFollowupForm">
                    <input type="hidden" id="arrival_followup_type_id_form" name="followup_type_id" value="">
                    <input type="hidden" id="arrival_object_id_form" name="object_id" value="">
                    <input type="hidden" id="arrival_follow_up_id" name="arrival_follow_up_id" value="">

                    <!-- Row 1: Call Date, Call Time, Mode of Arrival, Flight/Train Number -->
                    <div class="row">
                      <div class="col-md-3">
                        <div class="form-group">
                          <label for="call_date_form">Call Date</label>
                          <input type="date" class="form-control" id="call_date_form" name="call_date" required>
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                          <label for="arrival_call_time_form">Call Time</label>
                          <input type="time" class="form-control" id="arrival_call_time_form" name="call_time" required>
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                          <label for="mode_form">Mode of Arrival</label>
                          <input type="text" class="form-control" id="mode_form" name="mode" required maxlength="100">
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                          <label for="flight_train_form">Flight/Train Number</label>
                          <input type="text" class="form-control" id="flight_train_form" name="flight_train" required maxlength="100">
                        </div>
                      </div>
                    </div>

                    <!-- Row 2: City, Arrival Date, Arrival Time -->
                    <div class="row">
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="city_form">City</label>
                          <input type="text" class="form-control" id="city_form" name="city" required maxlength="100" pattern="^[A-Za-z\s\(\)]+$" title="Only alphabets, spaces, and parentheses are allowed.">
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="arrival_form">Arrival Date</label>
                          <input type="date" class="form-control" id="arrival_form" name="arrival" required>
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="time_form">Arrival Time</label>
                          <input type="time" class="form-control" id="time_form" name="time" required>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="arrival_comments_form">Comments</label>
                          <textarea class="form-control" id="arrival_comments_form" name="comments" maxlength="200"></textarea>
                        </div>
                      </div>
                    </div>
          ...
          <!-- departure -->
          <div class="modal fade" id="departureDetailsModal" tabindex="-1" role="dialog" aria-labelledby="departureDetailsModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header ">
                  <h5 class="modal-title" id="departureDetailsModalLabel">Departure Details</h5>
                  <button type="button" class="close text-black" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Departure Details followup goes here -->
                  <button class="btn btn-success custom-create-btn mb-3" id="btnDepartureFollowUP">
                    <i class="fas fa-plus"></i> Create New
                  </button>



                  <table id="departureFollowUpTable" class="table table-bordered">
                    <thead>
                      <tr>
                        <th>Call Date</th>
                        <th>Call Time</th>
                        <th>Mode Of Departure</th>
                        <th>City</th>
                        <th>Flight/Train Number</th>
          ...
          <!-- Add Departure Up Modal -->
          <!-- Add Departure Followup Modal -->
          <div class="modal fade" id="departureFollowUpModal" tabindex="-1" role="dialog" aria-labelledby="departureFollowUpModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document"> <!-- Using modal-lg for a larger modal -->
              <div class="modal-content">
                <div class="modal-header ">
                  <h5 class="modal-title" id="adddepartureFollowUpModalLabel">Add New Departure Followup</h5>
                  <button type="button" class="close text-black" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <form id="departureFollowupForm">
                    <!-- Hidden fields -->
                    <input type="hidden" id="departure_followup_type_id_form" name="followup_type_id" value="">
                    <input type="hidden" id="departure_object_id_form" name="object_id" value="">
                    <input type="hidden" id="departure_follow_up_id" name="departure_follow_up_id" value="">

                    <!-- Row 1: Call Date & Call Time -->
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          <label for="call_date_form">Call Date</label>
                          <input type="date" class="form-control" id="departure_call_date_form" name="call_date" required>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                          <label for="call_time_form">Call Time</label>
                          <input type="time" class="form-control" id="departure_call_time_form" name="call_time" required>
                        </div>
                      </div>
                    </div>

                    <!-- Row 2: Mode of Departure & Flight/Train Number -->
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          <label for="departure_mode_form">Mode of Departure</label>
                          <input type="text" class="form-control" id="departure_mode_form" name="mode" required maxlength="100">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                          <label for="flight_train_form">Flight/Train Number</label>
                          <input type="text" class="form-control" id="departure_flight_train_form" name="flight_train" required maxlength="100">
                        </div>
                      </div>
                    </div>

                    <!-- Row 3: City, Departure Date & Departure Time -->
                    <div class="row">
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="city_form">City</label>
                          <input type="text" class="form-control" id="departure_city_form" name="city" required maxlength="100"
                            pattern="^[A-Za-z\s\(\)]+$" title="Only alphabets, spaces, and parentheses are allowed.">
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="departure_form">Departure Date</label>
                          <input type="date" class="form-control" id="departure_form" name="departure" required>
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          <label for="time_form">Departure Time</label>
                          <input type="time" class="form-control" id="departure_time_form" name="time" required>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <label for="comments_form">Comments</label>
                          <textarea class="form-control" id="departure_comments_form" name="comments" maxlength="200"></textarea>
                        </div>
                      </div>
                    </div>
       ...
       <!-- ========== Placard Modal with Rich‐Text Editor ========== -->
<!-- Modal -->
<!-- ——— The Modal (no select inside!) ——— -->
<div
  class="modal fade"
  id="placardModal"
  tabindex="-1"
  role="dialog"
  aria-labelledby="placardModalLabel"
  aria-hidden="true"
>
  <div class="modal-dialog custom-modal-widths" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title" id="placardModalLabel">Placard Editor</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      ...
      <div class="modal-body">
        ...
        <div id="editor" style="height:300px; background:#fff;">
          <h1 style="text-align:center;">PSV Tours &amp; Travels</h1>
          <!-- <h2 style="text-align:center;">Welcomes</h2> -->
         
        </div>
      </div>

      <div class="modal-footer">
        <button type="button" id="printPlacard" class="btn btn-outline-primary">
          🖨️ Print
        </button>
        <button type="button" id="downloadPlacardPdf" class="btn btn-success">
          ⬇️ Download PDF
        </button>
        <button type="button" class="btn btn-secondary" data-dismiss="modal">
          ...
          <div class="modal fade" id="hotelConfirmationModal" tabindex="-1" role="dialog" aria-labelledby="hotelConfirmationModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="hotelConfirmationModalLabel">Hotel Confirmation</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Hotel Confirmation followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <div class="modal fade" id="transportFollowUpModal" tabindex="-1" role="dialog" aria-labelledby="transportFollowUpModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="transportFollowUpModalLabel">Transport Follow Up</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Transport Follow Up followup goes here -->
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="modal fade" id="itineraryModal" tabindex="-1" role="dialog" aria-labelledby="itineraryModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="itineraryModalLabel">Itinerary</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Itinerary followup goes here -->
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="modal fade" id="voucherModal" tabindex="-1" role="dialog" aria-labelledby="voucherModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <!-- Content for Voucher followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <div class="modal fade" id="transportItineraryModal" tabindex="-1" role="dialog" aria-labelledby="transportItineraryModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="transportItineraryModalLabel">Transport Itinerary</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Transport Itinerary followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <div class="modal fade" id="finalItineraryModal" tabindex="-1" role="dialog" aria-labelledby="finalItineraryModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="finalItineraryModalLabel">Final Itinerary</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Final Itinerary followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <div class="modal fade" id="proformaOfficeCopyModal" tabindex="-1" role="dialog" aria-labelledby="proformaOfficeCopyModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="proformaOfficeCopyModalLabel">Proforma (Office Copy)</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Proforma (Office Copy) followup goes here -->
                </div>
              </div>
            </div>
          </div>

          <div class="modal fade" id="proformaGuestCopyModal" tabindex="-1" role="dialog" aria-labelledby="proformaGuestCopyModalLabel" aria-hidden="true">
            <div class="modal-dialog custom-modal-widths" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="proformaGuestCopyModalLabel">Proforma (Guest Copy)</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                  </button>
                </div>
                <div class="modal-body">
                  <!-- Content for Proforma (Guest Copy) followup goes here -->
                </div>
              </div>
            </div>
          </div>
...
<!-- jQuery, Bootstrap JS, DataTables JS, and SweetAlert2 -->
...
<script>
  $(document).ready(function() {
    ...
    function normalizeDT(s) {
      return s ? s.replace('T', ' ').substr(0, 16) : '';
    }
    ...
    var currentObjectId;
    var callFollowUpTable;
    var arrivalFollowUpTable;
    var departureFollowUpTable;

    $('#markupTable').DataTable();

    // ── Open Follow-up Modal ─────────────────────────────────────────
    $(document).on('click', '.open-modal', function() {
      currentObjectId = $(this).data('object_id');
      console.log('Open-followup modal for object', currentObjectId);
      $('#followupModalTitle').text($(this).data('enquiry_name'));
      $('#object_id_form').val(currentObjectId);
      $.ajax({
        url: "<?= site_url('Enwuiryfollowup/getFollowupTypes'); ?>",
        type: "POST",
        data: {
    ...
    // ── Call Follow-ups (common modal) ────────────────────────────────
    $(document).on('click', '.common-modal-btn', function() {
      var followupTypeId = $(this).data('followup-id'),
        followupType = $(this).data('followup');
      $('#commonModalLabel').text(followupType + ' Follow-ups');
      $('#followup_type_id_form').val(followupTypeId);
      $('#commonModal').modal('show');

      if ($.fn.DataTable.isDataTable('#callFollowUpTable'))
        $('#callFollowUpTable').DataTable().destroy();

      callFollowUpTable = $('#callFollowUpTable').DataTable({
        ajax: {
          url: "<?= site_url('Enwuiryfollowup/getCallFollowups'); ?>",
          type: "POST",
          data: {
        ...
        columns: [{
          ...
          },
          {
            ...
            orderable: false,
            render: row => `
            <i class="fas fa-edit text-success edit-btn" data-id="${row.call_follow_up_id}" style="cursor:pointer"></i>
            <i class="fas fa-trash-alt text-success delete-btn ml-2" data-id="${row.call_follow_up_id}" style="cursor:pointer"></i>
          `
          }
        ]
      });
    ...
    // ── Add Call Follow-up ────────────────────────────────────────────
    $('#btnAddFollowUp').on('click', function() {
      $('#addFollowUpModalLabel').text('Add New Follow-up');
      $('#addFollowupForm')[0].reset();
      $('#call_follow_up_id_form').remove();
      $('#addFollowUpModal').modal('show');
    });

    // ── Edit Call Follow-up ───────────────────────────────────────────
    $('#callFollowUpTable tbody').on('click', '.edit-btn', function() {
      var id = $(this).data('id');
      $.ajax({
        url: "<?= site_url('Enwuiryfollowup/getCallFollowupById'); ?>",
        type: "POST",
        data: {
          id: id
        },
        ...
        success: function(data) {
          if (!data.status) {
            $('#call_time_form').val(data.call_time);
            $('#call_status_form').val(data.call_status);
            $('#comments_form').val(data.comments);
            $('#call_follow_up_id_form').remove();
            $('<input>').attr({
                type: 'hidden',
                id: 'call_follow_up_id_form',
                name: 'call_follow_up_id',
                value: data.call_follow_up_id
              })
              .appendTo('#addFollowupForm');
    ...
    // ── Delete Call Follow-up ─────────────────────────────────────────
    $('#callFollowUpTable tbody').on('click', '.delete-btn', function() {
      var id = $(this).data('id');
      Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
          $.post("<?= site_url('Enwuiryfollowup/deleteCallFollowup'); ?>", {
            id: id
          }, function(res) {
            if (res.status === 'success') {
              Swal.fire('Deleted!', 'Follow-up removed.', 'success');
              callFollowUpTable.ajax.reload();
            } else {
    ...
    $('#addFollowupForm').on('submit', function(e) {
      e.preventDefault();
      var isEdit = $('#call_follow_up_id_form').length > 0,
        $time = $('#call_time_form');
      clearFieldError($time);

      if (!isEdit) {
        var newDT = normalizeDT($time.val()),
          existing = callFollowUpTable.column(0).data().toArray().map(normalizeDT);
        if (existing.includes(newDT)) {
          showFieldError($time, 'A call follow-up at that time already exists.');
          return;
        }
      }

      var url = isEdit ?
        "<?= site_url('Enwuiryfollowup/updateCallFollowup'); ?>" :
        "<?= site_url('Enwuiryfollowup/saveCallFollowup'); ?>";
    ...
    // ── ARRIVAL Follow-ups ────────────────────────────────────────────
    $('#arrivalDetailsModal').on('shown.bs.modal', function() {
      var typeId = $(this).data('followup-id') || 2;
      if ($.fn.DataTable.isDataTable('#arrivalFollowUpTable'))
        $('#arrivalFollowUpTable').DataTable().destroy();

      arrivalFollowUpTable = $('#arrivalFollowUpTable').DataTable({
        ajax: {
          url: "<?= site_url('Enwuiryfollowup/getArrivalFollowups'); ?>",
          type: "POST",
          data: {
        ...
        columns: [{
          ...
          },
          {
            ...
            orderable: false,
            render: r => `
            <i class="fas fa-edit text-success editArrival" data-id="${r.arrival_follow_up_id}" style="cursor:pointer"></i>
            <i class="fas fa-trash-alt text-success deleteArrival ml-2" data-id="${r.arrival_follow_up_id}" style="cursor:pointer"></i>
          `
          }
        ]
      });
    ...
    $('#btnArrivalFollowUp').on('click', function() {
      $('#arrivalFollowupForm')[0].reset();
      $('#arrival_follow_up_id').val('');
      $('#addarrivalFollowUpModalLabel').text('Add New Arrival Followup');
      $('#arrival_followup_type_id_form').val(2);
      $('#arrival_object_id_form').val(currentObjectId);
      $('#arrivalFollowUpModal').modal('show');
    });

    $('#arrivalFollowupForm').on('submit', function(e) {
      e.preventDefault();
      var isEdit = $('#arrival_follow_up_id').val() !== '',
        $cd = $('#call_date_form'),
        $ct = $('#arrival_call_time_form'),
        $ad = $('#arrival_form'),
        $at = $('#time_form');
      [$cd, $ct, $ad, $at].forEach(clearFieldError);

      if (!isEdit) {
        var callDT = normalizeDT($cd.val() + 'T' + $ct.val()),
          arrDT = normalizeDT($ad.val() + 'T' + $at.val()),
          dup = arrivalFollowUpTable.rows().data().toArray().some(r =>
            normalizeDT(r.call_date) === callDT && normalizeDT(r.arrival_date) === arrDT
          );
        if (dup) {
          showFieldError($ct, 'An arrival at that date/time already exists.');
          showFieldError($at, 'An arrival at that date/time already exists.');
          return;
        }
      }

      $.post('<?= site_url("Enwuiryfollowup/saveArrivalFollowup"); ?>', $(this).serialize(), function(res) {
        if (res.status === 'success') {
          Swal.fire('Saved!', 'Your arrival follow-up has been saved.', 'success');
          $('#arrivalFollowUpModal').modal('hide');
          arrivalFollowUpTable.ajax.reload();
        } else {
          Swal.fire('Error!', res.message, 'error');
        }
      }, 'json');
    ...
    $(document).on('click', '.editArrival', function() {
      // TODO: Implement editArrival
    });

    $(document).on('click', '.deleteArrival', function() {
      // TODO: Implement deleteArrival
    });

    // ── DEPARTURE Follow-ups ─────────────────────────────────────────
    $('#departureDetailsModal').on('shown.bs.modal', function() {
      var typeId = $(this).data('followup-id') || 3;
      if ($.fn.DataTable.isDataTable('#departureFollowUpTable'))
        $('#departureFollowUpTable').DataTable().destroy();

      departureFollowUpTable = $('#departureFollowUpTable').DataTable({
        ajax: {
          url: "<?= site_url('Enwuiryfollowup/getDepartureFollowups'); ?>",
          type: "POST",
          data: {
            followupTypeId: typeId,
            objectId: currentObjectId
        ...
        columns: [{
          ...
          },
          {
            ...
            orderable: false,
            render: r => `
            <i class="fas fa-edit text-success editDeparture" data-id="${r.departure_follow_up_id}" style="cursor:pointer"></i>
            <i class="fas fa-trash-alt text-success deleteDeparture ml-2" data-id="${r.departure_follow_up_id}" style="cursor:pointer"></i>
          `
          }
        ]
      });
    ...
    // whenever the Add Call-FollowUp modal is about to show…
    $('#addFollowUpModal').on('show.bs.modal', function() {
      // clear every field error in that form
      $('#addFollowupForm .is-invalid').removeClass('is-invalid');
      $('#addFollowupForm .invalid-feedback').remove();
    });

    // same for Arrival modal
    $('#arrivalFollowUpModal').on('show.bs.modal', function() {
      $('#arrivalFollowupForm .is-invalid').removeClass('is-invalid');
      $('#arrivalFollowupForm .invalid-feedback').remove();
    });

    // same for Departure modal
    $('#departureFollowUpModal').on('show.bs.modal', function() {
      $('#departureFollowupForm .is-invalid').removeClass('is-invalid');
      $('#departureFollowupForm .invalid-feedback').remove();
    });
...
<script>
  ...
  // Build/update the placard content in Quill
  function updatePlacard() {
    if (!quill) return;
    quill.setContents([
      { insert: 'PSV Tours & Travels\n', attributes: { header: 1, align: 'center' } },
      { insert: 'Welcomes\n',            attributes: { header: 2, align: 'center' } },
      { insert: currentGuestName + '\n', attributes: { header: 2, align: 'center' } }
    ]);
    document.getElementById('followupModalTitle').innerText = currentGuestName;
  }

  // When the modal is shown, grab the guest name from the triggering element
  $('#placardModal').on('show.bs.modal', function (e) {
  // e.relatedTarget is the element (your "Placard" button) that opened the placardModal
  const triggerBtn = $(e.relatedTarget);

  // Find the closest ancestor modal, then grab its .modal-header .modal-title text
  const sourceModal = triggerBtn.closest('.modal');
  const guestName = sourceModal.find('.modal-header .modal-title').first().text().trim() || 'Guest';
  ...
  // Helpers for print & PDF (unchanged)
  function getPlacardHTML() {
    return `
      <div id="placardContent"
           style="border:1px solid #000; padding:2rem; text-align:center;">
        ${quill.root.innerHTML}
      </div>
    `;
  }

  document.getElementById('printPlacard').addEventListener('click', () => {
    const content = getPlacardHTML();
    const w = window.open('', '', 'width=800,height=600');
    w.document.write(`
      <html><head><title>Print Placard</title>
        <style>
          body { display:flex; justify-content:center; align-items:center;
                 height:100vh; margin:0; }
          #placardContent { border:1px solid #000; padding:2rem; text-align:center; }
        </style>
...
// DOWNLOAD PDF – fill entire A4 page
document.getElementById('downloadPlacardPdf').addEventListener('click', async () => {
  const { jsPDF } = window.jspdf;

  // 1) Render your placard into an off-screen DIV
  const tmp = document.createElement('div');
  tmp.innerHTML = getPlacardHTML();
  document.body.appendChild(tmp);
  const canvas = await html2canvas(tmp.querySelector('#placardContent'), { scale: 2 });
  document.body.removeChild(tmp);

  // 2) Convert to PNG data URL
  const imgData = canvas.toDataURL('image/png');

  // 3) Create a new A4 PDF (units in points)
  //    Page size: 595.28 pt × 841.89 pt
  const pdf = new jsPDF({ unit: 'pt', format: 'a4', orientation: 'portrait' });
  const pageWidth  = pdf.internal.pageSize.getWidth();
  const pageHeight = pdf.internal.pageSize.getHeight();

  // 4) Draw the image at 0,0 stretched to fill the page
  pdf.addImage(imgData, 'PNG', 0, 0, pageWidth, pageHeight);

  // 5) Save!
  pdf.save('placard.pdf');
});
</script>