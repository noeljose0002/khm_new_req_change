            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Source Report</h4>
                ...
                <body class="p-4">
                    ...
                    <div id="filterContainer">
                        <!-- <center><h4 class="mb-4">Source Report</h4></center> -->
                        <div class="card">
                            <div class="card-body">
                                <div class="form-inline filter-row align-items-center">
                                    <input type="hidden" id="system_id" value="<?= session('system_id') ?>">
                                    <div class="form-group">
                                        <label for="from_date" class="mr-2">From:</label>
                                        <input type="date" id="from_date" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label for="to_date" class="ml-3 mr-2">To:</label>
                                        <input type="date" id="to_date" class="form-control">
                                    </div>

                                    <!-- NEW: Source filter dropdown -->
                                    <div class="form-group ml-3">
                                        <label for="source_filter" class="mr-2">Source:</label>
                        ...
                        <div id="tableContainer">
                            <div class="card">
                                <div class="card-body p-4">
                                    <table id="SourceReportTable" style="width: 100%;">
                                        <thead>
                                            <tr>
...
<script>
    ...
    $(function() {
        ...
        $.fn.dataTable.ext.search.push(function(settings, data, dataIndex, rowData) {
            var from = $('#from_date').val(),
                to = $('#to_date').val(),
                srcF = $('#source_filter').val();

            // data[2] = Enquiry Date (DD‑MM‑YYYY)
            var d = data[2] ? moment(data[2], 'DD-MM-YYYY') : null;
            if (from && (!d || d.isBefore(moment(from)))) return false;
            if (to && (!d || d.isAfter(moment(to)))) return false;

            // data[3] = raw source code
            // if (srcF && data[3] !== srcF) return false;
            if (srcF && String(rowData.enquiry_source) !== srcF) return false;
            return true;
        }); // end custom filter :contentReference[oaicite:0]{index=0}
        ...
        var table = $('#SourceReportTable').DataTable({
            ...
            buttons: [{
                ...
                title: 'Source Report',
                filename: 'source_report_' + moment().format('YYYYMMDD_HHmmss'),
                exportOptions: {
                    columns: ':visible',
                    format: {
                        body: function(data, row, column, node) {
                            // only for the “Source” column index (e.g. column===3)
                            if (column === 3) {
                                var sourceMap = {
                                    1: 'Email',
                                    2: 'Reference',
            ...
            serverSide: false,
            ajax: {
                url: '<?= site_url("SourceReport/data") ?>',
                data: function(d) {
  ...
  render: function(data, type, row) {
    ...
    // Always treat source 2 as “Reference” for filter/sort/export
    if (row.enquiry_source == 2 && (type === 'filter' || type === 'sort' || type === 'export')) {
      return sourceMap[2];
    }

    // For display only, show source_reference if present; otherwise fall back to “Reference”
    if (row.enquiry_source == 2 && type === 'display') {
    ...
    // Default: return raw numeric value (so DataTables still knows it’s a “2” under the hood)
    return data;
  }
}
,
