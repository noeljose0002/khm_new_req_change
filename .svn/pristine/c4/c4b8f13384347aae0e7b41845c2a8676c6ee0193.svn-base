      ...
      <div class="app-content icon-content">
        <div class="section">
          ...
          <div class="page-header">
            <div class="page-leftheader">
              <h4 class="page-title mb-0">TAX RATE DEFINITION</h4>
        ...
        <head>
          ...
          <style>
            ...
            /* Action icon styling: default green color */
            .action-icon {
              color: green;
              cursor: pointer;
              margin-right: 10px;
        ...
        <body class="bg-light">
          <div class="container">
            ...
            <div class="card shadow-sm mb-4">
              <div class="card-header">
                <h4>Tax Rate Definition</h4>
              </div>
              <div class="card-body">
                <!-- Tax Form -->
                <form id="taxForm" class="mb-3" novalidate>
                  <!-- Hidden tax id field -->
                  <input type="hidden" name="tax_id" id="tax_id">
                  <!-- Hidden enterprise field -->
                  <input type="hidden" name="enterprise_id" id="enterprise_id" value="1">
                  <div class="form-group">
                    <label for="tax_name" class="font-weight-bold">Tax Name:</label>
                    <!-- Pattern allows letters, numbers, spaces, hyphens, percent signs and parentheses -->
                    <input type="text" class="form-control" name="tax_name" id="tax_name"
                      placeholder="Enter tax name" required maxlength="100"
                      pattern="^[A-Za-z0-9\s\-\%\(\)]+$"
                      title="Tax name can include letters, numbers, spaces, hyphens (-), percent signs (%) and parentheses. e.g. GST - 18% (Special Package)"
                      autocomplete="off" spellcheck="false">
                    <div class="invalid-feedback">
                      Please enter a valid tax name (e.g. GST - 18% (Special Package)).
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="tax" class="font-weight-bold">Tax Value (%):</label>
                    <!-- Validate tax value between 0 and 100 percent with up to 2 decimals -->
                    <input type="number" step="0.01" min="0" max="100" class="form-control" name="tax" id="tax" placeholder="Enter tax value" required>
                    <div class="invalid-feedback">
                      Please enter a valid tax value between 0% and 100%.
                    </div>
                  </div>

                  <!-- Hidden inputs for checkboxes to ensure a default value of 0 is sent if unchecked -->
                  <input type="hidden" name="b2b" value="0">
                  <div class="form-group form-check">
                    <input type="checkbox" class="form-check-input" name="b2b" id="b2b" value="1">
                    <label class="form-check-label" for="b2b">B2B</label>
                  </div>
                  <input type="hidden" name="b2c" value="0">
                  <div class="form-group form-check">
                    <input type="checkbox" class="form-check-input" name="b2c" id="b2c" value="1">
                    <label class="form-check-label" for="b2c">B2C</label>
                  </div>
                  <button type="submit" id="taxSubmitBtn" class="btn btn-success">Save </button>
                  <button type="button" id="taxClearBtn" class="btn btn-success">Clear</button>
                  <button type="button" id="taxCancelBtn" class="btn btn-success" style="display: none;">Cancel</button>
                </form>
                <hr>
                <!-- Tax DataTable -->
                <div class="table-responsive">
                  <table id="taxTable" class="table table-striped table-bordered" style="width:100%;">
...
<script>
  $(document).ready(function() {
    // Initialize the Tax DataTable using the fetchAll endpoint
    var taxTable = $('#taxTable').DataTable({
      processing: true,
      ajax: {
        url: "<?= site_url('Tax/fetchAll') ?>",
        type: "GET",
        dataSrc: ''
      },
      columns: [{
        ...
        },
        {
          ...
          render: function(data, type, row) {
            return '<i class="fas fa-edit action-icon editTax" data-id="' + row.tax_id + '" title="Edit"></i>' +
              '<i class="fas fa-trash-alt action-icon deleteTax" data-id="' + row.tax_id + '" title="Delete"></i>';
          }
        }
      ]
    });
    ...
    function clearTaxForm() {
      $('#taxForm')[0].reset();
      $('#tax_id').val('');
      $('#taxSubmitBtn').text('Save');
      $('#taxCancelBtn').hide();
      $('#taxForm').removeClass('was-validated');
    }

    // Form submission for adding/updating tax
    $('#taxForm').on('submit', function(e) {
      ...
      var enteredTaxName = $('#tax_name').val().trim().toLowerCase();
      var currentTaxId = $('#tax_id').val();
      var duplicateFound = false;

      // Loop through existing data in the DataTable to check for duplicate tax names
      taxTable.rows().data().each(function(value) {
        if (value.tax_name.trim().toLowerCase() === enteredTaxName) {
          if (currentTaxId && value.tax_id == currentTaxId) {
            return;
          }
          duplicateFound = true;
        }
      ...
      if (duplicateFound) {
        Swal.fire('Duplicate Entry', 'A tax record with this name already exists!', 'error');
        return false;
      }

      var formData = $(this).serialize();
      var url = currentTaxId ? "<?= site_url('Tax/update') ?>/" + currentTaxId : "<?= site_url('Tax/store') ?>";
      $.ajax({
        url: url,
        type: "POST",
        data: formData,
    ...
    // Clear button event
    $('#taxClearBtn').click(function() {
      clearTaxForm();
    });

    // Cancel update event
    $('#taxCancelBtn').click(function() {
      clearTaxForm();
    });

    // Edit tax event: fetch and populate the form using the fetchAll endpoint
    $('#taxTable').on('click', '.editTax', function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          var record = data.find(item => item.tax_id == taxId);
          if (record) {
            $('#tax_id').val(record.tax_id);
            $('#tax_name').val(record.tax_name);
            $('#tax').val(record.tax);
            $('#b2b').prop('checked', record.b2b == 1);
            $('#b2c').prop('checked', record.b2c == 1);
            $('#taxSubmitBtn').text('Update');
            $('#taxCancelBtn').show();
            $('html, body').animate({
              scrollTop: $('#taxForm').offset().top - 200
            }, 'fast');
    ...
    // Delete tax event
    $('#taxTable').on('click', '.deleteTax', function() {
      var taxId = $(this).data('id');
      Swal.fire({
        title: 'Are you sure?',
        text: "This will delete the tax record.",
        icon: 'warning',
