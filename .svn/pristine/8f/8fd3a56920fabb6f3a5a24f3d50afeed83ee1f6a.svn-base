                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">WhatsApp Add / Edit Template</h4>
...
<head>
  <meta charset="UTF-8">
  <title>WhatsApp Text Message Templates</title>
  <!-- Bootstrap 4.1.3 CSS -->
  ...
  <style>
    ...
    /* Custom icon styling for actions */
    .action-icon {
      cursor: pointer;
      margin-right: 10px;
      font-size: 1.2em;
...
<div class="container mt-5">
  <!-- Form Card (Static Form for Add/Edit) -->
  <div class="card">
    <div class="card-header form-header">
      <span><i class="fas fa-edit"></i> Add / Edit Template</span>
    </div>
    <div class="card-body">
      <form id="whatsappForm">
        <!-- Hidden fields for update detection and enterprise id -->
        <input type="hidden" name="whatsapp_text_message_id" id="whatsapp_text_message_id">
        <input type="hidden" name="enterprise_id" id="enterprise_id" value="1">
        <div class="form-group">
          <label for="template_name">Template Name</label>
          <input type="text" class="form-control" id="template_name" name="template_name" minlength="2" maxlength="100" required>
        </div>
        <div class="form-group">
          <label for="template_content">Template Content</label>
          <textarea class="form-control" id="template_content" name="template_content" rows="4" minlength="2" maxlength="500"></textarea>
        </div>
  ...
  <!-- Table Card (DataTable listing all templates) -->
  <div class="card">
    <div class="card-header table-header d-flex justify-content-between align-items-center">
      <span><i class="fas fa-comments"></i> WhatsApp Templates</span>
    </div>
    <div class="card-body">
...
<script>
  $(document).ready(function() {
    let table = $('#whatsappTable').DataTable({
      processing: true,
      serverSide: false,
      ajax: {
        url: "<?= site_url('WhatsappTextMessageController/fetch') ?>",
        type: "POST",
        dataSrc: ""
      },
      columns: [
        { data: 'whatsapp_text_message_id', visible: false }, // Hide ID column
        { data: 'template_name' },
        { data: 'template_content' },
        { data: 'enterprise_id', visible: false }, // Keep Enterprise ID hidden
        {
          data: null,
          render: function(data, type, row) {
            return `
              <i class="fas fa-edit text-success action-icon editIcon"
                 data-id="${row.whatsapp_text_message_id}"
                 data-name="${row.template_name}"
                 data-content="${row.template_content}"
                 data-enterprise="${row.enterprise_id}"
                 title="Edit"></i>
              <i class="fas fa-trash text-success action-icon deleteIcon"
                 data-id="${row.whatsapp_text_message_id}"
                 title="Delete"></i>
            `;
          }
    ...
    // Reset form function to clear and reset button texts
    function resetForm() {
      $('#whatsappForm')[0].reset();
      $('#whatsapp_text_message_id').val('');
      $('#submitBtn').html('<i class="fas fa-save"></i> Save');
      $('#clearBtn').html('<i class="fas fa-times"></i> Clear');
    }

    // Edit Icon click event
    $('#whatsappTable').on('click', '.editIcon', function() {
      let id = $(this).data('id');
      let name = $(this).data('name');
      let content = $(this).data('content');
      let enterprise = $(this).data('enterprise');

      $('#whatsapp_text_message_id').val(id);
      $('#template_name').val(name);
      $('#template_content').val(content);
      $('#enterprise_id').val(enterprise);
      $('#submitBtn').html(' Update');
      $('#clearBtn').html(' Cancel');

      // Scroll to form
      $('html, body').animate({
        scrollTop: $("#whatsappForm").offset().top - 100
      }, 500);
    });

    // Delete Icon click event
    $('#whatsappTable').on('click', '.deleteIcon', function() {
      let row = $(this).closest('tr');
      let data = table.row(row).data();
      let id = data.whatsapp_text_message_id;
      Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "<?= site_url('WhatsappTextMessageController/delete') ?>",
            type: "POST",
            data: { whatsapp_text_message_id: id },
            dataType: "json",
            success: function(response) {
    ...
    // Clear/Cancel button behavior
    $('#clearBtn').on('click', function() {
      resetForm();
    });

    // Save (Add/Update) record with duplicate validation
    $('#whatsappForm').on('submit', function(e) {
      e.preventDefault();
      let nameVal = $('#template_name').val().trim();
      let contentVal = $('#template_content').val().trim();
      ...
      // Name validation regex: starts with an uppercase letter followed by lowercase letters,
      // with any subsequent parts (separated by a space, hyphen, or apostrophe) also capitalized.
      let nameRegex = /^[A-Za-z0-9]+(?:[ '-][A-Za-z0-9]+)*$/;
  
      if (!nameRegex.test(nameVal)) {
        Swal.fire('Warning', 'Invalid Template Name. Ensure it starts with a letter/number and contains only letters, numbers, spaces, hyphens, or apostrophes.', 'warning');
        return;
      }
      
      // Duplicate validation: check if the template name already exists (case insensitive)
      let isDuplicate = false;
      let currentId = $('#whatsapp_text_message_id').val();
      table.rows().every(function() {
        let d = this.data();
        if (d.template_name.toLowerCase() === nameVal.toLowerCase()) {
          if (!currentId || (currentId && d.whatsapp_text_message_id != currentId)) {
            isDuplicate = true;
          }
        }
      });
      ...
      if (isDuplicate) {
        Swal.fire('Warning', 'Duplicate Template Name found!', 'warning');
        return;
      }
      
      $.ajax({
        url: "<?= site_url('WhatsappTextMessageController/save') ?>",
        type: "POST",
        data: $(this).serialize(),
        dataType: "json",
        success: function(response) {
