...
');
    ...
    //njnjnjenqdetails //
      public function get_tour_plan_details29($enquiry_header_id)
    {
        $db = \Config\Database::connect();
        $response = [];
        $selected_table = $db->table('khm_obj_enquiry_tour_details a');
...
//    public function get_tour_plan_details2($enquiry_header_id, $edit_id)
// {
//     $db = \Config\Database::connect();
//     $response = [];
//     $selected_table = $db->table('khm_obj_enquiry_tour_details a');
...
//     // If editing, get previous version (is_prev = 1), otherwise get current version
//     if ($edit_id) {
//         $builder->where('a.enquiry_details_id', $edit_id);
//     }
    
//     $builder->orderBy('a.location_sequence', 'ASC')
//         ->limit(1);
...
public function get_tour_plan_details2($enquiry_header_id, $edit_id, $current_enquiry_details_id = null) 
{ 
    $db = \Config\Database::connect(); 
    $response = []; 
    $selected_table = $db->table('khm_obj_enquiry_tour_details a'); 
    ...
    // If editing, get previous version based on enquiry_details_id comparison
    if ($edit_id) { 
        $builder->where('a.enquiry_details_id', $edit_id);
        
        // Check if edit_id is same as current enquiry_details_id
        if ($current_enquiry_details_id && $edit_id == $current_enquiry_details_id) {
            $builder->where('a.is_active', 1);
        } else {
            $builder->where('a.is_active', 0);
        }
    } else {
        // When not editing, get active records
        $builder->where('a.is_active', 1);
    }
