    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Entity Attribute Assign </title>
                    <!-- Bootstrap 4 CSS -->
                                ...
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-5">
                                            <label for="entity_class_id">Class Name</label>
                                            <select class="form-control" name="entity_class_id" id="entity_class_id">
                                                <option value="">-- Select Class --</option>
                                            </select>
                                        </div>

                                           <div class="form-group col-md-5">
                                            <label for="entity_attribute_id">Attribute Name</label>
                                            <select class="form-control" name="entity_attribute_id" id="entity_attribute_id">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            ...
                            <div class="card-body">
                                <form id="editForm">
                                    ...
                                    <input type="hidden" id="edit_entity_class_attr_id" name="entity_class_attr_id">

                                    <div class="form-row">

                                     <div class="form-group col-md-5">
                                            <label for="edit_entity_class_id">Class Name</label>
                                            <select class="form-control" name="entity_class_id" id="edit_entity_class_id" data-live-search="true">
                                                <option value="">-- Select Class --</option>
                                            </select>
                                        </div>

                                        <div class="form-group col-md-5">
                                            <label for="edit_entity_attribute_id">Attribute Name</label>
                                            <select class="form-control" name="entity_attribute_id" id="edit_entity_attribute_id" data-live-search="true">
                                                <option value="">-- Select Attribute --</option>
                                            </select>
                                        </div>
...
<script>
    $(document).ready(function() {
        ...
        $.getJSON('<?= site_url("SuperAdminEntityAttributeClassAssign/list2") ?>',function(res){
            allClass = res.data;

            const classopts = (sel) => {
                let h =  `<option value="">-- Select Class --</option>`;
                allClass.forEach(o=>{
                    const s = (''+o.entity_class_id === ''+sel) ? ' selected' : '';
                    h += `<option value="${o.entity_class_id}"${s}>${o.entity_class_name}</option>`;
                });
                return h;


            };
            $('#entity_class_id').html(classopts(null));
			// inject into EDIT form
			$('#edit_entity_class_id').html(classopts(null));

        });
         let allAttri = [];

         $.getJSON('<?= site_url("SuperAdminEntityAttributeClassAssign/list3") ?>',function(res){
            allAttri = res.data;

            const classopts = (sel) => {
                let f =  `<option value="">-- Select Attribute --</option>`;
                allAttri.forEach(o=>{
                    const s = (''+o.entity_attribute_id  === ''+sel) ? ' selected' : '';
                    f += `<option value="${o.entity_attribute_id }"${s}>${o.entity_attribute_name}</option>`;
                });
                return f;


            };
            $('#entity_attribute_id').html(classopts(null));
			// inject into EDIT form
			$('#edit_entity_attribute_id').html(classopts(null));
        ...
        const table = $('#entityattriassignTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminEntityAttributeClassAssign/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'entity_class_attr_id',
                    title: 'ID'
                },
                {
                    ...
                    title:'Class Name',
                    // render:cid =>{
                    //     const cls = allClass.find(c=>'+c.entity_class_id'=== '' + cid)
                    //     return cls ? cls.entity_class_id : '';
                    // }
                },
            
                {
                    data: 'entity_attribute_name',
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post('<?= site_url("SuperAdminEntityAttributeClassAssign/create") ?>', $(this).serialize())
                .done(() => {
                    $('#addForm')[0].reset();
                    table.ajax.reload();
        ...
        $('#entityattriassignTable').on('click', '.editBtn', function() {
            ...
            $.getJSON(`<?= site_url("SuperAdminEntityAttributeClassAssign/edit") ?>/${id}`, function(data) {
                // fill hidden ID
                $('#edit_entity_class_attr_id').val(data.entity_class_attr_id );
               
               
                // fill other fields
                $('#edit_entity_attribute_id').val(data.entity_attribute_id );
                
                $('#edit_enterprise_id').val(data.enterprise_id);

                let ch = `<option value="">-- Select Class --</option>`;
				allClass.forEach(o => {
					const s = ('' + o.entity_class_id === '' + data.entity_class_id) ? ' selected' : '';
					ch += `<option value="${o.entity_class_id}"${s}>${o.entity_class_name}</option>`;
				});

                $('#edit_entity_class_id ').html(ch);

                 let ah = `<option value="">-- Select Attribute --</option>`;
				allAttri.forEach(o => {
					const s = ('' + o.entity_attribute_id === '' + data.entity_attribute_id) ? ' selected' : '';
					ah += `<option value="${o.entity_attribute_id}"${s}>${o.entity_attribute_name}</option>`;
				});

                $('#edit_entity_attribute_id').html(ah);
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            const id = $('#edit_entity_class_attr_id').val();
            $.ajax({
                    url: `<?= site_url("SuperAdminEntityAttributeClassAssign/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .done(() => {
        ...
        $('#entityattriassignTable').on('click', '.deleteBtn', function() {
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminEntityAttributeClassAssign/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(response => {
