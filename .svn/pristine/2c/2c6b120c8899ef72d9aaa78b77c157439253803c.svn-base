    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Object Attributr Assign</h4>
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Object Attribute Assign </title>
                    <!-- Bootstrap 4 CSS -->
                    ...
                    <style>
                        ...
                        #entityattriassignTable thead th {
                ...
                <body>
                    <div class="container mt-5">
                                ...
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-5">
                                            <label for="object_class_id">Class Name</label>
                                            <select class="form-control" name="object_class_id" id="object_class_id">
                                                <option value="">-- Select Class --</option>
                                            </select>
                                        </div>

                                           <div class="form-group col-md-5">
                                            <label for="object_mst_attribute_id">Attribute Name</label>
                                            <select class="form-control" name="object_mst_attribute_id" id="object_mst_attribute_id">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            <div class="card-header form-header">
                                <h4>Edit Table Of Table</h4>
                            </div>
                            <div class="card-body">
                                <form id="editForm">
                                    <input type="hidden" name="_method" value="PUT">
                                    <input type="hidden" id="edit_object_class_attribute_id" name="object_class_attribute_id">

                                    <div class="form-row">

                                     <div class="form-group col-md-5">
                                            <label for="edit_object_class_id">Class Name</label>
                                            <select class="form-control" name="object_class_id" id="edit_object_class_id" data-live-search="true">
                                                <option value="">-- Select Class --</option>
                                            </select>
                                        </div>

                                        <div class="form-group col-md-5">
                                            <label for="edit_object_mst_attribute_id ">Attribute Name</label>
                                            <select class="form-control" name="object_mst_attribute_id" id="edit_object_mst_attribute_id" data-live-search="true">
                                                <option value="">-- Select Attribute --</option>
                                            </select>
                                        </div>
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <table id="entityattriassignTable" class="table table-bordered">
                                    <thead></thead>
                                    <tbody></tbody>
                                </table>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $.getJSON('<?= site_url("SuperAdminObjectAttributeClassAssign/list2") ?>',function(res){
            allClass = res.data;
         ...
         $.getJSON('<?= site_url("SuperAdminObjectAttributeClassAssign/list3") ?>',function(res){
            allAttri = res.data;

            const classopts = (sel) => {
                let f =  `<option value="">-- Select Attribute --</option>`;
                allAttri.forEach(o=>{
                    const s = (''+o.object_mst_attribute_id   === ''+sel) ? ' selected' : '';
                    f += `<option value="${o.object_mst_attribute_id  }"${s}>${o.object_mst_attribute_name}</option>`;
                });
                return f;


            };
            $('#object_mst_attribute_id ').html(classopts(null));
			// inject into EDIT form
			$('#edit_object_mst_attribute_id ').html(classopts(null));
        ...
        const table = $('#entityattriassignTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminObjectAttributeClassAssign/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'object_class_attribute_id',
                    title: 'ID'
                },
                {
                    data:'object_class_name',
                    title:'Class Name',
                    // render:cid =>{
                    //     const cls = allClass.find(c=>'+c.object_class_id'=== '' + cid)
                    //     return cls ? cls.object_class_id : '';
                    // }
                },
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post('<?= site_url("SuperAdminObjectAttributeClassAssign/create") ?>', $(this).serialize())
                .done(() => {
                    $('#addForm')[0].reset();
                    table.ajax.reload();
                    Swal.fire('Saved!', 'assigned attribute .', 'success');
                })
                .fail(xhr => Swal.fire('Error', xhr.responseJSON?.error || 'Failed', 'error'));
        });

        // 6) SHOW EDIT
        $('#entityattriassignTable').on('click', '.editBtn', function() {
            const id = $(this).data('id');
            $.getJSON(`<?= site_url("SuperAdminObjectAttributeClassAssign/edit") ?>/${id}`, function(data) {
                // fill hidden ID
                $('#edit_object_class_attribute_id ').val(data.object_class_attribute_id  );
				...
				allClass.forEach(o => {
					...
					ch += `<option value="${o.object_class_id}"${s}>${o.object_class_name}</option>`;
				});

                $('#edit_object_class_id ').html(ch);

                 let ah = `<option value="">-- Select Attribute --</option>`;
				allAttri.forEach(o => {
					const s = ('' + o.object_mst_attribute_id  === '' + data.object_mst_attribute_id ) ? ' selected' : '';
					ah += `<option value="${o.object_mst_attribute_id }"${s}>${o.object_mst_attribute_name}</option>`;
				});

                $('#edit_object_mst_attribute_id ').html(ah);
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            const id = $('#edit_object_class_attribute_id ').val();
            $.ajax({
                    url: `<?= site_url("SuperAdminObjectAttributeClassAssign/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .done(() => {
        ...
        // 9) DELETE
        $('#entityattriassignTable').on('click', '.deleteBtn', function() {
            const id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminObjectAttributeClassAssign/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(response => {
