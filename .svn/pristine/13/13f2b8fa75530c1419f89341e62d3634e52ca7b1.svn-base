            ...
            <div class="app-content icon-content">
                ...
                <body>
                    <div class="container mt-5">
                        ...
                        <!-- Add Process Type Section -->
                        <div id="addSection" class="card">
                            <div class="card-header form-header">
                                <h4>Add New Process/Transaction Type</h4>
                            </div>
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        <!-- now each takes up 6 of 12 columns -->
                                        <div class="form-group col-12 col-md-6">
                                            <label for="prs_name">Name</label>
                                            <input type="text" class="form-control" id="prs_name" name="prs_name" maxlength="30" required>
                                        </div>
                                        <div class="form-group col-12 col-md-6">
                        ...
                        <!-- Edit Process Type Section -->
                        <div id="editSection" class="card mt-4" style="display:none;">
                            <div class="card-header form-header">
                                <h4>Edit Process/Transaction Type</h4>
                            </div>
                            <div class="card-body">
                                <form id="editForm">
                                    <input type="hidden" id="edit_prs_type_id" name="prs_type_id">
                                    <div class="form-row">
                                        <div class="form-group col-12 col-md-6">
                                            <label for="edit_prs_name">Name</label>
                                            <input type="text" class="form-control" id="edit_prs_name" name="prs_name" maxlength="30" required>
                                        </div>
                                        <div class="form-group col-12 col-md-6">
                    ...
                    <!-- Process Types List Table -->
                    <div class="card">
                        <div class="card-header">
                            <h4>Process/TransactionTypes List</h4>
                        </div>
                        <div class="card-body">
                            <table id="typeTable" class="table table-bordered">
                                ...
                                <tbody><!-- populated via AJAX --></tbody>
                            </table>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        // 1) Fetch all process‐types via AJAX
        $.getJSON('<?= site_url("SuperAdminTransactionProcessType/list") ?>', function(response) {
            ...
            function buildOptions(selectedId) {
                ...
                allTypes.forEach(r => {
                    let sel = r.prs_type_id == selectedId ? ' selected' : '';
                    html += `<option value="${r.prs_type_id}"${sel}>${r.prs_name}</option>`;
                });
                return html;
            }
            $('#parent_id, #edit_parent_id').html(buildOptions());
        });

        // 2) Initialize DataTable once the list endpoint is ready
        let table = $('#typeTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminTransactionProcessType/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: null,
                    render: (t, type, row, meta) => meta.row + 1
                }, // No
                {
                    data: 'prs_name'
                }, // Name
                ...
                }, // Enterprise ID
                {
                    ...
                    orderable: false,
                    render: id => `
                  <i class="fas fa-edit action-icon editBtn" data-id="${id}" title="Edit"></i>
                  <i class="fas fa-trash-alt action-icon deleteBtn" data-id="${id}" title="Delete"></i>`
                }
            ],
            order: [
                [0, 'asc']
            ...
            language: {
                processing: 'Loading…',
                zeroRecords: 'No process types found'
            }
        });
        ...
        function isDuplicate(name, currentId = null) {
            name = name.trim().toLowerCase();
            return allTypes.some(r =>
                r.prs_name.trim().toLowerCase() === name &&
                (currentId == null || r.prs_type_id != currentId)
            );
        }
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            const name = $('#prs_name').val();
            if (!name.trim()) {
                return Swal.fire('Validation', 'Name can’t be blank', 'warning');
            }
            if (isDuplicate(name)) {
                return Swal.fire('Validation', 'That name already exists', 'warning');
            }
            $.post('<?= site_url("SuperAdminTransactionProcessType/create") ?>', $(this).serialize(), () => {
                $('#addForm')[0].reset();
                table.ajax.reload();
                Swal.fire('Saved!', 'New process type added.', 'success');
                // refresh the local array for duplicate checks
                $.getJSON('<?= site_url("SuperAdminTransactionProcessType/list") ?>', res => allTypes = res.data);
            }).fail(xhr => {
                Swal.fire('Error', xhr.responseJSON?.errors?.prs_name || 'Failed to save', 'error');
            });
        });

        // 6) SHOW EDIT
        $('#typeTable').on('click', '.editBtn', function() {
            let id = $(this).data('id');
            $.getJSON(`<?= site_url("SuperAdminTransactionProcessType/edit") ?>/${id}`, function(data) {
                $('#edit_prs_type_id').val(data.prs_type_id);
                $('#edit_prs_name').val(data.prs_name);
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            const name = $('#edit_prs_name').val();
            let id = parseInt($('#edit_prs_type_id').val(), 10);
            ...
            $.ajax({
                url: `<?= site_url("SuperAdminTransactionProcessType/update") ?>/${id}`,
                type: 'PUT',
                data: $(this).serialize(),
            }).done(() => {
                $('#editForm')[0].reset();
                ...
                $('#addSection').show();
                table.ajax.reload();
                Swal.fire('Updated!', 'Process type updated.', 'success');
                $.getJSON('<?= site_url("SuperAdminTransactionProcessType/list") ?>', res => allTypes = res.data);
            }).fail(xhr => {
                Swal.fire('Error', xhr.responseJSON?.errors?.prs_name || 'Failed to update', 'error');
            });
        });

        // 9) DELETE
        $('#typeTable').on('click', '.deleteBtn', function() {
            let id = $(this).data('id');
            Swal.fire({
                title: 'Delete this process type?',
                ...
                if (res.isConfirmed) {
                    $.ajax({
                        url: `<?= site_url("SuperAdminTransactionProcessType/delete") ?>/${id}`,
                        type: 'DELETE'
                    }).done(() => {
                        table.ajax.reload();
                        Swal.fire('Deleted!', 'Process type removed.', 'success');
                        $.getJSON('<?= site_url("SuperAdminTransactionProcessType/list") ?>', res => allTypes = res.data);
                    }).fail(() => {
