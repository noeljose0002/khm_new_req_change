                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">WhatsApp Document Template</h4>
...
<head>
  <meta charset="UTF-8">
  <title>WhatsApp Document Templates</title>
  ...
  <style>
    ...
    /* Style for the DataTable thumbnail images */
    .dt-image {
      max-width: 80px;
      cursor: pointer;
      border: 1px solid #ddd;
...
<body>
  <div class="container mt-5">
    ...
    <div class="card">
      <div class="card-header">
        <h5>WhatsApp Document Template Form</h5>
      </div>
      <div class="card-body">
        <form id="documentForm" enctype="multipart/form-data">
          <!-- Hidden field for update detection -->
          <input type="hidden" name="whatsapp_document_id" id="whatsapp_document_id">
          <!-- Hidden Enterprise ID -->
          ...
          <div class="form-group">
            ...
            <small class="form-text text-muted">Maximum file size: 1MB. Any file type is allowed.</small>
            <!-- Preview container for image file -->
            <div id="previewContainer">
              <p class="mt-2">Image Preview (Click image to enlarge):</p>
              <img id="previewImage" src="#" alt="Image Preview">
            </div>
          </div>
          <div class="form-group">
            <label for="template_path">Template Path</label>
            <!-- Readonly field automatically populated with selected file name or full path -->
            <input type="text" class="form-control" id="template_path" name="template_path" readonly required>
          </div>
          <!-- Form Buttons -->
          <div class="form-group form-buttons">
            ...
            <button type="button" class="btn btn-success" id="clearBtn">
             Clear
            </button>
          </div>
        </form>
        ...
        <table id="documentTable" class="table table-bordered table-striped">
          <thead>
            <tr>
              ...
              <!-- Hidden Enterprise ID -->
              <th style="display: none;">Enterprise ID</th>
              <th>Actions</th>
            </tr>
          </thead>
          <tbody></tbody>
...
<script>
  ...
  <!-- jQuery 3.2.1, Bootstrap 4.1.3, DataTables, and SweetAlert2 -->
  ...
  <script>
    // Set your upload folder path (adjust as needed)
    const BASE_URL = "<?= base_url() ?>";
    var uploadFolder = BASE_URL + "uploads/";


    // Helper function to check if filename is an image (by extension)
    function isImageFile(filename) {
      var ext = filename.split('.').pop().toLowerCase();
      return ['jpg', 'jpeg', 'png', 'gif', 'bmp'].includes(ext);
    }
    // Validate image URL existence (basic client-side check)
    function validateImageURL(url, successCallback, errorCallback) {
      var img = new Image();
    ...
    $(document).ready(function() {
      ...
      var table = $('#documentTable').DataTable({
        ...
        serverSide: false,
        ajax: {
          url: "<?= site_url('WhatsappDocumentController/fetch') ?>",
          type: "POST",
          dataSrc: ""
        },
        columns: [{
            data: 'whatsapp_document_id',
            visible: false
          },
          {
            data: 'template_name'
          },
          {
            ...
            render: function(data, type, row) {
              // If the template path is an image file, show a thumbnail.
              if (isImageFile(data)) {
                var imageURL = uploadFolder + data.split('/').pop();
                return '<img src="' + imageURL + '" alt="Thumbnail" class="dt-image">';
              }
              return data;
            }
          },
          ...
          },
          {
            data: null,
            render: function(data, type, row) {
              var html = '<i class="fas fa-edit action-icon editAssign" data-id="' + row.whatsapp_document_id + '" title="Edit" style="margin-right: 10px;"></i>';
              html += '<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.whatsapp_document_id + '" title="Delete"></i>';
              return html;
            }
          }
        ]
      ...
      // Update file input label, preview, and set Template Path field
      $('#template_file').on('change', function(e) {
        var fileInput = this;
        if (fileInput.files.length > 0) {
          var file = fileInput.files[0];
          var fileName = file.name;
          var fullPath = uploadFolder + fileName;
          $('#template_path').val(fullPath);
          $(this).next('.custom-file-label').html(fileName);

          // Validate file size (max 1MB)
          if (file.size > 1048576) {
            Swal.fire('Warning', 'Selected file must be 1MB or less.', 'warning');
            $(this).val('');
            $('#template_path').val('');
      ...
      // When the preview image (in the form) is clicked, show it in the modal
      ...
      // When a thumbnail in the DataTable is clicked, open modal with large image
      $(document).on('click', '.dt-image', function() {
        var imageSrc = $(this).attr('src');
        if (imageSrc) {
          $('#modalImage').attr('src', imageSrc);
          $('#imageModal').modal('show');
      ...
      // Clear/Cancel button: resets form and reverts button text
      ...
      function resetForm() {
        ...
        $('#template_file').next('.custom-file-label').html("Choose file");
        $('#previewContainer').hide();
        $('#saveBtn').html('<i class="fas fa-save"></i> Save');
        $('#clearBtn').html('<i class="fas fa-times"></i> Clear');
        $('#whatsapp_document_id').val('');
      }

      // Duplicate validation check (case-insensitive) for Template Name
      function isDuplicateTemplate(name, currentId) {
        var duplicate = false;
        table.rows().every(function() {
          var data = this.data();
          if (data.template_name.toLowerCase() === name.toLowerCase()) {
            if (currentId === "" || data.whatsapp_document_id != currentId) {
              duplicate = true;
            }
          }
        });
      ...
      // Form submission: Add/Update with validations and duplicate check
      $('#documentForm').on('submit', function(e) {
        e.preventDefault();
        var nameVal = $('#template_name').val().trim();

        // Validate Template Name length
        if (nameVal.length < 3 || nameVal.length > 100) {
          Swal.fire('Warning', 'Template Name must be between 3 and 100 characters.', 'warning');
          return;
        }
        // Validate Template Name format: starts with uppercase, letters, spaces, hyphens, apostrophes
        var nameRegex = /^[A-Za-z0-9]+(?:[ '-][A-Za-z0-9]+)*$/;
        if (!nameRegex.test(nameVal)) {
          Swal.fire('Warning', 'Invalid Template Name. Ensure it starts with a letter and contains only letters, numbers, spaces, hyphens, or apostrophes.', 'warning');
          return;
        }

        // Check for duplicate template name (case-insensitive)
        var currentId = $('#whatsapp_document_id').val();
        if (isDuplicateTemplate(nameVal, currentId)) {
          Swal.fire('Warning', 'Duplicate Template Name detected. Please use a unique name.', 'warning');
          return;
        }

        // Validate file size if a file is selected (max 1MB)
        ...
        $.ajax({
          url: "<?= site_url('WhatsappDocumentController/save') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          processData: false,
      ...
      // Edit action: populate form with row data and update button texts to reflect edit mode
      $('#documentTable').on('click', '.editAssign', function() {
        var id = $(this).data('id');
        var rowData = table.row($(this).parents('tr')).data();
        $('#whatsapp_document_id').val(rowData.whatsapp_document_id);
        $('#template_name').val(rowData.template_name);
        $('#template_path').val(rowData.template_path);
        $('#enterprise_id').val(rowData.enterprise_id);
        
        $('#saveBtn').html('Update');
        $('#clearBtn').html(' Cancel');
        
        if(rowData.template_path && isImageFile(rowData.template_path)) {
          var imageURL = uploadFolder + rowData.template_path.split('/').pop();
          
          validateImageSize(imageURL, function() {
            ...
            Swal.fire('Error', 'The image is too large. Please select an image under 1MB.', 'error');
            $('#previewContainer').hide();
          });
      ...
      // Function to validate image size (fetch image and check its size)
      function validateImageSize(url, successCallback, errorCallback) {
      ...
      // Delete action: confirm deletion then delete record via AJAX
      $('#documentTable').on('click', '.deleteAssign', function() {
        var id = $(this).data('id');
        Swal.fire({
          title: 'Are you sure?',
          text: "This will soft-delete the template.",
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?= site_url('WhatsappDocumentController/delete') ?>",
              type: "POST",
              data: {
                whatsapp_document_id: id
              },
              dataType: "json",
              success: function(response) {
                if (response.status === 'success') {
