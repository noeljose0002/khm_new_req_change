...
class MisReportModel extends Model
{
    ...
    protected function baseQuery()
    {
        ...
        $latestFollowupSub = $this->db
            ->table('khm_obj_executive_follow_up')
            ->select([
                'enquiry_header_id',
                'MAX(executive_follow_up_id) AS latest_follow_up_id'
            ])
            ->groupBy('enquiry_header_id')
            ->getCompiledSelect();
        ...
        return $this->db
            ...
            ->where('ede.is_active', 1)
            ->join(
                "($latestFollowupSub) AS lf",
                'lf.enquiry_header_id = h.enquiry_header_id',
                'left'
            )

            ->join(
                'khm_obj_executive_follow_up AS koef',
                'koef.enquiry_header_id = lf.enquiry_header_id
     AND koef.executive_follow_up_id = lf.latest_follow_up_id',
                'left'
            )
