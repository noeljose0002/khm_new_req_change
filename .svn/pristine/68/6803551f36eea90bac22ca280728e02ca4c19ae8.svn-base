            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Monthly Target Report</h4>
                ...
                <head>
                    ...
                    <style>
                        ...
                        #monthlytargetReportTable thead th {
                ...
                <body class="p-4">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="form-inline filter-row align-items-center">
                                    ...
                                    <div class="form-group">
                                        <label for="targetDropdown" class="ml-3 mr-1">Targets:</label>
                                        <select id="targetDropdown" class="form-control">
                    ...
                    <div id="tableContainer">
                        <div class="card">
                            <div class="card-body p-4">
                                <table id="monthlytargetReportTable" style="width: 100%;">
                                    <thead>
                                        <tr>
...
<script>
    ...
    $.ajax({
        url: '<?= site_url("MonthlyTargetReport/executive") ?>',
        method: 'GET',
        dataType: 'json',
    ...
    $.ajax({
        url: '<?= site_url("MonthlyTargetReport/target") ?>',
        method: 'GET',
        dataType: 'json',
        success: function(target) {
            target.forEach(function(target) {
                $('#targetDropdown').append(
                    $('<option>').val(target.id).text(target.name)
                );
            });
        },
        error: function(xhr, status, err) {
    ...
    $(function() {
        ...
        var table = $('#monthlytargetReportTable').DataTable({
            ...
            buttons: [{
                ...
                filename: 'MonthlyTargetReport_report_' + moment().format('YYYYMMDD_HHmmss'),
                exportOptions: {
                    columns: ':visible',
                    // format: {
            ...
            serverSide: false,
            ajax: {
                url: '<?= site_url("MonthlyTargetReport/data") ?>',
                data: function(d) {
                    ...
                    d.target_id = $('#targetDropdown').val();
                     d.system    = systemId;
                }
            },
            columns: [{
                ...
                {
                    title: 'Status',
                    data: null, // so we get the whole row in render()
                    render: function(_data, _type, row) {
                        var target = parseFloat(row.target_amount) || 0;
                        var achieved = parseFloat(row.total_tpc) || 0;
                        var text, cls;

                        // no target defined
                        if (!row.target_name || target === 0) {
                            text = 'Target Not Defined';
                            cls = 'text-success';
                        }
                        // achieved ≥ target
