                ...
                <body>
                    <div class="container mt-5">
                                ...
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="entity_class_name"> Entity Class Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="entity_class_parent_id">Entity Parent Class</label>
                                            <select
                                                class="form-control"
                                                id="entity_class_parent_id"
                                                name="entity_class_parent_id">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            <div class="card-header form-header">
                                <h4>Edit Entity Class</h4>
                            </div>
                            <div class="card-body">
                                <form id="editForm">
                                    <input type="hidden" name="_method" value="PUT">
                                    <!-- hidden field to hold the PK -->
                                    <input type="hidden" id="edit_entity_class_id " name="entity_class_id ">

                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="edit_entity_class_name">Entity Class Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="edit_entity_class_name"
                                                name="entity_class_name"
                                                maxlength="20"
                                                required>
                                        </div>
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="edit_entity_class_parent_id">Entity Parent Class</label>
                                            <select
                                                class="form-control"
                                                id="edit_entity_class_parent_id"
                                                name="entity_class_parent_id">
                                                <option value="">-- Select Class --</option>
                                                <!-- Populate options dynamically -->
                                            </select>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>Entity Class List</h4>
                            </div>
                            <div class="card-body">
                                <table id="entityClassTable" class="table table-bordered">
                                    <thead>
...
<script>
    $(document).ready(function() {
        ...
        $.getJSON('<?= site_url("SuperAdminEntityClass/list") ?>', function(res) {
            allClasses = res.data;
            ...
            $('#entity_class_parent_id').html(buildOptions(allClasses, '-- Select Class --'));
            // inject into EDIT form
            $('#edit_entity_class_parent_id').html(buildOptions(allClasses, '-- Select Class --'));
        });

        // 2) Initialize DataTable
        const table = $('#entityClassTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminEntityClass/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                ...
                {
                    data: 'entity_class_parent_id',
                    title: 'Entity Class Parent',
                    render: function(cid) {
                        // Look up a class whose entity_class_id equals the parent‑ID from this row:
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post(
                    '<?= site_url("SuperAdminEntityClass/create") ?>',
                    $(this).serialize()
                )
                .done(() => {
                    $('#addForm')[0].reset();
                ...
                .fail(xhr => {
                    Swal.fire('Error', xhr.responseJSON?.errors?.entity_class_name || 'Failed', 'error');
                });
        });

        // 5) SHOW EDIT
        $('#entityClassTable').on('click', '.editBtn', function() {
            const id = $(this).data('id');

            $.getJSON(`<?= site_url("SuperAdminEntityClass/edit") ?>/${id}`, function(data) {
                // Fill the form fields
                $('#edit_entity_class_id').val(data.entity_class_id);
                $('#edit_entity_class_name').val(data.entity_class_name);
                ...
                const parentOptions = allClasses.reduce((html, item) => {
                    const selected = item.entity_class_id == data.entity_class_parent_id ? ' selected' : '';
                    return html + `<option value="${item.entity_class_id}"${selected}>${item.entity_class_name}</option>`;
                }, '<option value="">-- Select Class --</option>');

                $('#edit_entity_class_parent_id').html(parentOptions);
        ...
        $('#editForm').submit(function(e) {
            ...
            $.ajax({
                    url: `<?= site_url("SuperAdminEntityClass/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .done(() => {
        ...
        // 8) DELETE
        $('#entityClassTable').on('click', '.deleteBtn', function() {
            const id = $(this).data('id');
            Swal.fire({
                title: 'Delete?',
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminEntityClass/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(() => {
                            table.ajax.reload();
