...
<head>
  ...
  <link rel="icon" href="<?php
                          ...
                          echo base_url('assets/images/brand/favicon.png'); ?>" type="image/x-icon" />
  ...
  <div class="page">
    <div class="page-main">
      ...
      <div class="app-content icon-content">
        <div class="section">
          ...
          <div class="page-header">
            <div class="page-leftheader">
              <h4 class="page-title mb-0">GEOGRAPHY LOCATIONS</h4>
        ...
        <head>
          ...
          <!-- Bootstrap 4.1.3 CSS, Font Awesome, jQuery UI, Leaflet, DataTables CSS -->
          ...
          <style>
            ...
            /* Main container for both form and DataTable with the same width */
            /* .container-custom {
      width: 80%; */
            /* max-width: 1000px; */
            /* margin: auto; */
    ...
    .dataTables_wrapper .dataTables_paginate {
      margin-top: 1rem;
            ...
            /* Custom Pagination Buttons: Green Background */
    ...
    .dataTables_wrapper .dataTables_paginate .paginate_button:hover {
      ...
      color: #fff !important;
            ...
            /* DataTable header background and text color */
        ...
        <body>
          <div class="container-custom mt-5">
            ...
            <div class="card card-form">
              <div class="card-header">
                <h4 class="mb-0">Geography Record Form</h4>
              </div>
              <div class="card-body">
                ...
                <form id="addGeogForm" class="needs-validation" novalidate>
                  ...
                  <div class="text-center">
                    ...
                    <!-- BOTTOM CENTRE BUTTON SET -->
                  </div>
                </form>
              </div>
            </div>

            <!-- Data Table Section inside a card -->
            <div class="card card-table">
              <div class="card-header">
                <h4 class="mb-0">Geography Records List</h4>
              </div>
              <div class="card-body">
                <table class="table table-bordered table-hover" id="recordsTable">
                  <thead>
                    <tr>
                      <th hidden>Geog ID</th>
                    ...
                    <?php if (isset($records) && is_array($records) && count($records)): ?>
                      <?php foreach ($records as $record): ?>
                        <tr id="row_<?= $record['geog_id'] ?>">
                          <td hidden><?= $record['geog_id'] ?></td>
                          <td hidden><?= $record['geog_level_id'] ?></td>
                          ...
                          <td class="actions-col text-center">
                            <div class="btn-group" role="group">
                              <!-- Action icons set to green -->
                              <button class="btn btn-light btn-sm view-btn" data-record='<?= htmlspecialchars(json_encode($record), ENT_QUOTES, "UTF-8") ?>'>
                                <i class="fas fa-eye text-success"></i>
                              </button>
                              <button class="btn btn-light btn-sm edit-btn" data-record='<?= htmlspecialchars(json_encode($record), ENT_QUOTES, "UTF-8") ?>'>
                                <i class="fas fa-pencil-alt text-success"></i>
                              </button>
                              <button class="btn btn-light btn-sm" onclick="deleteRecord('<?= $record['geog_id'] ?>')">
...
<script>
  ...
  $(function() {
    ...
    initEventHandlers();
    initDataTable();
    checkForQueryParams(); // check for query params (e.g., ?geog_id=, ?parentId=, etc.)
  });
  ...
  function initAutoComplete() {
    $("#geog_name").autocomplete({
      ...
      source: function(req, res) {
        ...
        if (parentName) {
          searchText = parentName + " " + req.term;
        }
        var apiUrl = "https://api.geoapify.com/v1/geocode/search?text=" +
          encodeURIComponent(searchText) +
          "&format=json&apiKey=46ab7b49387d4b59984151af1ce71554";
        if (parentLat && parentLng) {
          apiUrl += "&bias=proximity:" + parentLng + "," + parentLat;
        }
        $.ajax({
          type: "GET",
          url: apiUrl,
          ...
          success: function(data) {
            ...
            res(results.length ? $.map(results, function(item) {
              return {
  ...
  function autoDetectLevel(locationProps) {
    var detected = "";
    if ((locationProps.suburb && locationProps.suburb.trim()) || (locationProps.town && locationProps.town.trim())) {
      detected = "5";
    } else if (locationProps.county && locationProps.county.trim() && locationProps.city && locationProps.city.trim()) {
      detected = "4";
  ...
  function initEventHandlers() {
    ...
    $("#addGeogForm").on("submit", function(e) {
      ...
      var edit = $("#editing_geog_id").val();
      var name = $("#geog_name").val().trim().toLowerCase();
      var ent = $("#enterprise_id").val().trim();
      var isDuplicate = records.some(function(r) {
        return r.geog_name.trim().toLowerCase() === name &&
      ...
      var url = edit ? "GeogLocation/updateGeog/" + edit : "GeogLocation/insertGeog";
    ...
    window.deleteRecord = function(id) {
      Swal.fire({
        ...
        if (r.isConfirmed) {
          $.ajax({
            url: SITEURL + "/GeogLocation/deleteGeog/" + id,
            method: "DELETE",
            dataType: "json",
            success: function(resp) {
              if (resp.success) {
    ...
    $("#toggleMap").on("click", function() {
      ...
      if (m.is(":visible")) {
        ...
        m.slideDown(function() {
          ...
          var lat = parseFloat($("#geog_latitude").val());
          var lon = parseFloat($("#geog_longitude").val());
  ...
  function updateParentDropdown() {
    var lvl = parseInt($("#geog_level_id").val(), 10);
    $("#geog_parent_id").empty().append('<option value="">-- Select Parent --</option>');
    var p = parentMapping[lvl];
    if (!p) {
      $("#geog_parent_id").append('<option value="">No matching parent</option>');
      return;
    }
    if (p === 1) {
      var w = records.find(function(r) {
        return parseInt(r.geog_level_id) === 1;
      });
      if (w) {
    ...
    var filtered = records.filter(function(r) {
      return parseInt(r.geog_level_id) === p;
    });
    if (filtered.length) {
  ...
  function populateFormForEdit(rec) {
    ...
    var lat = parseFloat(rec.geog_latitude),
  ...
  function initDataTable() {
    $("#recordsTable").DataTable({
      ...
      // paging: false         // remove pagination (optional)
      paging: true,
      lengthChange: true,
      searching: true,
      ordering: true,
    ...
    map.on("click", function(e) {
      ...
      $.ajax({
        ...
        error: function(x, s, err) {
          console.error("Reverse geocoding error:", s, err);
        }
      });
    });
  }
  ...
  function checkForQueryParams() {
    ...
    if (editGeogId) {
      var editRecord = records.find(function(r) {
        return r.geog_id == editGeogId;
      });
      if (editRecord) populateFormForEdit(editRecord);
    } else {
      var params = new URLSearchParams(window.location.search),
        parentId = params.get('parentId'),
        parentNameParam = params.get('parentName'),
        newLevel = params.get('newLevel');
      if (parentId && parentNameParam && newLevel) {
        ...
        $("#geog_parent_name").val(parentNameParam);
        const levelLabels = {
          "2": "Add Country",
          "3": "Add Region",
          "4": "Add Location",
        ...
        const headingText = levelLabels[newLevel] || "Add Geography";
