    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0"> Group Enquiry MIS Report</h4>
                ...
                <head>
                    <meta charset="utf-8">
                    <title> Group Enquiry MIS Report</title>
                    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
                    ...
                    <style>
                        ...
                        /* 2. Hover on non-disabled, non-current */
                        ...
                        /* Light green header for datatable */
                        #groupmistargetReportTable thead th {
                        ...
                        /* Hide export button container until data is loaded */
                        .dt-buttons {
                            display: none;
                        }
                ...
                <body class="p-4">
                    ...
                    <div id="filterContainer">
                        <div class="card">
                            <div class="card-body">
                                <div class="form-inline filter-row align-items-center">
                                    <div class="form-group">
                                        <label for="from_date" class="mr-1">From:</label>
                                        <input type="date" id="from_date" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label for="to_date" class="ml-3 mr-1">To:</label>
                                        <input type="date" id="to_date" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label for="executiveDropdown" class="ml-3 mr-1">Executives:</label>
                                        <select id="executiveDropdown" class="form-control">
                                            <option value="all">All Executives</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="statusDropdown" class="ml-3 mr-1">Status:</label>
                                        <select id="statusDropdown" class="form-control">
                                            <option value="all">All Status</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="agentsDropdown" class="ml-3 mr-1">Agents:</label>
                                        <select id="agentsDropdown" class="form-control">
                                            <option value="all">All Agents</option>
                                        </select>
                                    </div>
                                    <!-- <div class="preset-btns ml-1">
                                    ...
                                    <button id="btnFilter" class="btn btn-success ml-auto">Generate</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="tableContainer">
                        <div class="card">
                            <div class="card-body p-4">
                                <table id="groupmistargetReportTable" style="width: 100%;">
                                    <thead>
                                        <tr>
...
<script>
    // Populate destination dropdown via $.ajax
    $.ajax({
        url: '<?= site_url("GroupEnquiryMisReport/executive") ?>',
        method: 'GET',
        dataType: 'json',
        success: function(executive) {
            executive.forEach(function(executive) {
                $('#executiveDropdown').append(
                    $('<option>').val(executive.id).text(executive.name)
                );
            });
        },
    ...
    $.ajax({
        url: '<?= site_url("GroupEnquiryMisReport/status") ?>',
        method: 'GET',
        dataType: 'json',
        success: function(status) {
            status.forEach(function(status) {
                $('#statusDropdown').append(
                    $('<option>').val(status.id).text(status.name)
                );
            });
        },
        error: function(xhr, status, err) {
    ...
    $.ajax({
        url: '<?= site_url("GroupEnquiryMisReport/agents") ?>',
        method: 'GET',
        dataType: 'json',
        success: function(agents) {
            agents.forEach(function(agents) {
                $('#agentsDropdown').append(
                    $('<option>').val(agents.id).text(agents.name)
                );
            });
        },
        error: function(xhr, status, err) {
    ...
    // if your data is "DD-MM-YYYY HH:mm:ss":
    $.fn.dataTable.moment('DD-MM-YYYY HH:mm:ss');
    // if you also have pure dates "DD-MM-YYYY":
    ...
    $(function() {
        ...
        var table = $('#groupmistargetReportTable').DataTable({
            dom: '<"row"<"col-sm-12 col-md-6"B><"col-sm-12 col-md-6"f>><"row"<"col-sm-12"tr>><"row"<"col-sm-12 col-md-5"i><"col-sm-12 col-md-7"lp>>',
            buttons: [{
                extend: 'excelHtml5',
                text: 'Export Excel',
                className: 'btn btn-outline-primary',
                title: 'Group Enquiry MIS Report',
                filename: 'GroupMISReport_report_' + moment().format('YYYYMMDD_HHmmss'),
                exportOptions: {
                    columns: ':visible',
                    // format: {
                ...
                //     $('row c[r^="D"], row c[r^="H"]', sheet).attr('s', '22'); // Format date columns
                // }
            }],
            processing: true,
            serverSide: false,
            ajax: {
                url: '<?= site_url("GroupEnquiryMisReport/data") ?>',
                data: function(d) {
                    d.from_date = convertDateToServerFormat($('#from_date').val());
                    d.to_date = convertDateToServerFormat($('#to_date').val());
                    d.executive_id = $('#executiveDropdown').val();
                    d.status_id = $('#statusDropdown').val();
                    d.agents_id = $('#agentsDropdown').val();
                    d.system = systemId;
            ...
            columns: [{
...

                {
                    ...
                    render: function(data, type, row) {
                        // only change what the user sees (display / filter); leave sort/search on raw value
                        if (type === 'display') {
                            var sourceMap = {
                                1: 'Email',
                                2: 'Reference',
                        ...
                        // for filter/order/export use the raw numeric value
                        return data;
                    }
                },
...

                {
                    ...
                    data: null,
                    render: function(data, type, row) {
                        let enqDateStr = row.enq_added_date + " 00:00:00";
                        let createdDateStr = row.created_date;
                        let enqDate = new Date(enqDateStr);
                        let createdDate = new Date(createdDateStr);
                        let diffMin = Math.floor((createdDate - enqDate) / 60000);
                        return diffMin;
                    }
                },
                ...
                {
                    title: 'Agent',
                    data: 'agent_entity_name'
                },

                {
                    title: 'Guest',
                    data: 'guest_entity_name'
                },
                ...
                {
                    title: 'Start Date',
                    data: 'date_of_tour_start'
                },

                {
                    title: 'Pax',
                    data: 'total_no_of_pax'
                },

                {
                    title: 'Rooms',
                    data: 'no_of_double_room'
                },
...

                {
                    ...
                    render: function(_, type, row) {
                        // only compute for display/filter; let DataTables sort on the raw number
                        const val = row.no_of_double_room * row.no_of_night;
                        return type === 'display' || type === 'filter' ? val : val;
                    },
                    // tell DataTables it’s a numeric column for proper sorting:
                    type: 'num'
                },
