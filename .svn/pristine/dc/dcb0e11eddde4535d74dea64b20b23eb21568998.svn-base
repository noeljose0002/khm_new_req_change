      ...
      <div class="app-content icon-content">
        <div class="section">
          ...
          <div class="page-header">
            <div class="page-leftheader">
              <h4 class="page-title mb-0">FACILITY</h4>
        ...
        <head>
          ...
          <style>
            ...
            /* Style for action buttons to display icons in green */
            .icon-btn {
              align-items: center;
              background: none;
              border: none;
        ...
        <body>
          <div class="container">
            ...
            <div class="card">
              <div class="card-header">
                <h4 class="mb-0">Add / Edit Facility</h4>
              </div>
              <div class="card-body">
                <form id="facilityForm">
                  ...
                  <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                      ...
                      <select class="form-control" id="facilityType" name="facility_type_id" required>
                  ...
                  <!-- Enterprise id can be hidden or selectable -->
                  <input type="hidden" name="enterprise_id" value="1">
            ...
            <div class="card">
              <div class="card-header">
                <h4 class="mb-0">Facility List</h4>
              </div>
              <div class="card-body">
...
<script>
  // Facility Name Validation Function
  function validateFacilityName(name) {
    ...
    if (trimmedName.length < 2 || trimmedName.length > 50) {
      return {
        valid: false,
        message: "Facility name must be between 2 and 50 characters."
      };
    }
    ...
    if (!regex.test(trimmedName)) {
      return {
        valid: false,
        message: "Facility name contains invalid characters. Only letters, spaces, hyphens, and apostrophes are allowed."
      };
    }
    return {
      valid: true,
      message: "Facility name is valid."
    };
  }
  ...
  $(document).ready(function() {
    ...
    var table = $('#facilityTable').DataTable({
      "ajax": {
        "url": "<?= site_url('Facility/getFacilities') ?>",
        "dataSrc": "data"
      },
      "columns": [{
          "data": null,
        ...
        },
        {
          ...
          "render": function(data, type, row) {
            return '<button class="btn btn-sm icon-btn editBtn" data-id="' + row.facility_id + '" title="Edit Facility"><i class="fas fa-edit"></i></button> ' +
              '<button class="btn btn-sm icon-btn deleteBtn" data-id="' + row.facility_id + '" title="Delete Facility"><i class="fas fa-trash-alt"></i></button>';
          }
        }
      ]
    });
    ...
    $('#clearBtn').on('click', function() {
      ...
      $('#facility_id').val('');
      $('#saveBtn').text('Save Facility');
    });

    // Submit facility form (create or update)
    $('#facilityForm').on('submit', function(e) {
      e.preventDefault();
      const facilityName = $('#facilityName').val();
      const validation = validateFacilityName(facilityName);
      if (!validation.valid) {
      ...
      // Check for duplicate facility name before submission
      $.ajax({
        url: "<?= site_url('Facility/checkDuplicate') ?>",
        type: "POST",
        data: {
          facility_name: facilityName,
          facility_id: facilityId // Exclude current record during edit if updating
        },
        dataType: "json",
        success: function(res) {
          if (res.exists) {
            Swal.fire('Error!', 'Facility name already exists.', 'error');
          } else {
            // Determine the URL based on whether we're creating or updating
            var url = facilityId ? "<?= site_url('Facility/update') ?>" : "<?= site_url('Facility/create') ?>";
            $.ajax({
              url: url,
              type: "POST",
              data: formData,
        ...
        error: function() {
          Swal.fire('Error!', 'Could not validate duplicate entry.', 'error');
        }
      });
    });
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.status === 'success') {
            ...
            $('#facilityName').val(facility.facility_name);
            $('#facilityType').val(facility.facility_type_id);
            $('#saveBtn').text('Update');
            $('html, body').animate({
        scrollTop: $("#facilityForm").offset().top - 200
      }, 200);
        ...
        error: function() {
          Swal.fire('Error!', 'Could not load facility data.', 'error');
        }
      });
    });

    // Delete button click functionality (soft delete)
    $('#facilityTable').on('click', '.deleteBtn', function() {
      var id = $(this).data('id');
      Swal.fire({
        title: 'Are you sure?',
        text: "This facility will be soft deleted!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "<?= site_url('Facility/softDelete') ?>/" + id,
            type: "POST",
            dataType: "json",
            success: function(response) {
              if (response.status === 'success') {
