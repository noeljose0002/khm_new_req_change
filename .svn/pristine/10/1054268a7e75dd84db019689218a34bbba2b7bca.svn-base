...
.modal-title {
	...
	<body class="app sidebar-mini">	
		...
		<div id="roleviewmodal" class="modal fade">
			<div class="modal-dialog custom-modal-width" role="document">
				<div class="modal-content ">
					<div class="modal-header">
						<h6 class="modal-title">Employee Roles</h6>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body pd-20 roleviewmodal_body">
					...
					<div class="section">
						...
						<div class="page-header">
							<div class="page-leftheader">
								<h4 class="page-title mb-0">Employee Roles</h4>
								<small class="text-muted mt-0 fs-14">Assign Roles to Employees</small>
							</div>
									...
									<div class="card-body">
										<div class="table-responsive product-datatable">
											<table id="example" class="table table-striped table-bordered " >
												<thead>
													<tr>
														<th class="w-15p">Employee</th>
													
														<th class="wd-10p">Roles</th>
													</tr>
												</thead>
												<tbody>
													<?php foreach($users_list as $key => $val) { ?>
														<tr>
															<td><?php echo $val['entity_name']; ?></td>
															
															<!--<td><span class="badge badge-danger-light badge-md">Stock</span></td>-->
															<td>
																<a class="nav-link entity_role_view_id" data-id-name="<?php echo $val['entity_name']; ?>" data-id="<?php echo $val['entity_id']; ?>" href=""><i class="fa fa-eye"></i></a>
...
<script type="text/javascript">
    $(document).on('click', '.entity_role_view_id', function(e) {
        e.preventDefault();
        var entity_id = $(this).attr('data-id');
		var entity_name = $(this).attr('data-id-name');
        $.ajax({
            type: "POST",
            url: "<?=site_url('Dashboard/assign_employee_roles');?>",
            data: {
                entity_id:entity_id,
            ...
            success: function(response) {
                $('.roleviewmodal_body').html(response);
...
<script>
/*$(document).on('click', '#btn_update_roles', function (e) {
        e.preventDefault();
		var entity_id = $('#hidden_entity_id').val();
		var selectedRoles = [];
        $('#entity-role-datatable input.entity-role-input:checked').each(function () {
            selectedRoles.push($(this).val());
        });
       
				$.ajax({
					url: '<?=site_url('Dashboard/updateRoles');?>',
					method: 'POST',
					data: {
						entity_id: entity_id,
    ...
    $('#entity-role-datatable input.entity-role-input:checked').each(function () {
        ...
        // Get matching <select> with the same data-role-id
        var leadSelect = $('select.team-lead-select[data-role-id="' + roleId + '"]');
        if (leadSelect.length) {
            var selectedLeadId = leadSelect.val();
            roleLeadMap[roleId] = selectedLeadId;
        } else {
            roleLeadMap[roleId] = null;
        }
    });
