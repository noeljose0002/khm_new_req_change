    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Add New Role</h4>
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Role CRUD with AJAX - Enhanced UI</title>
                    <!-- Bootstrap 4 CSS -->
                            ...
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="hierarchy_id">Hierarchy ID</label>
                                            <input type="number" class="form-control" id="hierarchy_id" name="hierarchy_id" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-row">
                                        ...
                                        <!-- you can add empty cols to push buttons, or just let them span full width -->
                                        <div class="col-md-8 d-flex align-items-end">
                                            <button type="submit" class="btn btn-success mr-2">Save Role</button>
                            ...
                            <div class="card-body">
                                <form id="editForm">
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="edit_parent_id">Parent Role</label>
                                            <select class="form-control" id="edit_parent_id" name="parent_id">
                                                <!-- options injected by JS -->
                                            </select>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label for="edit_hierarchy_id">Hierarchy ID</label>
                                            <input type="number" class="form-control" id="edit_hierarchy_id" name="hierarchy_id" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="col-md-8 d-flex align-items-end">
                                            <button type="submit" class="btn btn-success mr-2">Update Role</button>
...
<script>
    $(document).ready(function() {
        ...
        // 1) Fetch all roles via AJAX
        $.getJSON('<?= site_url("SuperAdminRole/list") ?>', function(response) {
            // The DataTables endpoint wraps roles in { data: [...] }
            allRoles = response.data;

            // Populate both Add + Edit dropdowns
            function buildOptions(selectedId) {
                let html = '<option value="">-- None --</option>';
                allRoles.forEach(r => {
                    let sel = r.role_id == selectedId ? ' selected' : '';
                    html += `<option value="${r.role_id}"${sel}>${r.role_name}</option>`;
                });
                return html;
            }

            $('#parent_id').html(buildOptions());
            $('#edit_parent_id').html(buildOptions());
        });

        // 2) Initialize DataTable once roles are loaded
        var table = $('#roleTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminRole/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'role_id',
            ...
            order: [
                [0, 'ASC']
            ], // Sort by hierarchy_id (4th column) descending
            lengthMenu: [
                [10, 25, 50, 100, -1],
                [10, 25, 50, 100, "All"]
            ],
        ...
        // 4) CREATE
        // Check for duplicate role names excluding soft-deleted records
        function isDuplicate(name, currentId = null) {
            name = name.trim().toLowerCase();
            return allRoles.some(r =>
                r.deleted !== 1 && // Exclude soft-deleted roles
                r.role_name.trim().toLowerCase() === name &&
                (currentId === null || r.role_id != currentId)
            );
        }
        ...
        $('#addForm').submit(function(e) {
            ...
            if (!name) {
                return Swal.fire('Validation', 'Role name can’t be blank', 'warning');
            }

            if (isDuplicate(name)) {
                return Swal.fire('Validation', 'That role name already exists', 'warning');
            }

            $.ajax({
                url: '<?= site_url("SuperAdminRole/create") ?>',
                type: 'POST',
                data: $(this).serialize(),
                success: () => {
                    $('#addForm')[0].reset();

                    // ✅ Reload table and update allRoles only after table reload is done
                    table.ajax.reload(() => {
                        allRoles = table.data().toArray(); // Now allRoles is synced
                    });

                    Swal.fire('Saved!', 'New role added.', 'success');
                },
                error: xhr => {
                    const message = xhr.responseJSON?.errors?.role_name || 'Failed';
        ...
        $('#roleTable').on('click', '.editBtn', function() {
            let id = $(this).data('id');
            $.getJSON(`<?= site_url("SuperAdminRole/edit") ?>/${id}`, function(data) {
                $('#edit_role_id').val(data.role_id);
                $('#edit_role_name').val(data.role_name);
                // repopulate and select in case parents change
                $('#edit_parent_id').val(data.parent_id);

                $('#edit_hierarchy_id').val(data.hierarchy_id);
        ...
        $('#editForm').submit(function(e) {
            ...
            // 1) grab the edited name from the correct input
            const name = $('#edit_role_name').val();
            ...
            // 3) grab the ID and include it in the duplicate check
            let id = parseInt($('#edit_role_id').val(), 10);
            ...
            // 4) do your PUT
            $.ajax({
                url: `<?= site_url("SuperAdminRole/update") ?>/${id}`,
                type: 'PUT',
                data: $(this).serialize(),
                success: () => {
                    $('#editForm')[0].reset();
                ...
                error: xhr => Swal.fire('Error', xhr.responseJSON.errors.role_name || 'Failed', 'error')
            });
        });
        ...
        $('#roleTable').on('click', '.deleteBtn', function() {
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    $.ajax({
                        url: `<?= site_url("SuperAdminRole/delete") ?>/${id}`,
                        type: 'DELETE'
                    }).done(() => {
                        table.ajax.reload();
