...
#entity_table th, #entity_table td {
...
#entity_table th.auto-width, #entity_table td.auto-width {
					...
					<div class="section">
						...
						<div class="page-header">
							<div class="page-leftheader">
								<h4 class="page-title mb-0"><?php echo $entity_class_name; ?></h4>
								<small class="text-muted mt-0 fs-14">View / Add / Edit <?php echo $entity_class_name; ?> Details</small>
							</div>
						...
						<div class="row">
							<div class="col-md-12 col-lg-12">
								<div class="card">
									...
									<div class="card-body">
			<?php
  // extract unique geog_name values
  $locations = array_unique(array_column($users_list, 'geog_name'));
  sort($locations);
?>
<div class="row mb-3">
  <div class="col-md-3">
    ...
    <select id="location_filter" class="form-control">
      ...
      <?php endforeach; ?>
										...
										<table id="entity_table" class="table table-bordered table-striped" cellspacing="0" width="100%">
											<thead style="background-color:#c6ecd9;">
												<?php if($entity_class_id == "13"){ ?>
													<tr>
														<th><?php echo $entity_class_name; ?> Name</th>
...
<script>
	function deleteEntity(entity_id){
		const userConfirmed = confirm("Are you sure you want to delete?");
            ...
            if (userConfirmed) {
				$.ajax({
					url: '<?=site_url('Dashboard/deleteEntity');?>',
					method: 'POST',
					data: {
						entity_id:entity_id
					},
					...
					success: function (response) {
						if(response == 1){
							...
							alert("Cannot be deleted. Dependency exist");
						}
					},
...
$(document).ready(function () {
		...
		var tableConfig = {
			...
			buttons: ['colvis'],
			ajax: {
				url: '<?= site_url('Dashboard/entity_list_view'); ?>',
				type: 'POST',
				data: function (d) {
					d.entity_class_id = entity_class_id;
					d.location = $('#location_filter').val();
				}
			}
		};

		// Conditionally set columns
		if (entity_class_id == "13") {
			tableConfig.columns = [
				{ data: 'entity_name' },
				{
					data: 'entity_mobile',
			...
			tableConfig.columns = [
				...
				},
				{
					data: 'entity_id',
					render: function (data) {
						return '<a href="<?= site_url('dashboard/view_entity/'); ?>' + entity_class_id + '/' + data + '" class="btn btn-success btn-sm text-white"><i class="fa fa-eye"></i></a>';
					}
				},
				{
					data: 'entity_id',
		...
		// ✅ Initialize and assign the DataTable to a variable
		var table = $('#entity_table').DataTable(tableConfig);

		// ✅ Reload DataTable on location change
