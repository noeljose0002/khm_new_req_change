            ...
            <div class="app-content icon-content">
                ...
                <head>
                    ...
                    <style>
                        ...
                        /* DataTable header: light green background, white text */
                        #taxRateTable thead th {
                        ...
                        /* #taxRateTable tbody tr:hover {
      background-color: #c8e6c9 !important;
    } */
                        /* Style for almost invisible separating line */
                        .separator {
                        ...
                        /* Adjust icon spacing inside buttons */
                        .btn i {
                            margin-right: 5px;
                        }
                ...
                <body>
                    <div class="container">
                        <div class="card shadow-sm">
                            <div class="card-header">
                                <h4 class="mb-0">Tax Rate Management</h4>
                            </div>
                            <div class="card-body">
                                <!-- Inline Form for Add/Edit -->
                                <form id="taxRateForm" class="needs-validation form-inline mb-4" novalidate style="flex-wrap: wrap;">
                                    <!-- Hidden fields -->
                                    <input type="hidden" name="tax_rate_id" id="tax_rate_id">
                                    <input type="hidden" name="enterprise_id" id="enterprise_id" value="1">

                                    <div class="form-group mb-2">
                                        <label for="tax_rate" class="mr-2">Tax Rate (%)</label>
                                        <!-- max="100" restricts percentage values -->
                                        <input type="number" step="0.01" min="0" max="100" class="form-control" name="tax_rate" id="tax_rate" placeholder="Enter tax rate" required>
                                        <div class="invalid-feedback">Please enter a valid tax rate (0 - 100%).</div>
                                    </div>

                                    <div class="form-group mb-2">
                                        <label for="applicable_hotel_range_from" class="mr-2">Hotel Range From</label>
                                        <!-- max="99999999" restricts the value up to 8 digits -->
                                        <input type="number" step="0.01" min="0" max="99999999" class="form-control" name="applicable_hotel_range_from" id="applicable_hotel_range_from" placeholder="Starting range" required>
                                        <div class="invalid-feedback">Please enter the starting range (up to 8 digits).</div>
                                    </div>

                                    <div class="form-group mb-2">
                                        <label for="applicable_hotel_range_to" class="mr-2">Hotel Range To</label>
                                        <!-- max="99999999" restricts the value up to 8 digits -->
                                        <input type="number" step="0.01" min="0" max="99999999" class="form-control" name="applicable_hotel_range_to" id="applicable_hotel_range_to" placeholder="Ending range" required>
                                        <div class="invalid-feedback">Please enter the ending range (up to 8 digits).</div>
                                    </div>

                                    <!-- Button container -->
                                    <div class="form-group mb-2 buttonsnj" id="buttonContainer">
                                        <!-- Default button order: Save, Clear, Cancel -->
                                        <button type="submit" id="saveBtn" class="btn btn-success">
                                            save
                                        </button>
                                        <button type="button" id="clearBtn" class="btn btn-success">
                                            clear
                                        </button>
                                        <button type="button" id="cancelBtn" class="btn btn-success" style="display: none;">
                                            cancel
                                        </button>
                                    </div>
                                </form>

                                <!-- Separator line between form and DataTable -->
                                <hr class="separator">
                                    ...
                                    <table class="table table-bordered" id="taxRateTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Tax Rate (%)</th>
                                                <th>Hotel Range From</th>
                                                <th>Hotel Range To</th>
...
<script>
    ...
    (function() {
        ...
        window.addEventListener('load', function() {
            var form = document.getElementById('taxRateForm');
    ...
    $(document).ready(function() {
        var table = $('#taxRateTable').DataTable({
            ajax: {
                url: "<?= site_url('TaxRate/fetchAll') ?>",
                dataSrc: ''
            },
            columns: [{ // Serial number column
                    data: null,
                    title: "No.",
                ...
                {
                    data: 'applicable_hotel_range_from',
                    title: "Hotel Range From"
                },
                {
                    data: 'applicable_hotel_range_to',
                    title: "Hotel Range To"
                },
                {
                    data: null,
                    title: "Actions",
                    ...
                    render: function(data, type, row) {
                        return `
                <i class="fas fa-edit action-icon edit" data-id="${row.tax_rate_id}" title="Edit"
                   style="margin-right: 10px; cursor: pointer; border: none; outline: none; color: green;">
                </i>
                <i class="fas fa-trash action-icon delete" data-id="${row.tax_rate_id}" title="Delete"
                   style="cursor: pointer; border: none; outline: none; color: green;">
                </i>
              `;
                    }
                }
        ...
        // Function to restore default button order: Save, Clear, Cancel
        function restoreDefaultOrder() {
            var container = $("#buttonContainer");
            $("#cancelBtn").detach().prependTo(container); // Save on left

            $("#saveBtn").detach().insertAfter($("#clearBtn")); // Cancel on the right
            $("#clearBtn").detach().insertAfter($("#saveBtn")); // Clear in the middle
        }

        // Function to clear form fields, reset buttons text, ordering and hide Cancel button
        function clearForm() {
            $('#taxRateForm')[0].reset();
            $('#taxRateForm').removeClass('was-validated');
            $('#tax_rate_id').val('');
            $('#saveBtn').html('save');
            $('#cancelBtn').hide();
            restoreDefaultOrder();
        ...
        $('#taxRateForm').on('submit', function(e) {
            ...
            var rangeFrom = parseFloat($('#applicable_hotel_range_from').val());
            var rangeTo = parseFloat($('#applicable_hotel_range_to').val());

            // Check for negative values
            if (taxRate < 0 || rangeFrom < 0 || rangeTo < 0) {
                Swal.fire('Error', 'Values cannot be negative!', 'error');
                return false;
            }

            // Check if 'from' is less than 'to'
            if (rangeFrom >= rangeTo) {
                Swal.fire('Error', '"From" range must be less than "To" range!', 'error');
                return false;
            }

            // Check form validity (HTML5 form checks)
            if (this.checkValidity() === false) {
                return false;
            }

            var isConflict = false;
            var currentId = $('#tax_rate_id').val().toString(); // empty string = “add” mode

            table.rows().every(function() {
                var rowData = this.data();
                var existingFrom = parseFloat(rowData.applicable_hotel_range_from);
                var existingTo = parseFloat(rowData.applicable_hotel_range_to);
                var existingId = rowData.tax_rate_id.toString();

                // If we’re editing, skip comparing against the same record:
                if (currentId && currentId === existingId) {
                    return;
                }

                if (!isNaN(existingFrom) && !isNaN(existingTo)) {
                    // Stricter overlap check: even 1 value cannot overlap
                    if (
                        (rangeFrom >= existingFrom && rangeFrom <= existingTo) || // from falls in existing range
                        (rangeTo >= existingFrom && rangeTo <= existingTo) || // to falls in existing range
                        (rangeFrom <= existingFrom && rangeTo >= existingTo) // new range fully overlaps existing
                    ) {
                        isConflict = true;
                        return false; // break
                    }
            ...
            if (isConflict) {
                Swal.fire('Error', 'This hotel‐range overlaps with an existing range!', 'error');
                return false;
            }

            // All validations passed – now submit form (you can use AJAX or trigger submit)
            ...
            var url = tax_rate_id ? "<?= site_url('TaxRate/update') ?>/" + tax_rate_id : "<?= site_url('TaxRate/store') ?>";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp.error) {
                        Swal.fire('Error', JSON.stringify(resp.error), 'error');
                    } else {
                        Swal.fire('Success', resp.success, 'success');
                        clearForm();
                        table.ajax.reload();
                    }
                },
        ...
        // Edit event: Populate form with the selected record data for updating
        $('#taxRateTable').on('click', '.edit', function() {
            var id = $(this).data('id');
            $.ajax({
                url: "<?= site_url('TaxRate/fetchAll') ?>",
                method: "GET",
                ...
                success: function(data) {
                    var record = data.find(item => item.tax_rate_id == id);
                    if (record) {
                        $('#tax_rate_id').val(record.tax_rate_id);
                        $('#tax_rate').val(record.tax_rate_percent);
                        $('#applicable_hotel_range_from').val(record.applicable_hotel_range_from);
                        $('#applicable_hotel_range_to').val(record.applicable_hotel_range_to);
                        $('#saveBtn').html('update');
                        $('#cancelBtn').show();


                        // Rearranging buttons: clear, update, cancel update
                        var container = $("#buttonContainer");
                        $("#saveBtn").detach().prependTo(container);
                        $("#clearBtn").detach().insertAfter($("#saveBtn"));
                        $("#cancelBtn").detach().appendTo(container);

                        $('html, body').animate({
                            scrollTop: $('#taxRateForm').offset().top - 200
                        }, 'fast');
                    }
                },
                error: function() {
        ...
        // Delete event
        $('#taxRateTable').on('click', '.delete', function() {
            var id = $(this).data('id');
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= site_url('TaxRate/delete') ?>/" + id,
                        method: "POST",
                        dataType: "json",
                        success: function(resp) {
                            if (resp.error) {
