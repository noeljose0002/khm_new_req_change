                                ...
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <input type="number" class="form-control" id="enterprise_id" name="enterprise_id">
                                        </div>
                                    </div>
                                    <div class="form-row">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            ...
                            <div class="card-body">
                                <form id="editForm">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <label for="edit_status_name">Status Name</label>
                                            <input type="text" class="form-control" id="edit_status_name" name="status_name" required>
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label for="edit_is_active">Active?</label><br>
                                            <input type="checkbox" id="edit_is_active" name="is_active" value="1">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label for="edit_enterprise_id">Enterprise ID</label>
                                            <input type="number" class="form-control" id="edit_enterprise_id" name="enterprise_id">
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="col text-right">
...
$(document).ready(function() {
    ...
    var table = $('#statusTable').DataTable({
        ajax: {
            url: '<?= site_url("SuperAdminEnquiryStatus/list") ?>',
            dataSrc: 'data'
        },
        columns: [
            { title: 'Status Id',   data: 'status_id' },
            ...
            { title: 'Enterprise Id', data: 'enterprise_id' },
            {
                title: 'Actions',
                data: 'status_id',
                orderable: false,
                render: id => `
                    ...
                    <i class="fas fa-trash-alt action-icon deleteBtn text-danger" data-id="${id}"></i>
                `
            }
        ],
        order: [[0, 'ASC']],
    ...
    $('#addForm').submit(function(e) {
        e.preventDefault();
        $.ajax({
            url: '<?= site_url("SuperAdminEnquiryStatus/create") ?>',
            type: 'POST',
            data: $(this).serialize(),
            success: () => {
                $('#addForm')[0].reset();
                table.ajax.reload();
                Swal.fire('Saved!', 'New status added.', 'success');
            },
            error: xhr => {
                const msg = xhr.responseJSON?.errors?.status_name || 'Failed to save';
    ...
    // SHOW EDIT
    $('#statusTable').on('click', '.editBtn', function() {
        let id = $(this).data('id');
        $.getJSON(`<?= site_url("SuperAdminEnquiryStatus/edit") ?>/${id}`, function(data) {
    ...
    $('#editForm').submit(function(e) {
        ...
        $.ajax({
            url: `<?= site_url("SuperAdminEnquiryStatus/update") ?>/${id}`,
            type: 'POST',
            data: $(this).serialize(),
            success: () => {
                $('#editForm')[0].reset();
            ...
            error: xhr => {
                const msg = xhr.responseJSON?.errors?.status_name || 'Failed to update';
                Swal.fire('Error', msg, 'error');
            }
        });
    });
    ...
    // DELETE
    $('#statusTable').on('click', '.deleteBtn', function() {
        let id = $(this).data('id');
        Swal.fire({
            ...
            if (res.isConfirmed) {
                $.ajax({
                    url: `<?= site_url("SuperAdminEnquiryStatus/delete") ?>/${id}`,
                    type: 'DELETE'
                }).done(() => {
                    table.ajax.reload();
