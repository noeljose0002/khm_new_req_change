                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Edit Request Reason </h4>
...
<head>
  <meta charset="UTF-8">
  <title>Edit Request Reasons</title>
  <!-- Bootstrap 4.1.3 CSS -->
  ...
  <style>
    ...
    /* Action icon styling updated to green */
    .action-icon {
      font-size: 1.2em;
      color: green;
    }
    /* Hover effect on DataTable rows */
    table.dataTable tbody tr:hover {
      background-color: #d4edda !important;
    }
    /* Button group style for form buttons */
    .form-buttons {
      margin-top: 15px;
    }
    .form-buttons button {
...
<body>
  <div class="container">
    ...
    <div class="card">
      <div class="card-header">
        <h4 class="mb-0">Edit Request Reason Form</h4>
      </div>
      <div class="card-body">
        <form id="editReasonForm">
          <!-- Hidden field for ID and Enterprise -->
          <input type="hidden" name="edit_request_reason_id" id="edit_request_reason_id">
          <input type="hidden" name="enterprise_id" id="enterprise_id" value="1">
          <!-- Reason Field -->
          <div class="form-group">
            <label for="edit_request_reason">Reason</label>
            <input type="text" class="form-control" id="edit_request_reason" name="edit_request_reason" placeholder="Enter reason" minlength="2" maxlength="100" required>
          </div>
          <!-- Form buttons for Save/Update and Clear/Cancel -->
    ...
    <div class="card">
      <div class="card-header">
        <h4 class="mb-0">Request Reasons List</h4>
      </div>
      <div class="card-body">
        <table id="editReasonTable" class="table table-bordered table-striped">
          <thead>
            <tr>
              <th>Reason</th>
...
<script>
  ...
  <script>
    $(document).ready(function() {
      // Initialize DataTable with updated action icons
      let table = $('#editReasonTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
          url: "<?= site_url('EditRequestReasonController/fetch') ?>",
          type: "POST",
          dataSrc: ""
        },
        columns: [
          { data: 'edit_request_reason' },
          { 
            data: null,
            width: "120px",
            render: function(data, type, row) {
              return '<i class="fas fa-edit action-icon editAssign" data-id="' + row.edit_request_reason_id + '" data-reason="' + row.edit_request_reason + '" title="Edit" style="margin-right: 10px; cursor: pointer;"></i>' +
                     '<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.edit_request_reason_id + '" title="Delete" style="cursor: pointer;"></i>';
            }
          }
        ]
      });

      // Clear/Cancel button event: Resets form and reverts button texts if in edit mode
      $('#clearButton').on('click', function() {
        resetForm();
      });

      // Form submission for Add/Update with duplicate validation
      $('#editReasonForm').on('submit', function(e) {
        e.preventDefault();
        let reasonVal = $('#edit_request_reason').val().trim();
        if (!reasonVal) {
          Swal.fire('Warning', 'Please fill the reason field!', 'warning');
          return;
        }
        
        // Duplicate Entry Validation (case insensitive)
        let newReason = reasonVal.toLowerCase();
        let currentId = $('#edit_request_reason_id').val();
        let duplicateFound = false;
        table.rows().every(function() {
          let data = this.data();
          if(newReason === data.edit_request_reason.toLowerCase()){
            // If editing, ignore the current row's record
            if(currentId === "" || currentId != data.edit_request_reason_id) {
              duplicateFound = true;
            }
          }
        });
        if(duplicateFound) {
          Swal.fire('Error', 'Duplicate entry. The reason already exists!', 'error');
          return;
        }

        // Submit the form via AJAX
        $.ajax({
          url: "<?= site_url('EditRequestReasonController/save') ?>",
          type: "POST",
          data: $(this).serialize(),
          dataType: "json",
          success: function(response) {
      ...
      // Edit record from datatable - populate static form for editing
      $('#editReasonTable').on('click', '.editAssign', function() {
        let id = $(this).data('id');
        let reason = $(this).data('reason');
        // Populate form fields
        $('#edit_request_reason_id').val(id);
        $('#edit_request_reason').val(reason);
        // Change button texts to indicate update mode
        $('#saveButton').html(' Update');
        $('#clearButton').html(' Cancel');
        // Scroll to the form
        $('html, body').animate({
          scrollTop: $("#editReasonForm").offset().top - 200
        }, 'fast');
      });

      // Delete record
      $('#editReasonTable').on('click', '.deleteAssign', function() {
        let id = $(this).data('id');
        Swal.fire({
          title: 'Are you sure?',
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?= site_url('EditRequestReasonController/delete') ?>",
              type: "POST",
              data: { edit_request_reason_id: id },
              dataType: "json",
              success: function(response) {
                if (response.status === 'success') {
      ...
      // Function to reset form to default add mode
      function resetForm() {
        $('#editReasonForm')[0].reset();
        $('#edit_request_reason_id').val('');
        // Revert button texts back to default add mode
        $('#saveButton').html('<i class="fas fa-save"></i> Save');
        $('#clearButton').html('<i class="fas fa-eraser"></i> Clear');
      }
    });
  </script>