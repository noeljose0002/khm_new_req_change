                            ...
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="object_class_id">Entity Class</label>
                                            <select
                                                class="form-control"
                                                id="object_class_id"
...
$(document).ready(function() {
  ...
  $.getJSON('<?= site_url("SuperAdminObjectBoolean/objClass") ?>', function(res) {
    allClasses = res.data;
    ...
    $('#object_class_id').html(buildOptions(allClasses, '-- Select Class --'));
    // inject into EDIT form
    $('#edit_object_class_id').html(buildOptions(allClasses, '-- Select Class --'));
  });

  // 2) Initialize DataTable
  const table = $('#booleanTable').DataTable({
    ajax: {
      url: '<?= site_url("SuperAdminObjectBoolean/list") ?>',
      dataSrc: 'data'
    },
    columns: [
  ...
  $('#addForm').submit(function(e) {
    e.preventDefault();
    $.post(
      '<?= site_url("SuperAdminObjectBoolean/create") ?>',
      $(this).serialize()
    )
    .done(() => {
      $('#addForm')[0].reset();
  ...
  $('#booleanTable').on('click', '.editBtn', function() {
    ...
    $.getJSON(`<?= site_url("SuperAdminObjectBoolean/edit") ?>/${id}`, function(data) {
      ...
      $('#edit_object_class_id').html(
        allClasses.reduce((h,c) => {
          const sel = c.object_class_id==data.object_class_id ? ' selected' : '';
          return h + `<option value="${c.object_class_id}"${sel}>${c.object_class_name}</option>`;
        }, '<option value="">-- Select Class --</option>')
      );
  ...
  $('#editForm').submit(function(e) {
    ...
    $.ajax({
      url: `<?= site_url("SuperAdminObjectBoolean/update") ?>/${id}`,
      type: 'POST',
      data: $(this).serialize()
    })
    .done(() => {
  ...
  $('#booleanTable').on('click', '.deleteBtn', function() {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        $.ajax({
          url: `<?= site_url("SuperAdminObjectBoolean/delete") ?>/${id}`,
          type: 'DELETE'
        })
        .done(() => {
          table.ajax.reload();
