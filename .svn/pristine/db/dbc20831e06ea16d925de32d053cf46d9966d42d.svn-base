            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Add New Object Primary Type</h4>
                ...
                <head>
                    ...
                    <style>
                        ...
                        #objectPrimaryTypeTable thead th {
                ...
                <body>
                    <div class="container mt-5">
                                ...
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-6">
                                            <label for="object_type_name"> Entity type Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="object_type_name"
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4 justify-content-end align-items-end">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            ...
                            <div class="card-body">
                                <form id="editForm">
                                    ...
                                    <input type="hidden" id="edit_object_type_id" name="object_type_id">

                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <label for="edit_object_type_name">Entity Class Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="edit_object_type_name"
                                                name="object_type_name"
                                                maxlength="100"
                                                required>
                                        </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>Entity Type List</h4>
                            </div>
                            <div class="card-body">
                                <table id="objectPrimaryTypeTable" class="table table-bordered">
                                    <thead>
...
<script>
    $(document).ready(function() {
        ...
        const table = $('#objectPrimaryTypeTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminObjectPrimaryType/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                    data: 'object_type_id',
        ...
        $('#addForm').submit(function(e) {
            ...
            const form = this;
            $.post(
                    '<?= site_url("SuperAdminObjectPrimaryType/create") ?>',
                    $(this).serialize()
                )
                .done(() => {
                    form.reset();
                ...
                .fail(xhr => {
                    const errs = xhr.responseJSON?.error || xhr.responseJSON?.errors;
                    const msg = errs ?
        ...
        // 5) SHOW EDIT
        $('#objectPrimaryTypeTable').on('click', '.editBtn', function() {
            const id = $(this).data('id');

            $.ajax({
                    url: `<?= site_url("SuperAdminObjectPrimaryType/edit") ?>/${id}`,
                    dataType: 'json'
                })
                .then(
                    function success(data) {
                        // fill hidden ID
                        $('#edit_object_type_id').val(data.object_type_id);
                        $('#edit_object_type_name').val(data.object_type_name);
                    ...
                    function error(xhr) {
                        ...
                        const msg = errs ?
                            Object.values(errs).flat()[0] :
                            'Could not load data for editing.';
                        Swal.fire('Error', msg, 'error');
                    }
                );
        ...
        $('#editForm').submit(function(e) {
            e.preventDefault();
            const form = this;
            const id = $('#edit_object_type_id').val();
            $.ajax({
                    url: `<?= site_url("SuperAdminObjectPrimaryType/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .then(
        ...
        // 8) DELETE
        $('#objectPrimaryTypeTable').on('click', '.deleteBtn', function() {
            const id = $(this).data('id');
            Swal.fire({
                title: 'Delete?',
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminObjectPrimaryType/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .then(
                            function success(response) {
                                // assume your controller returns { status: 'success', message: '...' }
                            ...
                            function error(xhr) {
                                // if you ever send back validation errors under `errors`
                                const errs = xhr.responseJSON?.errors;
                                const msg = errs ?
                                    Object.values(errs).flat()[0] :
