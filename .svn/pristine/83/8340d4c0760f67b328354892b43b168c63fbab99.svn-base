  ...
  <script>
    ...
    function initEventHandlers() {
      ...
      $("#addGeogForm").on("submit", function(e) {
        // 1. Run HTML5 validation
        if (!this.checkValidity()) {
          // 2a. Invalid → stop submit, show Bootstrap feedback, then exit
          e.preventDefault();
          e.stopPropagation();
        ...
        // 2b. Valid → proceed with AJAX (but still prevent default full-page submit)
        e.preventDefault();
        e.stopPropagation();
        ...
        if (isDuplicate) {
          Swal.fire("Duplicate Entry",
            "A record with this Geography Name already exists.",
            "warning"
          );
          return;
        }
        ...
        const url = edit ?
          `GeogController/updateGeog/${edit}` :
          "GeogController/insertGeog";
