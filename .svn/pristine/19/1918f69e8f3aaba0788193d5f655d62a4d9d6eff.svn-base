                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Insurance Maintenance</h4>
...
<head>
  <meta charset="UTF-8">
  <title>Insurance Maintenance</title>
  <!-- Bootstrap 4.1.3 CSS -->
  ...
  <!-- Custom CSS for table header, row hover, and action icon green color -->
  <style>
    /* Green header for the DataTable */
    ...
    .action-icon {
      font-size: 1.1em;
      color: green; /* Action icons in green */
    }
    .error {
      color: red;
      font-size: 0.875em;
...
<body>
  ...
  <!-- <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <a class="navbar-brand" href="#">Insurance Maintenance</a>
  </nav> -->
  ...
  <div class="container mt-4">
    ...
    <div class="card mb-4">
      <div class="card-header">
        <h4 class="card-title mb-0">Insurance Form</h4>
      </div>
      <div class="card-body">
        <form id="insuranceForm">
          <!-- Hidden field for insurance_id and enterprise_id -->
          <input type="hidden" name="insurance_id" id="insurance_id" />
          <input type="hidden" name="enterprise_id" id="enterprise_id" value="1" />

          <div class="form-row">
            <div class="form-group col-md-4">
              <label for="pax_count_from">Pax Count From</label>
              <input type="number" class="form-control" id="pax_count_from" name="pax_count_from" required />
            </div>
            <div class="form-group col-md-4">
              <label for="pax_count_to">Pax Count To</label>
              <input type="number" class="form-control" id="pax_count_to" name="pax_count_to" required />
            </div>
            <div class="form-group col-md-4">
              <label for="insurance_amount">Insurance Amount</label>
              <input type="number" step="0.01" class="form-control" id="insurance_amount" name="insurance_amount" required />
            </div>
          </div>
          <button type="submit" class="btn btn-success">Save</button>
          <button type="button" id="clearForm" class="btn btn-success ml-2">Clear</button>
          <!-- New Cancel Button, hidden by default -->
          <button type="button" id="cancelUpdate" class="btn btn-success ml-2" style="display:none;">Cancel</button>
        </form>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title mb-0">Insurance List</h3>
      </div>
      <div class="card-body">
 ...
 <!-- Scripts -->
  ...
  <script>
    $(document).ready(function() {
      // Initialize DataTable with serial no and hidden insurance_id column
      let table = $('#insuranceTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
          url: "<?= site_url('Insurances/fetch') ?>",
          type: "POST",
          dataSrc: ""
        },
        columns: [
          ...
          { data: 'pax_count_from', title: 'Pax Count From' },
          { data: 'pax_count_to', title: 'Pax Count To' },
          { data: 'insurance_amount', title: 'Insurance Amount' },
          {
            data: null,
            title: "Actions",
            render: function(data, type, row) {
              let html = '';
              html += '<i class="fas fa-edit action-icon editAssign" data-id="' + row.insurance_id + '" data-pax_from="' + row.pax_count_from + '" data-pax_to="' + row.pax_count_to + '" data-amount="' + row.insurance_amount + '" title="Edit" style="margin-right: 10px; cursor: pointer; border: none; outline: none;"></i>';
              html += '<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.insurance_id + '" title="Delete" style="cursor: pointer; border: none; outline: none;"></i>';
              return html;
            }
          }
        ]
      ...
      $('#clearForm').on('click', function() {
        ...
        $('#insurance_id').val('');
        // Remove any invalid styling on clear
        $('#pax_count_from, #pax_count_to, #insurance_amount').removeClass('invalid-input');
      });
      ...
      $('#cancelUpdate').on('click', function() {
        $('#insuranceForm')[0].reset();
        $('#insurance_id').val('');
        $('#insuranceForm button[type="submit"]').text('Save');
        $(this).hide();
        $('#pax_count_from, #pax_count_to, #insurance_amount').removeClass('invalid-input');
      });

      // When clicking on edit icon, populate the static form for editing
      $('#insuranceTable').on('click', '.editAssign', function() {
        let insuranceId = $(this).data('id');
        let paxFrom     = $(this).data('pax_from');
        let paxTo       = $(this).data('pax_to');
        let amount      = $(this).data('amount');

        $('#insuranceForm')[0].reset();
        $('#insurance_id').val(insuranceId);
        $('#pax_count_from').val(paxFrom);
        $('#pax_count_to').val(paxTo);
        $('#insurance_amount').val(amount);
        // Change the submit button text to "Update" and show cancel button
        $('#insuranceForm button[type="submit"]').text('Update');
        $('#cancelUpdate').show();
        // Scroll up to the form
        $('html, body').animate({
          scrollTop: $("#insuranceForm").offset().top-200
        },'fast');
      });

      // Delete action using deleteAssign class
      $('#insuranceTable').on('click', '.deleteAssign', function() {
        let insuranceId = $(this).data('id');
        Swal.fire({
          title: 'Are you sure?',
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?= site_url('Insurances/delete') ?>",
              type: "POST",
      ...
      // Dynamic input validation for immediate feedback.
      // For Pax Count From: validate number between 1 and 999
      $('#pax_count_from').on('input', function() {
      ...
      // For Pax Count To: validate number between 1 and 999
      $('#pax_count_to').on('input', function() {
        let val = parseInt($(this).val(), 10);
        if (isNaN(val) || val < 1 || val > 999) {
          $(this).addClass('invalid-input');
        } else {
      ...
      // For Insurance Amount: validate number between 0.01 and 999999999
      $('#insurance_amount').on('input', function() {
        let val = parseFloat($(this).val());
        if (isNaN(val) || val < 0.01 || val > 999999999) {
          $(this).addClass('invalid-input');
        } else {
          $(this).removeClass('invalid-input');
        }
      ...
      // Validate the static form using jQuery Validation Plugin
      $('#insuranceForm').validate({
        ...
        messages: {
          pax_count_from: {
            required: "Please enter a valid Pax Count From",
            number: "Please enter a number",
            min: "Value must be at least 1",
            max: "Maximum 3 digits allowed"
          },
          pax_count_to: {
            required: "Please enter a valid Pax Count To",
            number: "Please enter a number",
            min: "Value must be at least 1",
            max: "Maximum 3 digits allowed"
          },
          insurance_amount: {
            required: "Please enter a valid Insurance Amount",
            number: "Please enter a number",
            min: "Insurance Amount must be at least 0.01",
            max: "Maximum 9 digits allowed"
          }
        },
        submitHandler: function(form) {
          ...
          $.ajax({
  url: "<?= site_url('Insurances/save') ?>", // Correct endpoint
  type: "POST",
  data: $(form).serialize(), // Serializing form data (make sure form input names match backend expectations)
  dataType: "json",
  success: function(response) {
    if (response.status === 'success') {
      // Show success popup
      ...
      // Reset form and UI
      $('#insuranceForm')[0].reset(); // Reset all inputs
      $('#insurance_id').val(''); // Clear hidden field
      $('#insuranceForm button[type="submit"]').text('Save'); // Reset button text
      $('#cancelUpdate').hide(); // Hide cancel button if shown

      // Reload DataTable if used
      if (typeof table !== 'undefined') {
        table.ajax.reload(null, false); // false = don't reset pagination
      }

    } else {
      // Show error returned from controller
      Swal.fire('Error', response.message, 'error');
    }
  },
  error: function(xhr, status, error) {
    // Catch AJAX/network errors
