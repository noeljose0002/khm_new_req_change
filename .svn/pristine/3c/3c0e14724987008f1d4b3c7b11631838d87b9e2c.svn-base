				...
				<div class="section">
					...
					<div class="page-header">
						<div class="page-leftheader">
							<h4 class="page-title mb-0">Add New Object Attribute</h4>
				...
				<body>
					<div class="container mt-5">
								...
								<form id="addForm">
									<div class="form-row">
										...
										<div class="form-group col-md-4">
											<label for="object_mst_attribute_name">object Attribute Name</label>
											<input
												type="text"
												class="form-control"
												id="object_mst_attribute_name"
												name="object_mst_attribute_name"
												maxlength="50"
												required>
										</div>
										...
										<div class="form-group col-md-4">
											<label for="object_class_id">Object Class</label>
											<select
												class="form-control"
												id="object_class_id"
										...
										<div class="form-group col-md-4">
											<label for="object_mst_attribute_data_type">Entity Attribute Data Type</label>
											<select
												class="form-control"
												id="object_mst_attribute_data_type"
												name="object_mst_attribute_data_type">
						...
						<div id="editSection" class="card" style="display:none;">
							<div class="card-header form-header">
								<h4>Edit Entity Attribute</h4>
							</div>
							<div class="card-body">
								<form id="editForm">
									<input type="hidden" name="_method" value="PUT">
									<input type="hidden" id="edit_object_mst_attribute_id" name="object_mst_attribute_id">

									<div class="form-row">

										<div class="form-group col-md-4">
											<label for="edit_object_mst_attribute_name">Entity Attribute Name</label>
											<input
												type="text"
												class="form-control"
												id="edit_object_mst_attribute_name"
												name="object_mst_attribute_name"
												maxlength="50"
												required>
										</div>
										...
										<div class="form-group col-md-4">
											<label for="edit_object_class_id">Entity Class</label>
											<select
												class="form-control"
												id="edit_object_class_id"
												name="object_class_id">
												<option value="">-- Select Class --</option>
											</select>
										</div>
										...
										<div class="form-group col-md-4">
											<label for="edit_object_mst_attribute_data_type">Entity Attribute Data Type</label>
											<select
												class="form-control"
												id="edit_object_mst_attribute_data_type"
												name="object_mst_attribute_data_type">
												<option value="">-- Select Data Type --</option>
											</select>
										</div>
						...
						<div class="card mt-4">
							<div class="card-header">
								<h4>Object Attribute List</h4>
							</div>
							<div class="card-body">
								<table id="objectAttributeTable" class="table table-bordered">
									<thead></thead>
									<tbody></tbody>
								</table>
							</div>
...
<script>
	$(document).ready(function() {
		...
		// 1) Define MySQL data types & helper
		const allDataTypes = [
			...
			'tinytext', 'text', 'mediumtext', 'longtext','select_hotel_category','select_hotel_agency','select_rating','textarea',
			'enum', 'set', 'select_state', 'select_vehicle', 'select_hotel',
			'geometry', 'point', 'linestring', 'polygon',
			'multipoint', 'multilinestring', 'multipolygon', 'geometrycollection'
		];


		function buildOptions(arr, noneLabel, selectedVal = null) {
		...
		// 2) Fetch entity classes once, populate BOTH forms
		$.getJSON('<?= site_url("SuperAdminObjectAttribute/objClass") ?>', function(res) {
			allClasses = res.data;

			// build class‑ID dropdown for ADD + EDIT
			const classOpts = (sel) => {
				let h = `<option value="">-- Select Class --</option>`;
				allClasses.forEach(o => {
					const s = ('' + o.object_class_id === '' + sel) ? ' selected' : '';
					h += `<option value="${o.object_class_id}"${s}>${o.object_class_name}</option>`;
				});
				return h;
			};

			// inject into ADD form
			$('#object_class_id').html(classOpts(null));
			// inject into EDIT form
			$('#edit_object_class_id').html(classOpts(null));

			// inject data‑types into ADD form
			$('#object_mst_attribute_data_type').html(buildOptions(allDataTypes, '-- Select Data Type --'));
			// inject data‑types into EDIT form
			$('#edit_object_mst_attribute_data_type').html(buildOptions(allDataTypes, '-- Select Data Type --'));
		});

		// 3) Initialize DataTable
		const table = $('#objectAttributeTable').DataTable({
			ajax: {
				url: '<?= site_url("SuperAdminObjectAttribute/list") ?>',
				dataSrc: 'data'
			},
			columns: [{
					data: 'object_mst_attribute_id',
					title: 'ID'
				},
				{
					data: 'object_mst_attribute_name',
					title: 'Name'
				},
				{
					data: 'object_class_id',
					title: 'Class',
					render: cid => {
						const cls = allClasses.find(c => '' + c.object_class_id === '' + cid);
						return cls ? cls.object_class_name : '';
					}
				},
				{
					data: 'enterprise_id',
				...
				{
					data: 'object_mst_attribute_data_type',
					title: 'Data Type'
				},
				{
					title: 'Action',
		...
		$('#addForm').submit(function(e) {
			e.preventDefault();
			$.post('<?= site_url("SuperAdminObjectAttribute/create") ?>', $(this).serialize())
				.done(() => {
					$('#addForm')[0].reset();
					table.ajax.reload();
					Swal.fire('Saved!', 'Attribute added.', 'success');
				})
				.fail(xhr => Swal.fire('Error', xhr.responseJSON?.error || 'Failed', 'error'));
		});

		// 6) SHOW EDIT
		$('#objectAttributeTable').on('click', '.editBtn', function() {
			const id = $(this).data('id');
			$.getJSON(`<?= site_url("SuperAdminObjectAttribute/edit") ?>/${id}`, function(data) {
				// fill hidden ID
				$('#edit_object_mst_attribute_id').val(data.object_mst_attribute_id);
				// fill other fields
				$('#edit_object_mst_attribute_name').val(data.object_mst_attribute_name);
				$('#edit_enterprise_id').val(data.enterprise_id);

				// rebuild class dropdown with selection
				let ch = `<option value="">-- Select Class --</option>`;
				allClasses.forEach(o => {
					const s = ('' + o.object_class_id === '' + data.object_class_id) ? ' selected' : '';
					ch += `<option value="${o.object_class_id}"${s}>${o.entity_class_name}</option>`;
				});
				$('#edit_object_class_id').html(ch);

				// rebuild data‑type dropdown with selection
				$('#edit_object_mst_attribute_data_type')
					.html(buildOptions(allDataTypes, '-- Select Data Type --', data.object_mst_attribute_data_type));
		...
		$('#editForm').submit(function(e) {
			e.preventDefault();
			const id = $('#edit_object_mst_attribute_id').val();
			$.ajax({
					url: `<?= site_url("SuperAdminObjectAttribute/update") ?>/${id}`,
					type: 'POST',
					data: $(this).serialize()
				})
				.done(() => {
					...
					$('#addSection').show();
					table.ajax.reload();
					Swal.fire('Updated!', 'Attribute updated.', 'success');
				})
				.fail(xhr => Swal.fire('Error', xhr.responseJSON?.error || 'Failed', 'error'));
		});

		// 9) DELETE
		$('#objectAttributeTable').on('click', '.deleteBtn', function() {
			const id = $(this).data('id');
			Swal.fire({
				title: 'Delete?',
				...
				if (res.isConfirmed) {
					$.ajax({
							url: `<?= site_url("SuperAdminObjectAttribute/delete") ?>/${id}`,
							type: 'DELETE'
						})
						.done(() => {
							table.ajax.reload();
							Swal.fire('Deleted!', 'Attribute removed.', 'success');
						});
				}
			});
		});
