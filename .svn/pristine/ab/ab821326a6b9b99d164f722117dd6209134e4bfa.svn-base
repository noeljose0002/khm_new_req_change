    ...
    $(document).ready(function() {
        ...
        //var lunchValue = parseInt($('#lunch1').val()) || 0;
            ...
            var room_id = data.room_id;
                 var roomCheckbox = $('.chk_room[value="' + room_id + '"]');
            if (!roomCheckbox.is(':checked')) {
                return true; // Skip this room if checkbox is not checked
            }
...
<script>
$(document).ready(function () {
  // 1) Grab your tariff data once
  var tariff_datas = <?php echo json_encode($tariff_datas, JSON_HEX_TAG); ?>;

  // 2) Listen only for CP‐field changes
  //    (you can keep using .meal_class if you like, but we’ll bail out early
  //     unless the ID really starts with "CP_")
  $(document).on("input", ".meal_class", function () {
    // parse the “mealType” straight from data‑id
    var rawId   = $(this).data("id");    // e.g. "CP_1_8347" or "CP_1_8347_w"
    var parts   = rawId.split("_");
    var mealType= parts[0];              // "CP", "MAP", or "AP"
    if (mealType !== "CP") {
      // if they edited MAP or AP, do nothing
      return;
    }

    // OK — it's a CP edit, so do the calc
    var part    = parts[1];
    var room_id = parts[2];
    var suffix  = parts.length > 3 ? "_w" : "";

    var cpValue     = parseInt($("#" + rawId).val(),                10) || 0;
    var dinnerValue = parseInt($("#dinner" + part + suffix).val(), 10) || 0;
    var lunchValue  = parseInt($("#lunch"  + part + suffix).val(), 10) || 0;

    // lookup occupancy
     var roomData = tariff_datas.find(d => d.room_id.toString() === room_id.toString()) || {};
  var occ      = parseInt(roomData.max_occupancy, 10);
  var maxOcc   = (isNaN(occ) || occ < 1) ? 1 : occ;

  // **NEW**: only multiply by occupancy on part 1
  var occFactor = (part === "1" ? maxOcc : 1);

  var totalMAP = (dinnerValue * occFactor) + cpValue;
  var totalAP  = ((dinnerValue + lunchValue) * occFactor) + cpValue;

    // set them back
    $("#MAP_" + part + "_" + room_id + suffix).val(totalMAP);
    $("#AP_"  + part + "_" + room_id + suffix).val(totalAP);
  });
});
...
$(document).ready(function () {
    ...
    // Flag to prevent recursive triggering during auto-fill
    var isAutoFilling = false;
    
    // Get the first room ID (assuming tariff_datas is ordered and first item is the first row)
    var firstRoomId = tariff_datas.length > 0 ? tariff_datas[0].room_id.toString() : null;

    // Listen for CP field changes 
    $(document).on("input", ".meal_class", function () {
        // Skip if we're currently auto-filling to prevent recursion
        if (isAutoFilling) {
            return;
        }
        ...
        var parts = rawId.split("_");
        var mealType = parts[0];
        
        if (mealType !== "CP") {
            return; // Only handle CP changes
        }

        var part = parts[1];      // 1, 2, 3, or 4
        var room_id = parts[2];   // room ID
        var isWeekend = parts.length > 3 && parts[3] === "w";
        var suffix = isWeekend ? "_w" : "";
        
        // Check if this is the first row CP input
        var isFirstRowInput = (room_id.toString() === firstRoomId);
        
        // Adult tariff (part 1) - only manual entry, no auto-fill
        if (part === "1") {
            // Just do normal calculation for this specific room
            var cpValue = parseInt($(this).val(), 10) || 0;
            var dinnerValue = parseInt($("#dinner" + part + suffix).val(), 10) || 0;
            var lunchValue = parseInt($("#lunch" + part + suffix).val(), 10) || 0;
            ...
            var occ = parseInt(roomData.max_occupancy, 10);
            var maxOcc = (isNaN(occ) || occ < 1) ? 1 : occ;
            var occFactor = maxOcc; // Adult tariff uses max occupancy
        ...
        // For columns 2, 3, 4 - check if this is first row input
        if (isFirstRowInput && (part === "2" || part === "3" || part === "4")) {
            // This is first row input for columns 2, 3, or 4 - auto-fill down this column immediately
            var enteredValue = parseInt($(this).val(), 10) || 0;
            
            // Set auto-filling flag to prevent recursion
            isAutoFilling = true;
            
            $.each(tariff_datas, function(index, data) {
                var currentRoomId = data.room_id;
                var roomType = data.room_type_name;
                
                // Skip Single room types
                if (roomType === "Single") {
                    return true; // continue to next iteration
                }
                
                // Check if this room's checkbox is checked
                var roomCheckbox = $('.chk_room[value="' + currentRoomId + '"]');
                if (!roomCheckbox.is(':checked')) {
                    return true; // skip unchecked rooms
                }
                
                // Skip the field that was originally edited
                if (currentRoomId.toString() === room_id.toString()) {
                    // Just calculate MAP and AP for the original field
                    var dinnerValue = parseInt($("#dinner" + part + suffix).val(), 10) || 0;
                    var lunchValue = parseInt($("#lunch" + part + suffix).val(), 10) || 0;
                    
                    var roomData = tariff_datas.find(d => d.room_id.toString() === currentRoomId.toString()) || {};
                    var occ = parseInt(roomData.max_occupancy, 10);
                    var maxOcc = (isNaN(occ) || occ < 1) ? 1 : occ;
                    var occFactor = 1; // Non-adult columns use factor of 1
                    
                    var totalMAP = (dinnerValue * occFactor) + enteredValue;
                    var totalAP = ((dinnerValue + lunchValue) * occFactor) + enteredValue;
                    
                    $("#MAP_" + part + "_" + currentRoomId + suffix).val(totalMAP);
                    $("#AP_" + part + "_" + currentRoomId + suffix).val(totalAP);
                    return true;
                }
                
                // Auto-fill CP value for this room in this specific column immediately
                var targetCpId = "CP_" + part + "_" + currentRoomId + suffix;
                $("#" + targetCpId).val(enteredValue);
                
                // Also trigger the existing calculation logic for MAP and AP
                var dinnerValue = parseInt($("#dinner" + part + suffix).val(), 10) || 0;
                var lunchValue = parseInt($("#lunch" + part + suffix).val(), 10) || 0;
            ...
            // Reset auto-filling flag
            isAutoFilling = false;
            
        } else {
            // This is not first row input - do normal calculation for this specific room only
            var cpValue = parseInt($(this).val(), 10) || 0;
            var dinnerValue = parseInt($("#dinner" + part + suffix).val(), 10) || 0;
            var lunchValue = parseInt($("#lunch" + part + suffix).val(), 10) || 0;
