                ...
                <body>
                    <div class="container mt-5">
                            ...
                            <div class="card-body">
                                <form id="addForm">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="object_class_parent_id">Object Parent Class</label>
                                            <select
                                                class="form-control"
                                                id="object_class_parent_id"
                                                name="object_class_parent_id">
                        ...
                        <div id="editSection" class="card" style="display:none;">
                            <div class="card-header form-header">
                                <h4>Edit Object Class</h4>
                            </div>
                            <div class="card-body">
                                <form id="editForm">
                                    <input type="hidden" name="_method" value="PUT">
                                    <!-- hidden field to hold the PK -->
                                    <input type="hidden" id="edit_object_class_id " name="object_class_id ">

                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label for="edit_object_class_name">Object Class Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="edit_object_class_name"
                                                name="object_class_name"
                                                maxlength="50"
                                                required>
                                        </div>
                                        ...
                                        <div class="form-group col-md-4">
                                            <label for="edit_object_class_parent_id">Object Parent Class</label>
                                            <select
                                                class="form-control"
                                                id="edit_object_class_parent_id"
                                                name="object_class_parent_id">
                                                <option value="">-- Select Class --</option>
                                                <!-- Populate options dynamically -->
                                            </select>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>Object Class List</h4>
                            </div>
                            <div class="card-body">
                                <table id="objectClassTable" class="table table-bordered">
                                    <thead>
...
<script>
    $(document).ready(function() {
        ...
        // 1) Fetch object classes for the dropdowns
        $.getJSON('<?= site_url("SuperAdminObjectClass/list") ?>', function(res) {
            allClasses = res.data;
            const buildOptions = (arr, noneLabel, selected) => {
                let html = `<option value="">${noneLabel}</option>`;
                arr.forEach(o => {
                    const sel = ('' + o.object_class_id === '' + selected) ? ' selected' : '';
                    html += `<option value="${o.object_class_id}"${sel}>${o.object_class_name}</option>`;
                });
                return html;
            };
            // inject into ADD form
            $('#object_class_parent_id').html(buildOptions(allClasses, '-- Select Class --'));
            // inject into EDIT form
            $('#edit_object_class_parent_id').html(buildOptions(allClasses, '-- Select Class --'));
        });

        // 2) Initialize DataTable
        const table = $('#objectClassTable').DataTable({
            ajax: {
                url: '<?= site_url("SuperAdminObjectClass/list") ?>',
                dataSrc: 'data'
            },
            columns: [{
                ...
                {
                    data: 'object_class_parent_id',
                    title: 'Object Class Parent',
                    render: function(cid) {
                        // Look up a class whose object_class_id equals the parent‑ID from this row:
                        const cls = allClasses.find(c => '' + c.object_class_id === '' + cid);
                        // If found, return its name; otherwise blank:
        ...
        $('#addForm').submit(function(e) {
            e.preventDefault();
            $.post(
                    '<?= site_url("SuperAdminObjectClass/create") ?>',
                    $(this).serialize()
                )
                .done(() => {
                    $('#addForm')[0].reset();
                ...
                .fail(xhr => {
                    Swal.fire('Error', xhr.responseJSON?.errors?.object_class_name || 'Failed', 'error');
                });
        });

        // 5) SHOW EDIT
        $('#objectClassTable').on('click', '.editBtn', function() {
            const id = $(this).data('id');

            $.getJSON(`<?= site_url("SuperAdminObjectClass/edit") ?>/${id}`, function(data) {
                // Fill the form fields
                $('#edit_object_class_id').val(data.object_class_id);
                $('#edit_object_class_name').val(data.object_class_name);
                $('#edit_enterprise_id').val(data.enterprise_id);
                $('#edit_deleted').prop('checked', data.deleted == 1);

                // Rebuild the parent class dropdown
                const parentOptions = allClasses.reduce((html, item) => {
                    const selected = item.object_class_id == data.object_class_parent_id ? ' selected' : '';
                    return html + `<option value="${item.object_class_id}"${selected}>${item.object_class_name}</option>`;
                }, '<option value="">-- Select Class --</option>');

                $('#edit_object_class_parent_id').html(parentOptions);
        ...
        $('#editForm').submit(function(e) {
            ...
            $.ajax({
                    url: `<?= site_url("SuperAdminObjectClass/update") ?>/${id}`,
                    type: 'POST',
                    data: $(this).serialize()
                })
                .done(() => {
        ...
        // 8) DELETE
        $('#objectClassTable').on('click', '.deleteBtn', function() {
            const id = $(this).data('id');
            Swal.fire({
                title: 'Delete?',
                ...
                if (res.isConfirmed) {
                    $.ajax({
                            url: `<?= site_url("SuperAdminObjectClass/delete") ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(() => {
                            table.ajax.reload();
