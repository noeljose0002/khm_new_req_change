...
<body class="app sidebar-mini">
    ...
    <div class="page">
        <div class="page-main">
            ...
            <div class="app-content icon-content">
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">HUB LOCATIONS</h4>
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Hub Locations Management</title>
                    <!-- Bootstrap 4.1.3 CSS -->
                    ...
                    <script>
                        // Define base_url for AJAX endpoints (ensure it ends with a slash)
    ...
    margin-right: 5.25rem !important;
                        ...
                        /* #hubLocationsTable tbody tr:hover {
      background-color: #d4edda !important;
    } */
                    </style>
                </head>
                ...
                <body>
                    <div class="container mt-4">
                        <!-- Form Container -->
                        <div class="card mb-4 shadow" id="hubLocationFormContainer">
                            <div class="card-header">
                                <h4 class="mb-0">Manage Hub Location</h4>
                            </div>
                            <div class="card-body">
                                <!-- Using native HTML5 validation -->
                                <form id="hubLocationForm">
                                    <input type="hidden" id="original_hub_name" name="original_hub_name" value="">
                                    <!-- Hidden field for editing -->
                                    <input type="hidden" name="hub_location_id" id="hub_location_id">
                                    <!-- Hidden Enterprise ID (adjust value as needed) -->
                                    <input type="hidden" name="enterprise_id" id="enterprise_id" value="1">

                                    <div class="form-row">
                                        <!-- Geography Selection -->
                                        <div class="col-md-3 form-group">
                                            <label for="geog_select">Geography</label>
                                            <select class="form-control" id="geog_select" name="geog_id" required>
                                                <option value="">Select Geography</option>
                                                <!-- Options to be populated via AJAX -->
                                            </select>
                                        </div>

                                        <!-- Location Name with pattern for letters/spaces/parentheses only -->
                                        <div class="col-md-3 form-group">
                                            <label for="hub_location_name">Hub Location Name</label>
                                            <input
                                                type="text"
                                                class="form-control"
                                                id="hub_location_name"
                                                name="hub_location_name"
                                                placeholder="Location Name"
                                                required
                                                maxlength="50"
                                                pattern="^(?!.*\d)[A-Za-z][A-Za-z ()]{1,49}$"
                                                title="2-50 characters, starting with a letter. Only letters, spaces, and parentheses are allowed.">
                                        </div>

                                        <!-- Latitude (Auto-filled) with a max for demonstration -->
                                        <div class="col-md-3 form-group">
                                            <label for="hub_latitude">Latitude</label>
                                            <input
                                                type="number"
                                                step="any"
                                                class="form-control"
                                                ...
                                                readonly
                                                max="99"
                                                title="Must be less than or equal to 99.">
                                        </div>

                                        <!-- Longitude (Auto-filled) with a max for demonstration -->
                                        <div class="col-md-3 form-group">
                                            <label for="hub_longitude">Longitude</label>
                                            <input
                                                type="number"
                                                step="any"
                                                class="form-control"
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-auto text-right">
                                            <button type="submit"
                                                ...
                                                id="saveHubLocationBtn">
                                                Save
                                            </button>
                                        </div>
                                        <div class="col-auto text-left" id="extraButtons" style="display: none;">
                                            <button type="button"
                                                class="btn btn-success mx-1"
                                                id="clearBtn">
                                                Clear
                                    ...
                                    <!-- Extra Buttons for Update Mode (hidden by default) -->
                    ...
                    <div class="card shadow">
                        <div class="card-header">
                            <h4 class="mb-0">Hub Locations List</h4>
                        </div>
                        <div class="card-body">
                            <table id="hubLocationsTable" class="table table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
                                        <th>Hub Location Name</th>
                                        <th>Location Name</th>
                                        <th>Latitude</th>
                                        <th>Longitude</th>
...
<script>
  ...
  $(document).ready(function() {
    ...
    var hubTable = $('#hubLocationsTable').DataTable({
      ajax: {
        url: Site_url + '/HubLocation/list',
        dataSrc: function(json) {
          json.forEach((row, idx) => row.original_index = idx + 1);
          return json;
        }
      },
      columnDefs: [
      ...
      order: [],
      columns: [
        { data: null, title: "S.No", render: (d, t, r) => r.original_index },
        { data: 'hub_location_name', title: "Hub Location Name" },
        { data: 'geog_name', title: "Location Name" },
        { data: 'hub_latitude', title: "Latitude" },
        { data: 'hub_longitude', title: "Longitude" },
        {
          data: null,
          title: "Actions",
          className: 'text-center',
          render: (d, t, r) => `
            <i class="fas fa-edit action-icon editAssign" data-id="${r.hub_location_id}" title="Edit" style="margin-right:10px;cursor:pointer;"></i>
            <i class="fas fa-trash action-icon deleteAssign" data-id="${r.hub_location_id}" title="Delete" style="cursor:pointer;"></i>
          `
        }
      ]
    });

    // Form submit: always via AJAX
    $('#hubLocationForm').on('submit', function(e) {
      e.preventDefault(); // never fall back to normal POST
      ...
      var hubName      = $('#hub_location_name').val().trim().toLowerCase();
      var originalName = $('#original_hub_name').val().trim().toLowerCase();

      // only check duplicates if the name changed
      if (hubName !== originalName) {
        var duplicate = false;
        hubTable.rows().every(function() {
          if (this.data().hub_location_name.trim().toLowerCase() === hubName) {
            duplicate = true;
            return false; // break
          }
        });
        if (duplicate) {
          return Swal.fire('Warning', 'Duplicate hub location name detected. Please enter a unique name.', 'warning');
        }
      }

      var hubId = $('#hub_location_id').val();
      $.post(Site_url + '/HubLocation/save', $(this).serialize(), function(response) {
        if (response.success) {
          Swal.fire({
            title: 'Success',
            text: hubId ? 'Record updated successfully.' : 'Record saved successfully.',
            icon: 'success',
            timer: 1500,
    ...
    // Edit button click
    $('#hubLocationsTable tbody').on('click', '.editAssign', function() {
      var data = hubTable.row($(this).closest('tr')).data();
      $('#hub_location_id').val(data.hub_location_id);
      $('#hub_location_name').val(data.hub_location_name);
      $('#original_hub_name').val(data.hub_location_name);    // <-- set original
      $('#geog_select').val(data.geog_id).trigger('change');
      $('#hub_latitude').val(parseFloat(data.hub_latitude).toFixed(8) || '');
      $('#hub_longitude').val(parseFloat(data.hub_longitude).toFixed(8) || '');
      $('#saveHubLocationBtn').text('Update');
      $('#extraButtons').show();
      $('html,body').animate({
        scrollTop: $('#hubLocationFormContainer').offset().top - 200
      }, 'fast');
    });

    // Delete button click
    $('#hubLocationsTable tbody').on('click', '.deleteAssign', function() {
      var id = $(this).data('id');
      Swal.fire({
        title: 'Are you sure?',
        text: "This will perform a soft delete on the record.",
        icon: 'warning',
        ...
        if (res.isConfirmed) {
          $.post(Site_url + '/HubLocation/delete', { hub_location_id: id }, function(resp) {
            if (resp.success) {
          ...
          }, 'json').fail(() => Swal.fire('Error', 'AJAX request failed.', 'error'));
        }
      });
    });

    // Load and filter geographies
    function loadGeographies() {
      $.getJSON(Site_url + '/GeogController/getGeogs', function(response) {
        var opts = '<option value="">Select Geography</option>';
        response.forEach(item => {
          if (item.geog_level_id == 4) {
            opts += `<option value="${item.geog_id}" data-lat="${item.geog_latitude}" data-long="${item.geog_longitude}">${item.geog_name}</option>`;
          }
        });
        $('#geog_select').html(opts);
      }).fail(() => Swal.fire('Error', 'Failed to load geography data.', 'error'));
    }
    loadGeographies();

    // Auto-fill lat/long on geography change
    $('#geog_select').on('change', function() {
      var sel = $(this).find(':selected');
      var lat = parseFloat(sel.data('lat'));
      var lng = parseFloat(sel.data('long'));
      $('#hub_latitude').val(isNaN(lat) ? '' : lat.toFixed(8));
      $('#hub_longitude').val(isNaN(lng) ? '' : lng.toFixed(8));
    });

    // Clear & Cancel buttons
    $('#clearBtn').click(() => {
      $('#hub_location_name').val('');
      $('#geog_select').val('').trigger('change');
      $('#hub_latitude,#hub_longitude').val('');
    });
    $('#cancelBtn').click(resetForm);

    function resetForm() {
      $('#hubLocationForm')[0].reset();
      $('#hub_location_id,#original_hub_name').val('');
      $('#saveHubLocationBtn').text(' Save');
      $('#extraButtons').hide();
    }

    // Prevent digits in hub name
    $('#hub_location_name').on('input', function() {
