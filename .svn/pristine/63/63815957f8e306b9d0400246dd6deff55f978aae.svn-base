                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Bank Account Form</h4>
...
<head>
    <meta charset="UTF-8">
    <title>Bank Accounts Maintenance</title>
    ...
    <style>
      ...
      /* Row hover effect: light green */
      /* #bankAccountsTable tbody tr:hover {
        background-color: #d4edda !important;
      } */
      /* Action icons: now green and slightly larger */
      .action-icon {
        font-size: 1.1em;
        color: green;
      }
      ...
      /* Style for invalid input fields: red border */
      .invalid-input {
        border-color: red !important;
      }
      /* jQuery Validate error messages in red */
      label.error {
        color: red;
      }
    </style>
...
<div class="container mt-4">
  ...
  <div class="card mb-4">
    <div class="card-header">
      <h3 class="card-title mb-0">Bank Account Form</h3>
    </div>
    <div class="card-body">
      <form id="bankAccountForm">
        <!-- Hidden field for bank account id -->
        <input type="hidden" name="bank_accounts_id" id="bank_accounts_id" />
        <!-- Enterprise id remains static -->
        <input type="hidden" name="enterprise_id" id="enterprise_id" value="1" />

        <div class="form-group">
  <label for="bank_name">Bank Name</label>
  <!-- maxlength attribute for browser enforcement -->
  <input type="text" class="form-control" id="bank_name" name="bank_name" placeholder="Enter Bank Name" required maxlength="100" pattern="[A-Za-z\s]+" title="Bank name must contain only letters and spaces.">
</div>


        <div class="form-group">
          <label for="account_type">Account Type</label>
          <input type="text" class="form-control" id="account_type" name="account_type" placeholder="Enter Account Type"  pattern="[A-Za-z\s]+" required maxlength="150">
        </div>

        <div class="form-group">
          <label for="account_name">Account Name</label>
          <input type="text" class="form-control" id="account_name" name="account_name" placeholder="Enter Account Name"  pattern="[A-Za-z\s]+" required maxlength="150">
        </div>

        <div class="form-group">
          <label for="account_no">Account No</label>
          <!-- Although type="number" does not enforce maxlength, we validate dynamically -->
          <input type="number" class="form-control" id="account_no" name="account_no" placeholder="Enter Account No" required>
        </div>
        ...
        <button type="button" class="btn btn-success" id="btnCancel" style="display: none;">Cancel</button>
      </form>
    </div>
  </div>
  <!-- End of Static Form Card -->

  <!-- Header without Add New Button -->
  <!-- <div class="d-flex justify-content-between align-items-center mb-3">
    <h3>Bank Accounts Maintenance</h3>
  </div> -->

  <!-- DataTable Card -->
  <div class="card">
    <div class="card-header">
      <h3 class="card-title mb-0">Bank Accounts List</h3>
    </div>
    <div class="card-body">
...
<script>
// Custom validator for letters only (allows spaces)
...
$(document).ready(function() {
    ...
    // For text fields: bank_name, account_type, account_name
    $('#bank_name, #account_type, #account_name').on('input', function() {
        let maxLen = 150;
        ...
        // Also check letters-only dynamically; if non-letters found, mark as invalid.
        let regex = /^[a-zA-Z\s]*$/;
        if (!regex.test($(this).val())) {
            $(this).addClass('invalid-input');
        } else if ($(this).val().length <= maxLen) {
            $(this).removeClass('invalid-input');
        }
    });

    // For account_no: validate that the number does not exceed 20 digits
    ...
    // Initialize DataTable with serial number column and hidden bank_accounts_id
    let table = $('#bankAccountsTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
            url: "<?= site_url('BankAccounts/fetch') ?>",
            type: "POST",
            dataSrc: ""
        },
        columns: [
            ...
            { data: 'account_no' },
            {
                ...
                render: function(data, type, row) {
                    var html = '';
                    html += '<i class="fas fa-edit action-icon editAssign" data-id="' + row.bank_accounts_id + '" data-bank_name="' + row.bank_name + '" data-account_type="' + row.account_type + '" data-account_name="' + row.account_name + '" data-account_no="' + row.account_no + '" title="Edit" style="margin-right: 10px; cursor: pointer; border: none; outline: none;"></i>';
                    html += '<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.bank_accounts_id + '" title="Delete" style="cursor: pointer; border: none; outline: none;"></i>';
                    return html;
                }
            }
        ]
    ...
    // Clear button: only clears the visible text and number fields and removes invalid styling
    $('#btnClear').on('click', function() {
        $('#bankAccountForm').find('input[type="text"], input[type="number"]').val('');
        $('#bankAccountForm').find('input').removeClass('invalid-input');
    });

    // Cancel button action: reset the form state to default and remove invalid styling
    ...
    // Edit record: fill the static form with data from the selected row and update the form buttons
    $('#bankAccountsTable').on('click', '.editAssign', function() {
        let rowData = table.row($(this).parents('tr')).data();
        $('#bank_accounts_id').val(rowData.bank_accounts_id);
        $('#bank_name').val(rowData.bank_name);
        $('#account_type').val(rowData.account_type);
        $('#account_name').val(rowData.account_name);
        $('#account_no').val(rowData.account_no);
        $('#btnSubmit').text('Update');
        $('#btnCancel').show();
        $('html, body').animate({
          scrollTop: $("#bankAccountForm").offset().top -200
        }, 'fast');
    });

    // Delete record
    $('#bankAccountsTable').on('click', '.deleteAssign', function() {
        let bankAccountsId = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
            text: "This will soft-delete the bank account.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= site_url('BankAccounts/delete') ?>",
                    type: "POST",
                    data: { bank_accounts_id: bankAccountsId },
                    dataType: "json",
                    success: function(response) {
    ...
    // Implement form validation and submission using jQuery Validate
    $('#bankAccountForm').validate({
        ...
        messages: {
            bank_name: {
                ...
                lettersOnly: "Bank name must contain alphabets only",
                maxlength: "Bank name must not exceed 150 characters"
            },
            account_type: {
                required: "Please enter the account type",
                lettersOnly: "Account type must contain alphabets only",
                maxlength: "Account type must not exceed 150 characters"
            },
            account_name: {
                required: "Please enter the account name",
                lettersOnly: "Account name must contain alphabets only",
                maxlength: "Account name must not exceed 150 characters"
            },
            account_no: {
                required: "Please enter the account number",
                digits: "Enter a valid number",
                min: "Account number must be at least 1",
                maxlength: "Account number must not exceed 20 digits"
            }
        },
        submitHandler: function(form) {
            // Optionally, check for any fields still marked as invalid
            if ($('.invalid-input').length > 0) {
                Swal.fire('Warning', 'Please correct the highlighted fields.', 'warning');
                return;
            }
            $.ajax({
                url: "<?= site_url('BankAccounts/save') ?>",
                type: "POST",
                data: $(form).serialize(),
                dataType: "json",
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Reset the form state after save/update
                        $('#bankAccountForm')[0].reset();
                        $('#bank_accounts_id').val('');
                        $('#btnSubmit').text('Save');
                        $('#btnCancel').hide();
