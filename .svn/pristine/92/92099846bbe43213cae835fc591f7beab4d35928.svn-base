                ...
                <div class="section">
                    ...
                    <div class="page-header">
                        <div class="page-leftheader">
                            <h4 class="page-title mb-0">Markups</h4>
...
<head>
  <meta charset="UTF-8">
  <title>MarkUp Maintenance</title>
  <!-- Bootstrap 4.1.3 CSS -->
  ...
  <!-- Custom CSS for table header and row hover -->
  <style>
    ...
    /* Optional: visual feedback for invalid "system" field */
    .invalid-input {
...
<body>
  <div class="container mt-4">
    <!-- Static Form for Add/Edit -->
    <div class="card mb-4">
      <div class="card-header">
        <h3 class="card-title mb-0" id="formTitle">Add MarkUp</h3>
      </div>
      <div class="card-body">
        <form id="markupForm">
          <!-- Hidden fields -->
          <input type="hidden" name="mark_up_id" id="mark_up_id" />
  ...
  <select class="form-control" id="system" name="system" required>
    ...
    <option value="B2C">B2C</option>
          ...
          <div class="form-group">
            ...
            <!-- The browser will limit numeric input; dynamic validation is added in the script -->
            <input type="number" min="0" max="99" class="form-control" id="mark_up" name="mark_up" required />
          </div>
          <!-- Buttons -->
          <button type="submit" class="btn btn-success" id="submitButton">Save</button>
          <button type="button" class="btn btn-success" id="clearForm">Clear</button>
          <button type="button" class="btn btn-success" id="cancelEdit" style="display: none;">Cancel</button>
        </form>
      </div>
    </div>

    <!-- DataTable for MarkUp List -->
    <div class="card">
      <div class="card-header">
        <h3 class="card-title mb-0">MarkUp List</h3>
      </div>
      <div class="card-body">
 ...
 <!-- Scripts -->
  ...
  <script>
    $(document).ready(function() {
      // Initialize DataTable with a serial number column and custom action icons
      let table = $('#markupTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
          url: "<?= site_url('Markups/fetch') ?>",
          type: "POST",
          dataSrc: ""
        },
        columns: [
          {
            // Serial Number column (not from the server)
            data: null,
          ...
          { data: 'mark_up' },
          {
            ...
            render: function(data, type, row) {
              let html = '';
              html += '<i class="fas fa-edit action-icon editAssign" data-id="' + row.mark_up_id + '" title="Edit" style="margin-right: 10px; cursor: pointer; color: green;"></i>';
              html += '<i class="fas fa-trash action-icon deleteAssign" data-id="' + row.mark_up_id + '" title="Delete" style="cursor: pointer; color: green;"></i>';
              return html;
            }
          }
        ]
      ...
      // Function to reset the entire form to default state (used for Cancel)
      function resetForm() {
        $('#markupForm')[0].reset();
        $('#mark_up_id').val('');
        $('#formTitle').text('Add MarkUp');
        $('#submitButton').text('Save');
        $('#cancelEdit').hide();
        $('#system').removeClass('invalid-input');
      }

      // Clear button event: clear only the "system" and "mark_up" fields
      ...
      // Cancel button event to reset the form if editing
      $('#cancelEdit').on('click', function() {
        resetForm();
      });

      // Edit functionality: Populate the static form with the selected row's data
      $('#markupTable').on('click', '.editAssign', function() {
        let markUpId = $(this).data('id');
        // Find row data using mark_up_id
        let rowData = table.rows().data().toArray().find(item => item.mark_up_id == markUpId);
        if (rowData) {
          $('#mark_up_id').val(rowData.mark_up_id);
          $('#system').val(rowData.system);
          $('#mark_up').val(rowData.mark_up);
          $('#formTitle').text('Edit MarkUp');
          $('#submitButton').text('Update');
          $('#cancelEdit').show();
          // Scroll to the form
          $('html, body').animate({ scrollTop: $('#markupForm').offset().top-200}, 'fast');
        }
      });

      // Delete functionality using SweetAlert2 confirmation
      $('#markupTable').on('click', '.deleteAssign', function() {
        let markUpId = $(this).data('id');
        Swal.fire({
          title: 'Are you sure?',
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?= site_url('Markups/delete') ?>",
              type: "POST",
              data: { mark_up_id: markUpId },
              dataType: "json",
              success: function(response) {
      ...
      // Live validation for the "mark_up" field to keep it between 0 and 100:
      $('#mark_up').on('input', function() {
        let value = parseFloat($(this).val());
        if (!isNaN(value)) {
          if (value < 0) {
            $(this).val(0);
          } else if (value > 100) {
            Swal.fire('Error', " cannot exceed 100");
          }
        }
      });
      
      // Save (Add/Update) functionality with duplicate check and system length validation
      $('#markupForm').on('submit', function(e) {
        e.preventDefault();

        let systemVal = $('#system').val().trim();
        let markUpVal = $('#mark_up').val().trim();

        // Validate that the "system" field has 2 to 100 characters
        if (systemVal.length < 2 || systemVal.length > 100) {
          Swal.fire('Warning', 'System name must be between 2 and 100 characters!', 'warning');
          return;
        }

        // Validate that all fields are filled
        if (!systemVal || !markUpVal) {
          Swal.fire('Warning', 'Please fill all fields!', 'warning');
          return;
        }

        // Check for duplicate "system" entries in the DataTable
        let currentId = $('#mark_up_id').val(); // empty if adding a new record
        let duplicateFound = false;
        table.rows().every(function() {
          let rowData = this.data();
          if (rowData.system.toLowerCase() === systemVal.toLowerCase()) {
            if (currentId === '' || currentId != rowData.mark_up_id) {
              duplicateFound = true;
        ...
        if (duplicateFound) {
          Swal.fire('Warning', 'Duplicate entry is not allowed!', 'warning');
          return;
        }

        // Proceed with AJAX call if no duplicate is found
        $.ajax({
          url: "<?= site_url('Markups/save') ?>",
          type: "POST",
          data: $(this).serialize(),
          dataType: "json",
          success: function(response) {
            if (response.status === 'success') {
              ...
              // After a successful save/update, fully reset the form
              resetForm();
              table.ajax.reload();
            } else {
              Swal.fire('Error', response.message, 'error');
